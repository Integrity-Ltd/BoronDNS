...
The `serve` command currently validates configuration, performs initial AXFR attempts from configured primaries, binds configured UDP/TCP listeners, parses DNS queries, and emits authoritative responses from active in-memory zone snapshots. EDNS0 OPT parsing, UDP truncation, TCP keepalive advertisement, default-off response padding, configurable ANY-query minimisation, and authorized NOTIFY-triggered AXFR refresh are partially implemented. IXFR, recurring refresh timers, expire handling, TSIG, XoT, DNSSEC record serving, RRL, and full query semantics are tracked by the specification but are not complete yet.
