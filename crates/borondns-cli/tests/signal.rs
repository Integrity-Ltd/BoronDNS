...
fn serve_ignores_sighup() {
    ...
    assert!(
        ...
        "borondns should continue running after SIGHUP"
    );
    ...
    assert!(
        status.success(),
        "borondns should exit successfully after SIGTERM, got {status}"
    );
...
fn serve_survives_closed_stdout_and_stderr_consumers() {
    ...
    assert!(
        child
            ...
            .is_none(),
        "borondns should continue running after stdout/stderr consumers close"
    );
...
fn serve_exits_successfully_on_signal(signal: &str) {
    ...
    assert!(
        status.success(),
        "borondns should exit successfully after {signal}, got {status}"
    );
...
fn spawn_server(config_path: &std::path::Path) -> Child {
    Command::new(env!("CARGO_BIN_EXE_borondns"))
        .arg("serve")
        .arg("--config")
        .arg(config_path)
        .stdout(Stdio::piped())
...
fn write_test_config() -> std::path::PathBuf {
    let unique = SystemTime::now()
        ...
        .as_nanos();
    let path = std::env::temp_dir().join(format!("borondns-signal-{unique}.toml"));
    fs::write(
        &path,
        r#"
            [server]
...
fn wait_until_running(child: &mut Child, timeout: Duration) {
    ...
    loop {
        match child.try_wait().expect("poll child") {
            Some(status) => panic!("borondns exited before signal with {status}"),
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            let _ = Command::new("kill")
                ...
                .status();
            panic!("borondns did not exit after {signal}");
        }
