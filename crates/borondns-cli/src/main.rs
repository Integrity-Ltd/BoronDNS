...
const HELP_FOOTER: &str = concat!(
    "Configuration: default path /etc/borondns-secondary/config.toml",
    "; override config-loading modes with --config or BORONDNS_CONFIG.\n",
    "Operator Deployment Guide: docs/operator-deployment-guide.md\n",
    "Project: internal BoronDNS repository; see README.md and docs/."
);
const EXAMPLE_CONFIG: &str = include_str!("../../../config/borondns.example.toml");
...
fn main() -> ExitCode {
    #[cfg(unix)]
    if let Err(error) = borondns_server::install_process_signal_dispositions() {
...
fn default_config_path() -> PathBuf {
    std::env::var_os("BORONDNS_CONFIG")
        .map(PathBuf::from)
...
fn load_config_inner(path: &Path) -> anyhow::Result<LoadedConfig> {
    ...
    borondns_server::validate_runtime_config(&config)
        .context("validating runtime configuration")?;
    borondns_server::validate_secret_store_config(&config)
    ...
    warnings.extend(
        borondns_server::runtime_config_warnings(&config)
...
fn log_filter(configured_level: &str) -> anyhow::Result<EnvFilter> {
    let override_level = std::env::var("BORONDNS_LOG_LEVEL")
...
mod tests {
    ...
    fn commands_default_to_srs_config_path() {
        let cli = Cli::try_parse_from(["borondns", "check-config"]).expect("check-config CLI");
        assert_eq!(
            selected_mode(cli).expect("mode"),
            Mode::CheckConfig(PathBuf::from(DEFAULT_CONFIG_PATH))
        );

        let cli = Cli::try_parse_from(["borondns", "serve"]).expect("serve CLI");
        assert_eq!(
            selected_mode(cli).expect("mode"),
    ...
    fn global_config_path_feeds_config_loading_modes() {
        ...
        assert_eq!(
            selected_mode(cli).expect("mode"),
            Mode::Serve(PathBuf::from("config/borondns.example.toml"))
        );
        ...
        assert_eq!(
            selected_mode(cli).expect("mode"),
            Mode::ValidateConfig(PathBuf::from("config/borondns.example.toml"))
        );
        ...
        assert_eq!(
            selected_mode(cli).expect("mode"),
            Mode::DumpConfig(PathBuf::from("config/borondns.example.toml"))
        );
    }
    ...
    fn validate_config_flag_accepts_default_or_explicit_path() {
        let cli = Cli::try_parse_from(["borondns", "--validate-config"]).expect("validate CLI");
        assert_eq!(
            selected_mode(cli).expect("mode"),
    ...
    fn dump_config_flag_accepts_default_or_explicit_path() {
        let cli = Cli::try_parse_from(["borondns", "--dump-config"]).expect("dump CLI");
        assert_eq!(
            selected_mode(cli).expect("mode"),
        ...
        let cli =
            Cli::try_parse_from(["borondns", "--dump-config", "config/borondns.example.toml"])
                .expect("dump CLI");
        assert_eq!(
            selected_mode(cli).expect("mode"),
            Mode::DumpConfig(PathBuf::from("config/borondns.example.toml"))
    ...
    fn selecting_multiple_modes_is_rejected() {
        let cli = Cli::try_parse_from(["borondns", "--dump-config", "--validate-config"])
        ...
        let cli = Cli::try_parse_from(["borondns", "--example-config", "--version"])
            .expect("CLI parses before mode validation");
        let error = selected_mode(cli).expect_err("ambiguous mode must fail");
    ...
    fn version_flag_selects_version_mode() {
        let cli = Cli::try_parse_from(["borondns", "--version"]).expect("version CLI");
        assert_eq!(selected_mode(cli).expect("mode"), Mode::Version);

        let cli = Cli::try_parse_from(["borondns", "-V"]).expect("short version CLI");
        assert_eq!(selected_mode(cli).expect("mode"), Mode::Version);
    }
    ...
    fn example_config_flag_selects_example_config_mode() {
        let cli =
            Cli::try_parse_from(["borondns", "--example-config"]).expect("example config CLI");
    ...
    fn version_text_contains_srs_build_metadata() {
        ...
        assert!(text.starts_with(&format!("borondns {}\n", env!("CARGO_PKG_VERSION"))));
    ...
    fn transfer_protocol_errors_default_to_general() {
        ...
        let protocol_errors = [
            ...
            anyhow!(TransferError::Axfr(
                borondns_core::axfr::AxfrError::EmptyResponse
            )),
            anyhow!(TransferError::Ixfr(
                borondns_core::axfr::IxfrError::EmptyResponse
            )),
            anyhow!(TransferError::Soa(
                borondns_core::axfr::SoaQueryError::MissingSoa
            )),
            anyhow!(TransferError::RandomQueryId(getrandom::Error::UNSUPPORTED)),
            anyhow!(TransferError::Tsig(
                borondns_core::tsig::TsigError::InvalidKeyName
            )),
    ...
    fn clap_help_version_and_usage_errors_have_srs_exit_codes() {
        let help = Cli::try_parse_from(["borondns", "--help"]).expect_err("help exits");
        assert!(!help.use_stderr());

        let version = Cli::try_parse_from(["borondns", "--version"]).expect("version parses");
        assert_eq!(selected_mode(version).expect("mode"), Mode::Version);

        let unknown = Cli::try_parse_from(["borondns", "--definitely-not-valid"])
    ...
    fn configured_log_level_builds_env_filter() {
        log_filter("info,borondns_server=debug").expect("valid env filter");
        let error = log_filter("borondns_server=notalevel").expect_err("invalid env filter");
    ...
    fn borondns_environment_overrides_supported_scalar_config() {
        ...
        assert_eq!(config.chaos.version, "BoronDNS anycast");
    ...
    fn logfmt_formatter_emits_structured_tracing_events() {
        ...
        assert!(text.contains(" target=borondns_test "));
