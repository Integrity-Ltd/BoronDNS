    ...
    fn parses_xot_transfer_primary_config() {
        let config = ServerConfig::from_toml_str(
            r#"
                ...
                client_cert = "/etc/borondns/client.pem"
                client_key = "/etc/borondns/client.key"
            "#,
        )
        .expect("valid config");
        ...
        assert_eq!(target.trust_anchors, vec!["/etc/borondns/ca.pem"]);
        assert_eq!(
            target.client_cert.as_deref(),
            Some("/etc/borondns/client.pem")
        );
        assert_eq!(
            target.client_key.as_deref(),
            Some("/etc/borondns/client.key")
        );
    ...
    fn parses_xot_transfer_primary_with_inline_client_key() {
        ...
        assert!(dumped.contains("client_cert = \"/etc/borondns/client.pem\""));
    ...
    fn xot_transfer_primary_enforces_trust_anchor_count_limit() {
        ...
        let anchors = format!(r#"{anchors}, "/etc/borondns/extra-ca.pem""#);
        let error = ServerConfig::from_toml_str(&format!(
            r#"
                [server]
                listen_udp = ["127.0.0.1:5300"]
