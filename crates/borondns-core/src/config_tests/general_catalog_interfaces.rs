...
fn parses_minimal_valid_config() {
    ...
    assert_eq!(
        config.zone_publication,
        ZonePublicationConfig {
    ...
    assert_eq!(
        config.zone_publication.policy(),
        ZonePublicationPolicy {
            strategy: ZonePublicationStrategy::Auto,
            sharded_rrset_threshold: 1_000_000,
            overlay_compaction_dirty_owner_threshold: 100_000,
        }
    );
...
#[test]
fn parses_and_validates_zone_publication_policy() {
    let config = ServerConfig::from_toml_str(
        r#"
                [server]
                listen_udp = ["127.0.0.1:5300"]
                ...
                [zone_publication]
                strategy = "sharded"
                sharded_rrset_threshold = 5000000
    ...
    .expect("valid sharded publication config");

    assert_eq!(
        config.zone_publication.policy(),
        ZonePublicationPolicy {
            strategy: ZonePublicationStrategy::Sharded,
            sharded_rrset_threshold: 5_000_000,
            overlay_compaction_dirty_owner_threshold: 100_000,
        }
    );
    ...
    .expect_err("zero publication threshold must fail");
    assert!(
        error
            .to_string()
            .contains("zone_publication.sharded_rrset_threshold")
    );
}
...
fn control_plane_node_ids_are_strict_opaque_segments() {
    ...
    for section in ["telemetry", "operations"] {
        for node_id in [
            ...
            let error =
                parse(section, node_id).expect_err(&format!("accepted unsafe node ID {node_id:?}"));
            assert!(
...
fn rejects_invalid_catalog_zone_names_before_runtime_startup() {
    for (name, expected) in [
        ...
        ("example", "must be an absolute DNS name ending with '.'"),
...
fn secret_store_allows_runtime_tsig_key_references() {
    ...
    assert_eq!(
        ...
        Some(std::path::Path::new("/etc/borondns/secrets.d/current"))
    );
