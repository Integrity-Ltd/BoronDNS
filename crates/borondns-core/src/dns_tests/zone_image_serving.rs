...
fn incremental_overlay_matches_fresh_compact_publication_for_dirty_and_clean_names() {
    ...
    overlay_store.insert_snapshot(base.clone());
    ...
    for packet in [
        query(
            b"\x09unchanged\x07example\x04test\x00",
            RecordType::A as u16,
            1,
        ),
        query(
            b"\x07changed\x07example\x04test\x00",
            RecordType::A as u16,
            1,
        ),
        query(
...
#[test]
fn signed_incremental_overlay_matches_fresh_compact_positive_answer() {
    let origin = DomainName::from_absolute_str("example.test.").unwrap();
    let owner = DomainName::from_absolute_str("signed.example.test.").unwrap();
    let mut old_signature = rrsig_rdata(RecordType::A);
    old_signature.push(1);
    let base = ZoneSnapshot::active(
        origin.clone(),
        Some(1),
        vec![
    ...
    overlay_store.insert_snapshot(base.clone());
    let mut new_signature = rrsig_rdata(RecordType::A);
    new_signature.push(2);
    let updated = base.with_cow_rrset_replacements(
        2,
        vec![
            (
    ...
    let mut packet = query(
        b"\x06signed\x07example\x04test\x00",
        RecordType::A as u16,
        1,
    );
    append_opt(&mut packet, 1232, 0x8000, &[]);
    ...
    assert_eq!(
        response_answer_types(&overlay_response),
        vec![RecordType::A as u16, RecordType::Rrsig as u16],
    );
}

#[test]
fn signed_overlay_reuses_clean_compact_plan_and_falls_back_for_dirty_dependencies() {
    let origin = DomainName::from_absolute_str("example.test.").unwrap();
    let clean = DomainName::from_absolute_str("clean.example.test.").unwrap();
    let changed = DomainName::from_absolute_str("changed.example.test.").unwrap();
    let mut clean_signature = rrsig_rdata(RecordType::A);
    clean_signature.push(1);
    let mut changed_signature = rrsig_rdata(RecordType::A);
    changed_signature.push(2);
    let base = ZoneSnapshot::active(
        origin.clone(),
        Some(1),
        vec![
    ...
    for (mut packet, expected_image_use) in [
        (
            query(b"\x05clean\x07example\x04test\x00", RecordType::A as u16, 1),
            true,
        ),
        (
            query(
        ...
        assert_semantic_response_eq(&overlay_response, &store_response(&packet, &compact_store));
        assert_eq!(
            observed.get().map(|metrics| metrics.zone_image_used),
            Some(expected_image_use),
        );
    }
}
...
fn zone_image_publishes_rrset_beyond_u16_and_separates_udp_from_tcp_limits() {
    ...
    let packet = query(b"\x04wide\x07example\x04test\x00", RecordType::A as u16, 1);
...
fn zone_image_direct_answer_fast_path_handles_case_insensitive_owner() {
    ...
    let direct = direct_zone_image_response_for_packet(&packet, &image, AnswerOptions::default())
...
fn zone_image_direct_answer_fast_path_rejects_unsupported_shapes() {
    ...
    let soa_with_compressible_rdata = query(b"\x07example\x04test\x00", RecordType::Soa as u16, 1);
...
fn zone_image_generic_response_capacity_uses_plan_wire_bound_before_udp_ceiling() {
    ...
    let metadata = RequestMetadata::parse(&header, &packet, &question).expect("metadata parses");
    let options = AnswerOptions::default();
    let udp_ceiling = metadata.udp_ceiling(options);
    let response_sizing = zone_image_response_sizing(&question, udp_ceiling, &metadata, options);
    let plan = image.lookup_response_plan(
...
fn zone_image_serving_handles_dnssec_do_queries() {
    ...
    let snapshot_response = store_response_with_options(&packet, &store, AnswerOptions::default());
...
fn zone_image_serving_preserves_rrsig_ttl_by_type_covered() {
    ...
    for (rr_type, expected_ttl) in [(RecordType::A as u16, 300), (RecordType::Aaaa as u16, 600)] {
