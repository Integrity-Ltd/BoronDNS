...
impl BenchConfig {
    fn from_env_and_args() -> Self {
        let mut config = Self {
            records: env_usize("BORONDNS_ZONE_IMAGE_BENCH_RECORDS", 10_000),
            zone_directory_zones: env_usize(
                "BORONDNS_ZONE_IMAGE_BENCH_ZONE_DIRECTORY_ZONES",
                1_000,
            ),
            stress_candidates: env_usize(
                "BORONDNS_ZONE_IMAGE_BENCH_STRESS_CANDIDATES",
                env_usize("BORONDNS_ZONE_IMAGE_BENCH_RECORDS", 10_000).min(2_000),
            ),
            iterations: env_usize("BORONDNS_ZONE_IMAGE_BENCH_ITERATIONS", 200_000),
            build_profile: env_string("BORONDNS_ZONE_IMAGE_BENCH_BUILD_PROFILE", "unknown"),
            git_revision: env_string("BORONDNS_ZONE_IMAGE_BENCH_GIT_REVISION", "unknown"),
            git_dirty: env_string("BORONDNS_ZONE_IMAGE_BENCH_GIT_DIRTY", "unknown"),
            kernel: env_string("BORONDNS_ZONE_IMAGE_BENCH_KERNEL", "unknown"),
            rustc: env_string("BORONDNS_ZONE_IMAGE_BENCH_RUSTC", "unknown"),
            rust_target: env_string("BORONDNS_ZONE_IMAGE_BENCH_RUST_TARGET", "unknown"),
            cpu_model: env_string("BORONDNS_ZONE_IMAGE_BENCH_CPU_MODEL", "unknown"),
            network_device: env_string("BORONDNS_ZONE_IMAGE_BENCH_NETWORK_DEVICE", "unknown"),
            artifact: env_string("BORONDNS_ZONE_IMAGE_BENCH_OUTPUT", "stdout"),
            trace_path: env_string(
                "BORONDNS_ZONE_IMAGE_BENCH_TRACE",
                "crates/borondns-core/examples/zone_image_reference_trace.tsv",
            ),
        };
...
fn time_zone_directory_linear_active_count(
    ...
    for _ in 0..iterations {
        let active_count = snapshots
            .iter()
            .filter(|snapshot| snapshot.state() == borondns_core::zone::ZoneState::Active)
            .count();
...
fn time_zone_image_response_plan(
    ...
    for index in 0..iterations {
        ...
        let plan = image.lookup_response_plan(
            ...
            query.qclass,
            8,
            borondns_core::dns::AnyResponseMode::Minimal,
        );
...
fn time_chaos_packet_response(store: &ZoneStore, packet: &[u8], iterations: usize) -> TimedLookup {
    let options = AnswerOptions {
        chaos: borondns_core::dns::ChaosOptions {
            version: "BoronDNS",
            hostname: "",
        },
        ..AnswerOptions::default()
...
fn boundary_packet_cases() -> Vec<PacketCase> {
    ...
    vec![
        ...
        PacketCase {
            ...
            options: AnswerOptions {
                any_response: borondns_core::dns::AnyResponseMode::Full,
                ..AnswerOptions::default()
            },
        },
        PacketCase {
