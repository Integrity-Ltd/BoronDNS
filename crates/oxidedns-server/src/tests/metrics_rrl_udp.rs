...
fn udp_mmsg_and_worker_metrics_are_reported() {
    ...
    assert!(body.contains("oxidedns_udp_mmsg_receive_interrupted_syscalls_total 1"));
    ...
    assert!(body.contains("oxidedns_udp_mmsg_send_interrupted_retries_total 4"));
    assert!(body.contains("oxidedns_udp_mmsg_send_resource_backoff_retries_total 3"));
...
fn af_xdp_packet_io_metrics_are_reported() {
    ...
    assert!(body.contains("oxidedns_af_xdp_tx_kick_successes_total 3"));
    assert!(body.contains("oxidedns_af_xdp_tx_kick_transient_failures_total 1"));
    assert!(body.contains("oxidedns_af_xdp_tx_delivery_failures_total 2"));
    ...
    assert!(body.contains("oxidedns_udp_receive_batches_total 1"));
    assert!(body.contains("oxidedns_udp_received_datagrams_total 19"));
    assert!(body.contains("oxidedns_udp_send_batches_total 1"));
    assert!(body.contains("oxidedns_udp_sent_datagrams_total 18"));
    assert!(body.contains(
...
fn af_xdp_kick_observation_is_exposed_without_a_batched_stats_flush() {
    ...
    assert!(body.contains("oxidedns_af_xdp_tx_wakeups_total 1"));
    assert!(body.contains("oxidedns_af_xdp_tx_kick_transient_failures_total 1"));
    assert!(body.contains("oxidedns_af_xdp_tx_kick_successes_total 0"));
    assert!(body.contains("oxidedns_af_xdp_tx_delivery_failures_total 0"));
}
...
fn zero_transport_admission_does_not_create_udp_send_batch_metrics() {
    ...
    assert!(!body.contains("oxidedns_udp_worker_send_batches_total{worker=\"1\"}"));
    assert!(!body.contains("oxidedns_af_xdp_worker_send_batches_total{worker=\"2\"}"));
}
...
fn hot_path_detail_off_suppresses_udp_packet_counters() {
    ...
    assert!(body.contains("oxidedns_udp_mmsg_receive_interrupted_syscalls_total 0"));
    assert!(body.contains("oxidedns_udp_mmsg_send_wouldblock_retries_total 0"));
    assert!(body.contains("oxidedns_udp_mmsg_send_interrupted_retries_total 0"));
    assert!(body.contains("oxidedns_udp_mmsg_send_resource_backoff_retries_total 0"));
    assert!(body.contains("oxidedns_af_xdp_rx_recv_calls_total 11"));
    assert!(body.contains("oxidedns_af_xdp_tx_send_calls_total 7"));
    assert!(body.contains("oxidedns_af_xdp_completed_packets_total 88"));
    assert!(body.contains("oxidedns_af_xdp_worker_received_packets_total{worker=\"2\"} 19"));
...
async fn assert_pending_af_xdp_send_recovery_preserves_provenance(error: ControlledUdpIoError) {
    ...
    assert!(body.contains("oxidedns_af_xdp_tx_wakeups_total 4"));
    assert!(body.contains("oxidedns_af_xdp_tx_kick_successes_total 1"));
    match error {
        ControlledUdpIoError::Kind(std::io::ErrorKind::WouldBlock) => {
            assert!(body.contains("oxidedns_af_xdp_tx_kick_transient_failures_total 3"));
            assert!(body.contains("oxidedns_af_xdp_tx_delivery_failures_total 0"));
        }
        ControlledUdpIoError::Busy => {
            assert!(body.contains("oxidedns_af_xdp_tx_kick_transient_failures_total 0"));
            assert!(body.contains("oxidedns_af_xdp_tx_delivery_failures_total 3"));
        }
...
async fn af_xdp_worker_metric_uses_exact_tx_ring_admission_count_on_wakeup_error() {
    ...
    assert!(body.contains("oxidedns_af_xdp_worker_send_batches_total{worker=\"2\"} 1"));
    assert!(body.contains("oxidedns_af_xdp_worker_sent_packets_total{worker=\"2\"} 1"));
    assert!(!body.contains("oxidedns_af_xdp_worker_sent_packets_total{worker=\"2\"} 3"));
    assert!(body.contains("oxidedns_udp_send_batches_total 1"));
    assert!(body.contains("oxidedns_udp_sent_datagrams_total 1"));
}
...
fn dedicated_udp_partial_fatal_send_records_successes_exactly_once() {
    ...
    assert!(body.contains("oxidedns_udp_mmsg_send_syscalls_total 1"));
    assert!(body.contains("oxidedns_udp_mmsg_sent_datagrams_total 1"));
    assert!(body.contains("oxidedns_udp_worker_send_batches_total{worker=\"2\"} 1"));
    assert!(body.contains("oxidedns_udp_worker_sent_datagrams_total{worker=\"2\"} 1"));
    assert!(body.contains(
        "oxidedns_query_pipeline_duration_seconds_count{stage=\"send\",query_category=\"udp_direct\"} 1"
    ));
...
fn dedicated_udp_resource_pressure_records_error_and_backoffs_without_double_counting() {
    ...
    assert!(body.contains("oxidedns_udp_send_errors_total 1"));
    assert!(body.contains("oxidedns_udp_mmsg_send_syscalls_total 1"));
    assert!(body.contains("oxidedns_udp_mmsg_sent_datagrams_total 1"));
    assert!(body.contains("oxidedns_udp_mmsg_send_partial_syscalls_total 1"));
    assert!(body.contains("oxidedns_udp_mmsg_send_wouldblock_retries_total 0"));
...
fn dedicated_udp_wouldblock_retry_exhaustion_surfaces_outer_send_error_without_backoff() {
    ...
    assert!(body.contains("oxidedns_udp_send_errors_total 1"));
    assert!(body.contains("oxidedns_udp_mmsg_send_wouldblock_retries_total 256"));
    assert!(body.contains("oxidedns_udp_mmsg_send_interrupted_retries_total 0"));
    assert!(body.contains("oxidedns_udp_mmsg_send_resource_backoff_retries_total 0"));
}
...
fn dedicated_udp_idle_shutdown_flushes_one_wouldblock_receive() {
    ...
    assert!(body.contains("oxidedns_udp_mmsg_receive_syscalls_total 0"));
    assert!(body.contains("oxidedns_udp_mmsg_receive_wouldblock_syscalls_total 1"));
    assert!(body.contains("oxidedns_udp_mmsg_received_datagrams_total 0"));
...
fn dedicated_udp_post_receive_admission_close_flushes_exact_receive_stats() {
    ...
    assert!(body.contains("oxidedns_udp_mmsg_receive_syscalls_total 1"));
    assert!(body.contains("oxidedns_udp_mmsg_receive_wouldblock_syscalls_total 0"));
    assert!(body.contains("oxidedns_udp_mmsg_received_datagrams_total 1"));
    assert_eq!(metrics.snapshot().udp_receive_batches, 0);
}
