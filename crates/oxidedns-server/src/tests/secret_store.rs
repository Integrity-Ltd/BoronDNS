...
fn secret_store_manifest_enforces_exact_byte_limit_and_growth_fence() {
    ...
    let root = unique_test_path("oxidedns-secret-store-manifest-limit", "dir");
...
fn secret_store_manifest_rejects_invalid_utf8_without_rendering_inline_secret() {
    let root = unique_test_path("oxidedns-secret-store-manifest-invalid-utf8", "dir");
    std::fs::create_dir_all(&root).expect("create secret store directory");
    let manifest_path = root.join("secrets.toml");
...
fn secret_store_material_enforces_exact_byte_limit() {
    let root = unique_test_path("oxidedns-secret-store-material-limit", "dir");
...
fn secret_store_tsig_budget_counts_repeated_files_at_exact_and_plus_one() {
    let root = unique_test_path("oxidedns-secret-store-tsig-aggregate", "dir");
    std::fs::create_dir_all(&root).expect("create secret store");
    let half = oxidedns_core::config::MAX_TSIG_ENCODED_BYTES_PER_SNAPSHOT / 2;
...
fn secret_store_tsig_key_count_is_bounded_before_material_loading() {
    let root = unique_test_path("oxidedns-secret-store-tsig-count", "dir");
    let manifest = (0..=oxidedns_core::config::MAX_TSIG_KEYS_PER_SNAPSHOT)
        .map(|index| {
            format!(
                "[[tsig_keys]]\nname = \"key-{index}.\"\nalgorithm = \"hmac-sha256\"\nsecret = \"YQ==\"\n"
            )
    ...
    assert!(error
        .to_string()
        .contains(&oxidedns_core::config::MAX_TSIG_KEYS_PER_SNAPSHOT.to_string()));
    let _ = std::fs::remove_dir_all(root);
}

#[test]
fn secret_store_xot_budget_counts_repeated_material_at_exact_and_over_limit() {
    let root = unique_test_path("oxidedns-secret-store-xot-aggregate", "dir");
    write_secret_store_manifest(&root, "");
    ...
    drop(material);
    let repetitions =
        oxidedns_core::config::MAX_XOT_TLS_MATERIAL_BYTES_PER_PROFILE / chunk_size;
    ...
    assert!(
        error.to_string().contains(
            &oxidedns_core::config::MAX_XOT_TLS_MATERIAL_BYTES_PER_PROFILE.to_string()
        )
    );

    let profile_count = oxidedns_core::config::MAX_XOT_TLS_MATERIAL_BYTES_PER_SNAPSHOT
        / oxidedns_core::config::MAX_XOT_TLS_MATERIAL_BYTES_PER_PROFILE;
    let profiles = vec![
    ...
    assert_eq!(
        ...
        oxidedns_core::config::MAX_XOT_TLS_MATERIAL_BYTES_PER_SNAPSHOT
    );
    ...
    assert!(
        error.to_string().contains(
            &oxidedns_core::config::MAX_XOT_TLS_MATERIAL_BYTES_PER_SNAPSHOT.to_string()
        )
    );
...
async fn secret_reload_only_advances_and_cancels_for_changed_material() {
    let root = unique_test_path("oxidedns-secret-store-generation", "dir");
    write_secret_store_manifest(
        &root,
        r#"
            [[tsig_keys]]
...
fn malformed_secret_store_manifest_does_not_expose_source_secret() {
    let root = unique_test_path("oxidedns-secret-store-malformed-secret", "dir");
...
fn file_secret_store_rejects_manifest_symlink_and_non_regular_file() {
    ...
    let root = unique_test_path("oxidedns-secret-store-manifest-link", "dir");
    std::fs::create_dir_all(&root).expect("create secret store root");
    let target = unique_test_path("oxidedns-secret-store-manifest-target", "toml");
    ...
    let root = unique_test_path("oxidedns-secret-store-manifest-directory", "dir");
...
fn file_secret_store_rejects_fifo_root_without_blocking() {
    let fifo = unique_test_path("oxidedns-secret-store-root-fifo", "pipe");
...
fn file_secret_store_rejects_generation_symlink_to_fifo_without_blocking() {
    ...
    let base = unique_test_path("oxidedns-secret-store-current-fifo", "dir");
...
fn file_secret_store_checks_world_mode_and_rejects_symlinked_secret_files() {
    ...
    let root = unique_test_path("oxidedns-secret-store-secret-file", "dir");
    write_secret_store_manifest(
        &root,
        r#"
            [[tsig_keys]]
...
fn file_secret_store_rejects_non_utf8_secret_file_without_rendering_secret_bytes() {
    ...
    let root = unique_test_path("oxidedns-secret-store-non-utf8", "dir");
    write_secret_store_manifest(
        &root,
        r#"
            [[tsig_keys]]
...
fn file_secret_store_rejects_writable_xot_material_and_intermediate_symlinks() {
    ...
    let root = unique_test_path("oxidedns-secret-store-xot-mode", "dir");
    ...
    let nested_target = unique_test_path("oxidedns-secret-store-nested-target", "dir");
...
fn captured_intermediate_directory_prevents_same_uid_symlink_swap() {
    ...
    let root = unique_test_path("oxidedns-secret-store-intermediate-race", "dir");
    let outside = unique_test_path("oxidedns-secret-store-intermediate-outside", "dir");
...
async fn xot_path_only_reload_refreshes_provenance_without_cancelling_material_generation() {
    let root = unique_test_path("oxidedns-secret-store-xot-provenance", "dir");
...
fn file_secret_store_enforces_xot_profile_and_trust_anchor_count_limits() {
    let root = unique_test_path("oxidedns-secret-store-xot-count-limits", "dir");
    ...
    let profiles = (0..oxidedns_core::config::MAX_XOT_PROFILES_PER_SNAPSHOT)
        .map(|index| {
            format!(
                r#"
                    [[xot_profiles]]
    ...
    assert!(
        error
            .to_string()
            .contains(&oxidedns_core::config::MAX_XOT_PROFILES_PER_SNAPSHOT.to_string())
    );

    let anchors = std::iter::repeat_n(
        r#""anchor.pem""#,
        oxidedns_core::config::MAX_XOT_TRUST_ANCHORS_PER_PROFILE,
    )
    .collect::<Vec<_>>()
    .join(", ");
    write_secret_store_manifest(
    ...
    assert!(
        error.to_string().contains(
            &oxidedns_core::config::MAX_XOT_TRUST_ANCHORS_PER_PROFILE.to_string()
        )
    );
...
fn transfer_resolves_xot_and_tsig_from_one_snapshot_across_reload() {
    let root = unique_test_path("oxidedns-transfer-secret-generation", "dir");
...
fn file_secret_store_xot_material_is_one_immutable_generation() {
    let root = unique_test_path("oxidedns-secret-store-xot-generation", "dir");
...
fn captured_secret_root_prevents_cross_generation_xot_and_tsig_mixing() {
    ...
    let base = unique_test_path("oxidedns-secret-store-generation-switch", "dir");
