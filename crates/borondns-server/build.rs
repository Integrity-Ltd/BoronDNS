...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=BORONDNS_BUILD_COMMIT");
    println!("cargo:rerun-if-env-changed=BORONDNS_BUILD_RUST_VERSION");
    println!("cargo:rerun-if-env-changed=BORONDNS_BUILD_TIMESTAMP");

    let commit = std::env::var("BORONDNS_BUILD_COMMIT")
        .ok()
        .unwrap_or_else(|| command_output("git", &["rev-parse", "--short=8", "HEAD"]));
    let rust_version = std::env::var("BORONDNS_BUILD_RUST_VERSION")
        .ok()
        .unwrap_or_else(|| command_output("rustc", &["--version"]));
    let build_timestamp = std::env::var("BORONDNS_BUILD_TIMESTAMP")
        .ok()
        .unwrap_or_else(|| command_output("date", &["-u", "+%Y-%m-%dT%H:%M:%SZ"]));

    println!("cargo:rustc-env=BORONDNS_BUILD_COMMIT={commit}");
    println!("cargo:rustc-env=BORONDNS_BUILD_RUST_VERSION={rust_version}");
    println!("cargo:rustc-env=BORONDNS_BUILD_TIMESTAMP={build_timestamp}");
}
