...
async fn hidden_active_catalog_does_not_satisfy_readiness() {
    ...
    assert!(metrics.contains("borondns_zones_active 0"));
    ...
    state.catalog_manager = CatalogManager {
        catalogs_by_key: Arc::new(HashMap::from([(
            ...
            CatalogRuntimeConfig {
                ...
                config: borondns_core::config::CatalogZoneConfig {
                    ...
                    member_transfer_policy:
                        borondns_core::config::CatalogMemberTransferPolicyConfig::default(),
...
async fn observability_api_reports_profile_backed_xot_certificates_from_current_snapshot() {
    let root = unique_test_path("borondns-observability-xot-profile", "dir");
...
fn metrics_body_reports_catalog_membership() {
    ...
    assert!(metrics.contains(
            "borondns_catalog_member_info{catalog_zone=\"catalog.example.\",zone=\"alpha.example.\",managed=\"true\"} 1"
        ));
    assert!(metrics.contains(
            "borondns_catalog_member_info{catalog_zone=\"catalog.example.\",zone=\"static.example.\",managed=\"false\"} 1"
        ));
}
...
async fn health_endpoint_handles_readyz_metrics_404_and_405() {
    ...
    assert!(metrics.contains("borondns_zones_total 2"));
    ...
    assert!(metrics.contains("borondns_query_responses_total{rcode=\"SERVFAIL\"} 0"));
    assert!(metrics.contains("borondns_query_responses_total{rcode=\"NXDOMAIN\"} 1"));
    assert!(metrics.contains("borondns_query_responses_total{rcode=\"BADCOOKIE\"} 1"));
    assert!(metrics.contains("borondns_secondary_query_responses_total{rcode=\"NOERROR\"} 1"));
    assert!(metrics.contains("borondns_secondary_query_responses_total{rcode=\"BADCOOKIE\"} 1"));
    assert!(metrics.contains(concat!(
        "borondns_secondary_build_info{version=\"",
        env!("CARGO_PKG_VERSION"),
        "\",commit=\""
    )));
    assert!(metrics.contains("rust_version=\"rustc "));
    assert!(metrics.contains(
            "borondns_secondary_query_duration_seconds_bucket{query_category=\"udp_direct\",le=\"0.0001\"} 0"
        ));
    assert!(metrics.contains(
            "borondns_secondary_query_duration_seconds_bucket{query_category=\"udp_direct\",le=\"0.00025\"} 1"
        ));
    assert!(metrics.contains(
        "borondns_secondary_query_duration_seconds_sum{query_category=\"udp_direct\"} 0.000250000"
    ));
    assert!(metrics.contains(
        "borondns_secondary_query_duration_seconds_count{query_category=\"udp_direct\"} 1"
    ));
    assert!(metrics.contains(
            "borondns_secondary_query_duration_seconds_bucket{query_category=\"tcp_cname_chain\",le=\"0.0025\"} 0"
        ));
    assert!(metrics.contains(
            "borondns_secondary_query_duration_seconds_bucket{query_category=\"tcp_cname_chain\",le=\"0.005\"} 1"
        ));
    assert!(metrics.contains("borondns_dns_cookie_queries_total{case=\"no_cookie\"} 1"));
    assert!(metrics.contains("borondns_dns_cookie_queries_total{case=\"client_only\"} 1"));
    assert!(metrics.contains("borondns_dns_cookie_queries_total{case=\"valid_server\"} 1"));
    assert!(metrics.contains("borondns_dns_cookie_queries_total{case=\"invalid_server\"} 1"));
    assert!(metrics.contains("borondns_dns_cookie_badcookie_responses_total 1"));
    assert!(metrics.contains(
            "borondns_dns_cookie_queries_by_prefix_total{source_prefix=\"192.0.2.0/24\",case=\"no_cookie\"} 1"
        ));
    assert!(metrics.contains(
            "borondns_dns_cookie_queries_by_prefix_total{source_prefix=\"192.0.2.0/24\",case=\"client_only\"} 1"
        ));
    assert!(metrics.contains(
            "borondns_dns_cookie_queries_by_prefix_total{source_prefix=\"2001:db8::/56\",case=\"valid_server\"} 1"
        ));
    assert!(metrics.contains(
        "borondns_dns_cookie_badcookie_responses_by_prefix_total{source_prefix=\"192.0.2.0/24\"} 1"
    ));
    assert!(metrics.contains("borondns_secondary_configuration_warnings_total 4"));
    assert!(metrics.contains("borondns_dnssec_nsec3_iterations_exceed_cap_total 1"));
    assert!(metrics.contains("borondns_chaos_queries_total{outcome=\"answered\"} 1"));
    assert!(metrics.contains("borondns_chaos_queries_total{outcome=\"missing_value\"} 1"));
    assert!(metrics.contains("borondns_chaos_queries_total{outcome=\"unrecognized_name\"} 1"));
    assert!(metrics.contains("borondns_chaos_queries_total{outcome=\"non_txt\"} 1"));
    assert!(metrics.contains("borondns_transfer_sessions_started_total{protocol=\"axfr\"} 1"));
    assert!(metrics.contains("borondns_transfer_sessions_started_total{protocol=\"ixfr\"} 0"));
    assert!(metrics.contains("borondns_transfer_sessions_completed_total{protocol=\"axfr\"} 1"));
    assert!(metrics.contains("borondns_transfer_sessions_failed_total{protocol=\"axfr\"} 0"));
    assert!(metrics.contains("borondns_notify_messages_received_total 1"));
    assert!(metrics.contains("borondns_notify_messages_unauthorized_total 1"));
    assert!(metrics.contains("borondns_notify_refresh_actions_total{action=\"signalled\"} 1"));
    assert!(metrics.contains("borondns_notify_refresh_actions_total{action=\"deduplicated\"} 1"));
    assert!(metrics.contains("borondns_tsig_notify_verifications_total{result=\"ok\"} 1"));
    assert!(metrics.contains("borondns_tsig_notify_verifications_total{result=\"badkey\"} 1"));
    assert!(metrics.contains("borondns_tsig_notify_verifications_total{result=\"badsig\"} 1"));
    assert!(metrics.contains("borondns_tsig_notify_verifications_total{result=\"badtime\"} 1"));
    assert!(metrics.contains("borondns_tsig_notify_verifications_total{result=\"badalg\"} 1"));
    assert!(metrics.contains("borondns_tsig_notify_verifications_total{result=\"badtrunc\"} 1"));
    assert!(metrics.contains("borondns_zone_state{zone=\"example.test.\",state=\"active\"} 1"));
    assert!(metrics.contains("borondns_zone_state{zone=\"example.test.\",state=\"loading\"} 0"));
    assert!(metrics.contains("borondns_zone_state{zone=\"loading.test.\",state=\"loading\"} 1"));
    assert!(metrics.contains("borondns_zone_loading_seconds{zone=\"example.test.\"} 0"));
    assert!(metrics.contains("borondns_zone_loading_seconds{zone=\"loading.test.\"} "));
    assert!(
        metrics
            .contains("borondns_secondary_zone_state{zone=\"example.test.\",state=\"active\"} 1")
    );
    assert!(
        metrics
            .contains("borondns_secondary_zone_state{zone=\"example.test.\",state=\"loading\"} 0")
    );
    assert!(
        metrics
            .contains("borondns_secondary_zone_state{zone=\"loading.test.\",state=\"loading\"} 1")
    );
    assert!(metrics.contains("borondns_secondary_zone_loading_seconds{zone=\"example.test.\"} 0"));
    assert!(metrics.contains("borondns_secondary_zone_loading_seconds{zone=\"loading.test.\"} "));
    assert!(!metrics.contains("borondns_zone_soa_serial{zone=\"loading.test.\"}"));
    assert!(metrics.contains("borondns_zone_soa_serial{zone=\"example.test.\"} 1"));
    assert!(!metrics.contains("borondns_secondary_zone_soa_serial{zone=\"loading.test.\"}"));
    assert!(metrics.contains("borondns_secondary_zone_soa_serial{zone=\"example.test.\"} 1"));
    assert!(metrics.contains("borondns_zone_shape_rrsets{zone=\"example.test.\"} 1"));
    assert!(metrics.contains("borondns_zone_shape_rdata_records{zone=\"example.test.\"} 1"));
    assert!(metrics.contains("borondns_zone_shape_single_rdata_rrsets{zone=\"example.test.\"} 1"));
    assert!(metrics.contains("borondns_zone_shape_multi_rdata_rrsets{zone=\"example.test.\"} 0"));
    assert!(metrics.contains("borondns_zone_shape_spilled_rdata_rrsets{zone=\"example.test.\"} 0"));
    assert!(metrics.contains("borondns_zone_shape_max_rdata_per_rrset{zone=\"example.test.\"} 1"));
    assert!(metrics.contains("borondns_zone_shape_owner_names{zone=\"example.test.\"} 1"));
    assert!(
        metrics.contains("borondns_zone_shape_empty_non_terminal_names{zone=\"example.test.\"} 0")
    );
    assert!(
        metrics
            .contains("borondns_zone_shape_name_key_deduplicated_bytes{zone=\"example.test.\"} 13")
    );
    assert!(metrics.contains(
        "borondns_zone_shape_child_name_fanout_names{zone=\"example.test.\",bucket=\"0\"} 1"
    ));
    assert!(metrics.contains(
    ...
    assert!(metrics.contains(
            "borondns_zone_shape_rdata_payload_bytes_per_rrset{zone=\"example.test.\",bucket=\"33_64\"} 1"
        ));
    assert!(!metrics.contains("borondns_zone_shape_rrsets{zone=\"loading.test.\"}"));
    assert!(metrics.contains(
        "borondns_zone_last_success_timestamp_seconds{zone=\"example.test.\"} 1700000000"
    ));
    assert!(metrics.contains(
        "borondns_secondary_zone_last_refresh_seconds{zone=\"example.test.\"} 1700000000"
    ));
    assert!(metrics.contains(
        "borondns_zone_next_refresh_timestamp_seconds{zone=\"example.test.\"} 1700003600"
    ));
    assert!(metrics.contains(
        "borondns_secondary_zone_next_refresh_seconds{zone=\"example.test.\"} 1700003600"
    ));
    assert!(
        !metrics.contains("borondns_zone_last_success_timestamp_seconds{zone=\"loading.test.\"}")
    );
    assert!(
        !metrics.contains("borondns_secondary_zone_last_refresh_seconds{zone=\"loading.test.\"}")
    );
    assert!(metrics.contains(
        "borondns_zone_next_refresh_timestamp_seconds{zone=\"loading.test.\"} 1700000060"
    ));
    assert!(metrics.contains(
        "borondns_secondary_zone_next_refresh_seconds{zone=\"loading.test.\"} 1700000060"
    ));
    assert!(
        metrics.contains("borondns_zone_refresh_failures_since_success{zone=\"example.test.\"} 0")
    );
    assert!(
        metrics.contains("borondns_zone_refresh_failures_since_success{zone=\"loading.test.\"} 1")
    );
    assert!(metrics.contains("borondns_secondary_zone_refresh_failures{zone=\"example.test.\"} 0"));
    assert!(metrics.contains("borondns_secondary_zone_refresh_failures{zone=\"loading.test.\"} 1"));
    assert!(metrics.contains("borondns_zone_queries_total{zone=\"example.test.\"} 2"));
    assert!(metrics.contains("borondns_zone_queries_total{zone=\"loading.test.\"} 0"));
    assert!(metrics.contains("borondns_secondary_queries_total{zone=\"example.test.\"} 2"));
    assert!(metrics.contains("borondns_secondary_queries_total{zone=\"loading.test.\"} 0"));
    assert!(metrics.contains(
        "borondns_zone_query_responses_total{zone=\"example.test.\",rcode=\"NOERROR\"} 1"
    ));
    assert!(metrics.contains(
        "borondns_zone_query_responses_total{zone=\"example.test.\",rcode=\"NXDOMAIN\"} 1"
    ));
    assert!(metrics.contains(
        "borondns_secondary_query_responses_total{zone=\"example.test.\",rcode=\"NOERROR\"} 1"
    ));
    assert!(metrics.contains(
        "borondns_secondary_query_responses_total{zone=\"loading.test.\",rcode=\"NOERROR\"} 0"
    ));
    ...
    assert!(decoded_metrics.contains("borondns_zones_total 2"));
