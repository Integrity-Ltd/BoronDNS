...
sweep_dir="${BORONDNS_UDP_RUNTIME_SWEEP_DIR:-$repo_root/target/evidence/udp-runtime-sweep-$timestamp}"
runtimes_raw="${BORONDNS_UDP_RUNTIME_SWEEP_RUNTIMES:-tokio dedicated}"
workers_raw="${BORONDNS_UDP_RUNTIME_SWEEP_WORKERS:-1 4}"
batch_sizes_raw="${BORONDNS_UDP_RUNTIME_SWEEP_BATCH_SIZES:-32 128 256 512}"
client_sockets_raw="${BORONDNS_UDP_RUNTIME_SWEEP_CLIENT_SOCKETS_PER_THREAD:-1 4}"
affinity_modes_raw="${BORONDNS_UDP_RUNTIME_SWEEP_AFFINITY_MODES:-none auto}"
overwrite="${BORONDNS_UDP_RUNTIME_SWEEP_OVERWRITE:-false}"
preflight_only="${BORONDNS_UDP_RUNTIME_SWEEP_PREFLIGHT_ONLY:-false}"
trace_override="${BORONDNS_UDP_RUNTIME_SWEEP_TRACE_FILE:-}"

case "$overwrite" in
true | false) ;;
*)
    printf 'BORONDNS_UDP_RUNTIME_SWEEP_OVERWRITE must be true or false, got %q\n' "$overwrite" >&2
    exit 64
    ;;
esac
...
true | false) ;;
*)
    printf 'BORONDNS_UDP_RUNTIME_SWEEP_PREFLIGHT_ONLY must be true or false, got %q\n' "$preflight_only" >&2
    exit 64
    ;;
esac
if [[ -n "$trace_override" && ! -f "$trace_override" ]]; then
    printf 'BORONDNS_UDP_RUNTIME_SWEEP_TRACE_FILE does not exist: %s\n' "$trace_override" >&2
    exit 64
fi
...
for runtime in "${runtimes[@]}"; do
    ...
    tokio | dedicated) ;;
    *)
        printf 'unsupported runtime in BORONDNS_UDP_RUNTIME_SWEEP_RUNTIMES: %q\n' "$runtime" >&2
        exit 64
        ;;
    esac
done
...
export BORONDNS_BENCH_RECORDS="${BORONDNS_BENCH_RECORDS:-10000}"
export BORONDNS_BENCH_STRESS_CANDIDATES="${BORONDNS_BENCH_STRESS_CANDIDATES:-0}"
...
export BORONDNS_BENCH_HOT_PATH_DETAIL="${BORONDNS_BENCH_HOT_PATH_DETAIL:-reduced}"
...
if [[ "$preflight_only" == true ]]; then
    ...
    BORONDNS_DNS_CLIENT_BENCHMARK_DIR="$sweep_dir/preflight" \
        BORONDNS_BENCH_UDP_RUNTIME="${runtimes[0]}" \
        BORONDNS_BENCH_UDP_REUSEPORT_WORKERS="${workers_list[0]}" \
        BORONDNS_BENCH_UDP_BATCH_SIZE="${batch_sizes[0]}" \
        BORONDNS_BENCH_UDP_CLIENT_SOCKETS_PER_THREAD="${client_sockets_list[0]}" \
...
if [[ -d "$sweep_dir" ]] && [[ -n "$(find "$sweep_dir" -mindepth 1 -maxdepth 1 -print -quit)" ]]; then
    if [[ "$overwrite" != true ]]; then
        ...
        printf 'Set BORONDNS_UDP_RUNTIME_SWEEP_OVERWRITE=true or choose a new BORONDNS_UDP_RUNTIME_SWEEP_DIR.\n' >&2
        exit 64
    fi
    rm -rf "$sweep_dir"
fi
            ...
            for client_sockets in "${client_sockets_list[@]}"; do
                for affinity_mode in "${affinity_modes[@]}"; do
                    ...
                    run_env=(
                        ...
                        BORONDNS_BENCH_UDP_RUNTIME="$runtime"
                        BORONDNS_BENCH_UDP_REUSEPORT_WORKERS="$workers"
                        BORONDNS_BENCH_UDP_BATCH_SIZE="$batch_size"
                        BORONDNS_BENCH_UDP_CLIENT_SOCKETS_PER_THREAD="$client_sockets"
                    )
                    if [[ -n "$affinity_value" ]]; then
                        run_env+=(BORONDNS_BENCH_UDP_WORKER_CPU_AFFINITY="$affinity_value")
                    else
                        run_env+=(BORONDNS_BENCH_UDP_WORKER_CPU_AFFINITY="")
                    fi
                    if [[ -n "$retained_trace" ]]; then
                        run_env+=(BORONDNS_BENCH_TRACE_FILE="$retained_trace")
                    else
                        run_env+=(BORONDNS_BENCH_TRACE_ENABLED=true)
                    fi
