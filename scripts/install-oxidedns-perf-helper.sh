...
script_path="$repo_root/scripts/install-oxidedns-perf-helper.sh"
source_helper="$repo_root/scripts/oxidedns-perf-capture-root.sh"
installed_helper="/usr/local/libexec/oxidedns-perf-capture"
...
Installs a root-owned OxideDNS perf helper and a narrow sudoers rule for the
...
The sudoers rule allows the current user to run only:
  ...
  /usr/local/libexec/oxidedns-perf-capture *
EOF
}
...
sudoers_path="/etc/sudoers.d/oxidedns-perf-capture-$target_user"
tmp_sudoers="$(mktemp)"
cat >"$tmp_sudoers" <<EOF
# Installed by OxideDNS scripts/install-oxidedns-perf-helper.sh.
...
printf 'usage=OXIDEDNS_BENCH_PERF_PRIVILEGED_HELPER=true OXIDEDNS_BENCH_PERF_STAT=true scripts/benchmark-dns-clients.sh\n'
