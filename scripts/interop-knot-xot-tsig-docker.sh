...
BDS-FR-XOT-001	retained-real-primary	knot_xot_tsig_axfr_tls	primary-version.txt; alpn-probe.txt; borondns.log; knot-xot-tsig-summary.env	BoronDNS successfully transfers TSIG-authenticated AXFR over TLS from a real Knot primary; logs retain negotiated TLS version and cipher-suite fields for release review.
BDS-FR-XOT-002	retained-real-primary	knot_xot_tsig_cipher_observed	borondns.log; alpn-probe.txt	The BoronDNS XoT+TSIG session log records the negotiated cipher suite; broader prohibited-suite rejection remains covered by release TLS-matrix review.
BDS-FR-XOT-003	retained-real-primary	knot_xot_tsig_port_override	borondns.toml.redacted; primary-version.txt	The primary uses an explicit per-primary XoT port override in configuration rather than cleartext TCP.
BDS-FR-XOT-004	retained-real-primary	knot_xot_tsig_alpn_dot	alpn-probe.txt; borondns.log; knot-xot-tsig-summary.env	Knot and BoronDNS negotiate ALPN dot; missing-ALPN abort behavior remains covered by focused tests.
BDS-FR-XOT-005	retained-real-primary	knot_xot_tsig_certificate_validation	server-certificate.txt; ca.crt; borondns.toml.redacted; readyz.json	BoronDNS trusts the configured CA and validates the SAN/SNI-bound real-primary certificate before publishing the transferred zone.
BDS-FR-XOT-006	retained-real-primary	knot_xot_tsig_no_cleartext_fallback	borondns.log; metrics.txt; knot-xot-tsig-summary.env	The retained log has no XoT connection or TLS failure markers and transfer metrics show the TLS AXFR completed without cleartext fallback.
BDS-FR-XOT-008	retained-real-primary	knot_xot_tsig_required	unsigned-xot-axfr.out; signed-xot-axfr.out; knot-xot-tsig-summary.env	The real primary rejects unsigned XoT AXFR and accepts HMAC-SHA256 TSIG over XoT; retained BoronDNS artifacts prove publication from the signed transfer.
BDS-FR-XOT-011	retained-real-primary	knot_xot_tsig_session_logging	borondns.log; knot-xot-tsig-summary.env	BoronDNS logs XoT TLS session establishment with version/cipher and session close with byte counters while retaining TLS key-material and TSIG-secret redaction evidence.
EOF
