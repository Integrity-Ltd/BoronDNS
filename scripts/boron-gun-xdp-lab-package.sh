...
usage() {
    cat >&2 <<'EOF'
Usage:
  scripts/boron-gun-xdp-lab-package.sh EVIDENCE_DIR

Validates an BoronGun XDP lab evidence directory, writes a SHA-256 manifest
inside it, and creates a tar.gz package under target/boron-gun-xdp-evidence-packages.
EOF
}
...
if not evidence.get("boron_gun"):
    raise SystemExit("boron_gun summary is missing from evidence-summary.json")
if not preflight:
...
package_dir="$repo_root/target/boron-gun-xdp-evidence-packages"
mkdir -p "$package_dir"
...
printf 'boron-gun XDP lab evidence package: %s\n' "$package"
printf 'boron-gun XDP lab evidence package sha256: %s.sha256\n' "$package"
