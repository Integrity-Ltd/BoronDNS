...
binary="${1:-$repo_root/target/debug/oxidedns}"
if [[ "$binary" != /* ]]; then
    binary="$repo_root/$binary"
fi
out_dir="$repo_root/target/oxidedns-af-xdp-veth-smoke"
...
srv_ip="198.18.10.53"
peer_ip="198.18.10.1"
run_as_user="${OXIDEDNS_SMOKE_RUN_AS_USER:-nobody}"

redirect_object="${OXIDEDNS_XDP_REDIRECT_OBJECT:-}"
if [[ -z "$redirect_object" ]]; then
    built_object="$repo_root/crates/oxidedns-server-ebpf/target/bpfel-unknown-none/release/oxidedns-xdp-redirect.bpf.o"
    if [[ ! -f "$built_object" ]]; then
        echo "OxideDNS XDP redirect object is missing." >&2
        echo "Build it before running this root smoke: ./scripts/oxidedns-server-build-ebpf.sh" >&2
        exit 1
    fi
...
if [[ ! -f "$redirect_object" ]]; then
    echo "OxideDNS XDP redirect object does not exist: $redirect_object" >&2
    exit 1
fi

if [[ "$(id -u)" -ne 0 ]]; then
    echo "OxideDNS AF_XDP veth smoke requires root; run with pkexec or sudo" >&2
    exit 77
fi

if [[ ! -x "$binary" ]]; then
    echo "oxidedns binary is not executable: $binary" >&2
    echo "build it with: cargo build -p oxidedns-cli --features af-xdp" >&2
    exit 1
fi
...
if [[ "$status" -ne 124 ]]; then
    echo "OxideDNS AF_XDP veth smoke failed: server exited with status $status" >&2
    cat "$log" >&2
    exit 1
fi

if ! grep -q "UDP listener bound" "$log"; then
    echo "OxideDNS AF_XDP veth smoke failed: UDP listener did not bind" >&2
    cat "$log" >&2
    exit 1
fi

printf 'OxideDNS AF_XDP veth smoke passed: %s\n' "$out_dir"
