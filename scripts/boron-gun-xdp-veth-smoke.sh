...
out_dir="$repo_root/target/boron-gun-xdp-veth-smoke"
...
if [[ "$(id -u)" -ne 0 ]]; then
    echo "boron-gun XDP veth smoke requires root; run with pkexec or sudo" >&2
    exit 77
fi
...
printf 'boron-gun XDP veth smoke passed: %s\n' "$out_dir"
