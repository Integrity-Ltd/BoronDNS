...
REMOTE
    ...
    ssh_control "$server_ssh" bash -s -- "$out_abs" "$original_server_txqueuelen" "${server_txqueuelen:-__none__}" "$effective_txqueuelen" "$server_tx_fq_limit" "${server_tx_fq_flow_limit:-__none__}" "$original_server_wmem_max" "${server_wmem_max:-__none__}" "$effective_wmem_max" <<'REMOTE'
set -euo pipefail
out_abs="$1"
original_txqueuelen="$2"
requested_txqueuelen="$3"
effective_txqueuelen="$4"
requested_fq_limit="$5"
requested_fq_flow_limit="$6"
original_wmem_max="$7"
requested_wmem_max="$8"
effective_wmem_max="$9"
...
python3 - "$target" "$workers" "$rate" "$kxdpgun_batch" "$kxdpgun_mode" "$udp_batch_size" "$hot_path" "$idle_strategy" "$socket_receive_buffer" "$socket_send_buffer" "$socket_max_pacing_rate" "$server_txqueuelen" "$server_tx_qdisc" "$server_tx_fq_limit" "$server_tx_fq_flow_limit" "$server_wmem_max" "$cpus" "$server_prefix" "$server_interface" "$run_abs" "$run_abs/kxdpgun.log" >>"$out_abs/summary.tsv" <<'PY'
import re
import sys

target, workers, rate, kxdpgun_batch, kxdpgun_mode, udp_batch_size, hot_path, idle_strategy, socket_receive_buffer, socket_send_buffer, socket_max_pacing_rate, server_txqueuelen, server_tx_qdisc, server_tx_fq_limit, server_tx_fq_flow_limit, server_wmem_max, cpus, server_prefix, interface, run_abs, log = sys.argv[1:22]
