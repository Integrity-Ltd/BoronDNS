...
if ! docker run -d --name "$container" \
    ...
    -v "$workdir:/work:rw" \
    "$nsd_image" \
    sh -c 'nsd-checkzone alpha.test. /work/alpha.test.zone >/dev/null && nsd-checkconf /work/nsd.conf && exec nsd -d -c /work/nsd.conf' \
    >/dev/null; then
    echo "skipping NSD NOTIFY Docker interop: failed to start Alpine/NSD container" >&2
    exit 0
fi
record_docker_primary_version "$workdir" "$container" "NSD" "$nsd_image" "nsd" "nsd-notify-refresh" "udp-notify+tcp-axfr" "none" "nsd -v" "$workdir/nsd.conf" "$workdir/alpha.test.zone"
...
BDS-FR-NOTIFY-001	retained-real-primary	nsd_udp_notify_reception	notify-proxy.log; primary-version.txt	The NSD primary emits OPCODE=4 NOTIFY packets observed by the forwarding proxy and BoronDNS receives them on the DNS listener.
BDS-FR-NOTIFY-006	retained-real-primary	nsd_notify_response	notify-proxy.log	The forwarding proxy observes a successful BoronDNS NOTIFY response with RCODE=0 for NSD-generated NOTIFY.
BDS-FR-NOTIFY-007	retained-real-primary	nsd_refresh_signal	metrics.txt; nsd-notify-refresh-summary.tsv	BoronDNS metrics record real-primary NOTIFY receipt and refresh-signalled actions, and the served zone advances from serial 2026052401 to 2026052402.
BDS-FR-NOTIFY-010	retained-real-primary	nsd_notify_logging	borondns.log	BoronDNS emits an accepted NOTIFY log for the real-primary NSD message, including source, zone, and refresh action.
BDS-FR-ZSM-003	retained-real-primary	nsd_notify_triggered_refresh	nsd-notify-refresh-summary.tsv; metrics.txt	The accepted real-primary NOTIFY triggers the refresh path and BoronDNS republishes the updated SOA serial and A record.
EOF
