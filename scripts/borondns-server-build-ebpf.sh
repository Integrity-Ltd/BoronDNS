...
manifest="$repo_root/crates/borondns-server-ebpf/Cargo.toml"
toolchain="nightly-2026-06-12"
bpf_linker_version="0.11.0"
# Upstream's v0.11.0 x86_64-musl release binary reports 0.0.0 because its
# Bazel build omits Cargo package metadata. Pin its executable digest instead
# of accepting arbitrary binaries with that broken version string.
...
target_dir="$repo_root/crates/borondns-server-ebpf/target/bpfel-unknown-none/release"
artifact="$target_dir/libborondns_server_ebpf.so"
object="$target_dir/borondns-xdp-redirect.bpf.o"
