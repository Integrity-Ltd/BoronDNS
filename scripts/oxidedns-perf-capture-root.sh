...
usage() {
    cat >&2 <<'EOF'
Usage:
  oxidedns-perf-capture stat --pid PID --duration SECONDS --events EVENTS --output PATH
  oxidedns-perf-capture record --pid PID --duration SECONDS --frequency HZ --output PATH
...
if [[ ${EUID:-$(id -u)} -ne 0 ]]; then
    printf 'oxidedns-perf-capture must run as root through sudo or pkexec\n' >&2
    exit 77
fi
