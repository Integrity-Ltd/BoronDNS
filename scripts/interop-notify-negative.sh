...
BDS-FR-NOTIFY-001	retained-runtime	tcp_and_udp_notify_reception	notify-negative-summary.tsv; borondns.toml; crates/borondns-server/src/lib.rs::runtime_serves_queries_and_notify_on_configured_dns_interface	This harness receives NOTIFY on the configured DNS interface over both UDP and TCP, preserving the three-role interface model.
BDS-FR-NOTIFY-002	retained-runtime	non_soa_question	notify-negative-summary.tsv; client.log	A NOTIFY with QTYPE=A receives FORMERR with QID, opcode, and question echoed.
BDS-FR-NOTIFY-003	retained-runtime	unknown_zone	notify-negative-summary.tsv; client.log	A NOTIFY for an unconfigured zone receives REFUSED and no refresh action is expected.
BDS-FR-NOTIFY-004	retained-runtime	unauthorized_source_discard	notify-negative-summary.tsv; metrics.txt; borondns.log	Unauthorized sources receive no response, increment unauthorized metrics, emit the first warning log, and feed retained suppression-summary evidence for repeats.
BDS-FR-NOTIFY-005	retained-runtime	signed_valid_and_tampered_notify	notify-negative-summary.tsv; metrics.txt; borondns.log	A signed zone receiving an unsigned authorized NOTIFY returns NOTAUTH with BADKEY TSIG evidence; valid signed NOTIFY increments ok and receives signed NOERROR response evidence; tampered signed NOTIFY returns NOTAUTH with BADSIG evidence.
BDS-FR-NOTIFY-006	retained-runtime	accepted_notify_response	notify-negative-summary.tsv; client.log	Accepted and duplicate NOTIFY messages over UDP and TCP receive QR=1, OPCODE=NOTIFY, AA=1, NOERROR responses with QID and question echoed; signed valid NOTIFY responses carry a NOERROR TSIG.
BDS-FR-NOTIFY-007	retained-runtime	accepted_refresh_signalled	metrics.txt; borondns.log	Unsigned and signed accepted NOTIFY messages record action=refresh_signalled and increment the refresh signalled metric.
BDS-FR-NOTIFY-008	retained-runtime-plus-support	embedded_soa_serial	notify-negative-summary.tsv; borondns.log; crates/borondns-core/src/dns.rs notify embedded SOA tests	The accepted NOTIFY carries embedded SOA serial 2026052502 and logs it; malformed owner/class and timer-field isolation remain covered by focused tests.
BDS-FR-NOTIFY-009	retained-runtime	duplicate_deduplicated	notify-negative-summary.tsv; metrics.txt; borondns.log	A duplicate well-formed NOTIFY still receives a response but records action=deduplicated and does not create a second signalled metric.
BDS-FR-NOTIFY-010	retained-runtime	notify_logging	borondns.log	Accepted unsigned, accepted signed, deduplicated, unauthorized, and TSIG-failure paths have retained log evidence with action, source, zone, and serial where applicable.
BDS-FR-NOTIFY-011	retained-runtime	log_rate_suppression_summary	borondns.log; notify-negative-summary.tsv	This harness retains first warning logs plus a periodic aggregate summary proving repeated unauthorized and TSIG-failure NOTIFY warnings are suppressed and counted.
EOF
