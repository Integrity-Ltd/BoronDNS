#!/usr/bin/env python3
"""Check that documented RRL defaults match Rust, example TOML, and the SRS."""
...
RUST_CONFIG = ROOT / "crates" / "borondns-core" / "src" / "config.rs"
...
def main() -> None:
    ...
    for key, expected in EXPECTED.items():
        ...
        match = re.search(
            rf"fn default_rrl_{re.escape(key)}\(\) -> \w+\s*\{{\s*(true|false|[0-9_]+)\s*\}}",
            rust_config,
        )
        if not match or match.group(1).replace("_", "") != expected:
            fail(f"{RUST_CONFIG}:default_rrl_{key} must return {expected}")
