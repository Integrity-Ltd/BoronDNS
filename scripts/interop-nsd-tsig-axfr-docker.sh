...
set +e
unsigned_axfr="$(dig "@127.0.0.1" -p "$nsd_port" alpha.test. AXFR +time=2 +tries=1 2>&1)"
unsigned_status=$?
set -e
...
signed_axfr="$(dig "@127.0.0.1" -p "$nsd_port" -y "hmac-sha256:transfer-key.:$tsig_secret" alpha.test. AXFR +time=2 +tries=1)"
