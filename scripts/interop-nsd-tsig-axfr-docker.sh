...
if ((${#missing[@]} > 0)); then
    printf 'skipping NSD TSIG Docker interop: missing %s\n' "${missing[*]}" >&2
    exit 0
fi

if ! docker info >/dev/null 2>&1; then
    echo "skipping NSD TSIG Docker interop: Docker daemon is unavailable" >&2
    exit 0
fi
...
record_docker_primary_version "$workdir" "$container" "NSD" "$nsd_image" "nsd" "nsd-tsig-axfr" "tcp-axfr" "tsig-hmac-sha256" "nsd -v" "$workdir/nsd.conf" "$workdir/alpha.test.zone"
...
if [[ "$primary_soa" != *"2026052401"* ]]; then
    echo "NSD TSIG primary did not answer expected SOA serial" >&2
    exit 1
fi

set +e
dig "@127.0.0.1" -p "$nsd_port" alpha.test. AXFR +nocmd +time=2 +tries=1 >"$unsigned_axfr_out" 2>&1
unsigned_status=$?
set -e
unsigned_axfr="$(cat "$unsigned_axfr_out")"
if ((unsigned_status == 0)) && [[ "$unsigned_axfr" == *"www.alpha.test."* ]]; then
    echo "NSD TSIG primary unexpectedly allowed unsigned AXFR" >&2
    exit 1
fi

dig "@127.0.0.1" -p "$nsd_port" -y "hmac-sha256:transfer-key.:$tsig_secret" alpha.test. AXFR +nocmd +time=2 +tries=1 \
    >"$signed_axfr_out"
signed_axfr="$(cat "$signed_axfr_out")"
if [[ "$signed_axfr" != *"www.alpha.test."* ]] || [[ "$signed_axfr" != *"alias.alpha.test."* ]]; then
    echo "NSD TSIG primary signed AXFR did not include expected fixture records" >&2
    exit 1
fi
...
echo "NSD Docker TSIG AXFR interop passed"
