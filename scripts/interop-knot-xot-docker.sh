...
container="borondns-knot-xot-$$"
server_name="primary.alpha.test"
artifact_dir="${BORONDNS_KNOT_XOT_ARTIFACT_DIR:-}"
...
if [[ "$ready" != *'"status":"ready"'* ]]; then
    echo "BoronDNS did not become ready after Knot XoT AXFR" >&2
    exit 1
fi
...
if [[ "$answer_a" != *"www.alpha.test."* ]] || [[ "$answer_a" != *"192.0.2.10"* ]]; then
    echo "BoronDNS did not serve expected A response after Knot XoT AXFR" >&2
    exit 1
fi
...
if [[ "$answer_cname" != *"alias.alpha.test."* ]] || [[ "$answer_cname" != *"www.alpha.test."* ]] || [[ "$answer_cname" != *"192.0.2.10"* ]]; then
    echo "BoronDNS did not serve expected CNAME-chain response after Knot XoT AXFR" >&2
    exit 1
fi
...
if [[ "$tcp_soa" != *"2026052401"* ]]; then
    echo "BoronDNS did not serve expected TCP SOA response after Knot XoT AXFR" >&2
    exit 1
fi
...
for expected in \
    ...
    if [[ "$metrics" != *"$expected"* ]]; then
        echo "BoronDNS metrics missing expected line after Knot XoT AXFR: $expected" >&2
        exit 1
    fi
done
...
for expected_log in \
    ...
    if ! grep -F "$expected_log" "$workdir/borondns.log" >/dev/null 2>&1; then
        echo "BoronDNS XoT log missing expected field or event: $expected_log" >&2
        exit 1
    fi
done
...
borondns_ready_after_xot_axfr=1
