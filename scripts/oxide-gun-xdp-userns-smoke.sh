...
if [[ "${OXIDE_GUN_XDP_USERNS_INSIDE:-0}" != "1" ]]; then
    command -v unshare >/dev/null
    exec unshare -Urn env OXIDE_GUN_XDP_USERNS_INSIDE=1 "$0" "$binary"
fi

out_dir="$repo_root/target/oxide-gun-xdp-userns-smoke"
...
printf 'oxide-gun rootless XDP userns smoke passed: %s\n' "$out_dir"
