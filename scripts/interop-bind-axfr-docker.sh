...
if ((${#missing[@]} > 0)); then
    printf 'skipping BIND Docker interop: missing %s\n' "${missing[*]}" >&2
    exit 0
fi

if ! docker info >/dev/null 2>&1; then
    echo "skipping BIND Docker interop: Docker daemon is unavailable" >&2
    exit 0
fi
...
cp "$template_file" "$workdir/named.conf"
...
if ! docker run -d --name "$container" \
    ...
    >/dev/null; then
    echo "skipping BIND Docker interop: failed to start Alpine/BIND container" >&2
    exit 0
fi
...
if [[ "$primary_soa" != *"2026052401"* ]]; then
    echo "BIND Docker primary did not answer expected SOA serial" >&2
    exit 1
fi
...
if [[ "$primary_axfr" != *"www.alpha.test."* ]] || [[ "$primary_axfr" != *"alias.alpha.test."* ]]; then
    echo "BIND Docker primary AXFR did not include expected fixture records" >&2
    exit 1
fi
...
docker logs "$container" >"$bind_log" 2>&1 || true
...
echo "BIND Docker AXFR interop passed"
