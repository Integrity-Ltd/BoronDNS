...
evidence_dir="${OXIDEDNS_RESOURCE_EVIDENCE_DIR:-$repo_root/target/evidence/resource-$$}"
sample_seconds="${OXIDEDNS_RESOURCE_IDLE_SAMPLE_SECONDS:-3}"
...
cd "$repo_root"
cargo build --release -q -p oxidedns-cli

binary="$repo_root/target/release/oxidedns"
...
require_text "$evidence_dir/metrics.txt" 'oxidedns_secondary_build_info'
...
cp "/proc/$oxidedns_pid/status" "$evidence_dir/proc-status-after.txt"
