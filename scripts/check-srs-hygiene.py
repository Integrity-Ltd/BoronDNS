...
FORBIDDEN_TEXT = {
    ...
    "The design specified below follows the Vixie / Schryver model implemented in BIND 9": "RRL implementation models differ; define the BoronDNS model directly",
    "Vixie / Schryver RRL design; BIND 9 RRL implementation": "avoid treating BIND behavior as the direct source for BoronDNS RRL semantics",
    "Vixie/Schryver operational design": "RRL semantics are BoronDNS project policy, not a vendor-derived design claim",
    "BIND 9 default RRL configuration": "BoronDNS RRL thresholds are project defaults, not BIND defaults",
    ...
    "Operational RRL practice documented by BIND 9, Knot DNS, and NSD": "RRL semantics are BoronDNS project policy, not a vendor-derived standard",
    ...
    "then resets. Over many rate-limited responses": "RRL limited_count is retained per key until eviction or process restart",
    ...
    "comparable to NSD and Knot on equivalent hardware": "performance targets require retained BoronDNS benchmark evidence before conformance claims",
    ...
    "The 30% factor is operationally typical": "TCP target ratio is an BoronDNS formal acceptance ratio, not an unsourced practice claim",
    ...
    "BDS-NFR-OBS-008 (catalog metrics)": "catalog observability wording must match implemented membership metric plus ordinary metrics",
    "borondns_secondary_catalog_member_zones": "SRS must align with implemented catalog membership metric",
    "borondns_secondary_catalog_last_transfer_timestamp_seconds": "SRS must align with implemented catalog membership metric",
    "borondns_secondary_catalog_transfer_failures_total": "SRS must align with implemented catalog membership metric",
    "borondns_secondary_catalog_member_additions_total": "SRS must align with implemented catalog membership metric",
    "borondns_secondary_catalog_member_removals_total": "SRS must align with implemented catalog membership metric",
    "borondns_secondary_catalog_members_rejected_total": "SRS must align with implemented catalog membership metric",
    "borondns_secondary_catalog_state": "SRS must align with implemented catalog membership metric",
    ...
    "used for the metrics endpoint per BDS-NFR-OBS-003": "OpenMetrics is not the current metrics endpoint format",
    "the project prefix `borondns_secondary_` MUST be applied to every metric": "current implementation uses borondns_ as the first-party prefix with selected borondns_secondary_ stable families",
    'borondns_secondary_queries_total{zone="example.com",rcode="NOERROR"}': "borondns_secondary_queries_total is per-zone; RCODE labels belong on query response metrics",
    "de-facto operational practice": "avoid unsourced operational-practice claims in normative SRS text",
    "de-facto probe names": "CHAOS probe names are an explicit BoronDNS compatibility policy",
    ...
    "already-adopted BoronDNS": "import-process wording belongs in review disposition, not the current SRS",
    "**Document Version:** v0.9.1 (Draft 9, point release 1)": "current SRS front matter should not look like an imported draft artifact",
    "**Status:** Draft — CHAOS class self-identification addition, BoronDNS namespace alignment": "current SRS front matter should state its active baseline and acceptance boundary",
    ...
    "single-source synchronisation": "BDS-VER-014 must use canonical register wording instead of hand-maintained duplication wording",
    ...
    "BORONDNS_HEALTH_LIVEZ_TIMEOUT_MS": "SRS must not document an environment override for a removed configuration parameter",
    "explicit liveness-probe-timeout": "SRS must treat liveness probe timeout policy as external to BoronDNS configuration",
    "TSIG secret loading from environment": "SRS must align BDS-NFR-SEC-008 with inline and file-backed TSIG secrets",
    ...
    "borondns_dnssec_nsec3_cap_exceeded_total": "SRS must use the implemented global NSEC3 cap metric name",
    "per-zone counter `borondns_dnssec_nsec3_cap_exceeded_total": "SRS must not require a per-zone NSEC3 cap counter in the current profile",
...
REQUIRED_TEXT = [
    ...
    "an uncaught panic in any task is therefore a process-fatal implementation failure",
    "Supervision MUST surface ordinary task-return errors and join failures before graceful shutdown where execution remains possible",
    "The release gate MUST confirm the packaged release profile uses abort-on-panic semantics",
    ...
    "Uncaught Rust panics are process-fatal implementation bugs in the abort-on-panic release profile",
    ...
    "BoronDNS may select already-transferred NSEC or NSEC3 RRsets that prove a negative response",
    ...
    "RFC 9018 §4, §4.3, §4.4",
    "configuration MUST support a 16-octet Server Secret encoded as 32 hexadecimal characters",
    "previous Server Secret for staged rollover",
    "validate received Server Cookies against both current and previous secrets",
    "single-instance behavior keeps secret material off disk",
    "MUST NOT be combined with configured shared Server Secret material",
    "anycast or load-balanced rollover uses the current-plus-previous shared-secret mechanism",
    "BoronDNS defaults to the lenient project policy because it preserves interoperability with clients that do not yet have a Server Cookie",
    ...
    "The tested primary version and the XoT capability decision MUST be recorded per BDS-VER-013",
    ...
    "This subsection therefore defines the BoronDNS project model explicitly",
    "(no IETF RFC; BoronDNS project RRL policy)",
    "BoronDNS project default baseline, recorded in `docs/rrl-release-thresholds.md`",
    "These defaults are project defaults, not inherited vendor defaults.",
    "The counter is retained with the accounting key until that key is evicted or the process restarts.",
    "DNS Cookie valid Server Cookie exemption is specified under BDS-FR-COOKIE-009.",
    "formal project acceptance targets for the BoronDNS reference verification profile",
    ...
    "The 30% factor is the BoronDNS formal acceptance ratio for the Reference Profile",
    "NSEC3 benchmark variants MUST record the signed-zone parameters in the retained run artifacts.",
    "NSEC3-cap refusal is SERVFAIL whether EDE is enabled or disabled",
    ...
    "BoronDNS uses 100 as a compatibility default, not as an RFC compliance ceiling.",
    "responses that fail closed because the configured NSEC3 iteration cap prevents correct denial-proof construction",
    "The metric remains observable when `edns.extended_dns_errors = \"off\"` and EDE INFO-CODE 27 is absent.",
    "The counter is driven by the lookup-time cap observation, not by serialized EDE options",
    "Metric endpoint inspection after queries that exceed the configured NSEC3 iteration cap with `edns.extended_dns_errors = \"off\"`",
    ...
    "BoronDNS does not implement that option in the current scope, so it does not claim RFC 7314 compliance.",
    ...
    'borondns_secondary_query_responses_total{zone="example.com",rcode="NOERROR"}',
    ...
    "BoronDNS selects the §4.1 subset-of-available-RRsets profile because it answers from transferred zone data",
