...
REQUIRED_TEXT = [
    ...
    "RFC 9276 specifies INFO-CODE 27 for validating-resolver handling of unsupported NSEC3 iteration counts; BoronDNS uses that allocated code only as an opt-in authoritative diagnostic",
    ...
    "Engineering MVP evidence records the proof-omission signal before optional EDE emission",
    "the metric remains observable when `edns.extended_dns_errors = \"off\"` and EDE INFO-CODE 27 is absent from the response",
    "The counter is driven by the lookup-time proof-omission observation, not by serialized EDE options",
