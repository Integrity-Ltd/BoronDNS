...
container="borondns-nsd-axfr-$$"
artifact_dir="${BORONDNS_NSD_AXFR_ARTIFACT_DIR:-}"
...
if [[ "$ready" != "ready" && "$ready" != *'"status":"ready"'* ]]; then
    echo "BoronDNS did not become ready after NSD AXFR" >&2
    exit 1
fi
printf '%s\n' "$ready" >"$readyz_out"
...
if [[ "$answer_a" != *"www.alpha.test."* ]] || [[ "$answer_a" != *"192.0.2.10"* ]]; then
    echo "BoronDNS did not serve expected A response after NSD AXFR" >&2
    exit 1
fi
...
if [[ "$answer_cname" != *"alias.alpha.test."* ]] || [[ "$answer_cname" != *"www.alpha.test."* ]] || [[ "$answer_cname" != *"192.0.2.10"* ]]; then
    echo "BoronDNS did not serve expected CNAME-chain response after NSD AXFR" >&2
    exit 1
fi
...
if [[ "$tcp_soa" != *"2026052401"* ]]; then
    echo "BoronDNS did not serve expected TCP SOA response after NSD AXFR" >&2
    exit 1
fi
...
for expected in \
    ...
    if [[ "$metrics" != *"$expected"* ]]; then
        echo "BoronDNS metrics missing expected line after NSD AXFR: $expected" >&2
        exit 1
    fi
done
