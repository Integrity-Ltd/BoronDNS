...
BDS-FR-TCP-001	retained-runtime	tcp_framing_and_multi_message_exchange	client-summary.env; tcp-pipeline-summary.env; fake-primary.log	TCP client and fake primary exchange DNS messages using the two-octet length prefix; one connection carries multiple independently framed queries.
