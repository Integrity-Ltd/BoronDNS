...
against BoronDNS. The script starts a synthetic TCP AXFR primary, loads a
`perf.test.` zone into BoronDNS, pins BoronDNS to four CPUs with `taskset` when
...
BORONDNS_BENCH_UDP_BATCH_SIZE=1 \
BORONDNS_BENCH_UDP_REUSEPORT_WORKERS=1 \
BORONDNS_BENCH_RECORDS=10000 \
BORONDNS_BENCH_DURATION_SECONDS=10 \
BORONDNS_BENCH_HOT_PATH_DETAIL=full \
BORONDNS_BENCH_PIPELINE_TIMING_ENABLED=false \
BORONDNS_BENCH_ZONE_SHAPE_METRICS_ENABLED=false \
BORONDNS_BENCH_TRACE_ENABLED=false \
BORONDNS_BENCH_LISTEN_ADDRESS=127.0.0.1 \
BORONDNS_BENCH_CLIENT_SERVER=127.0.0.1 \
BORONDNS_BENCH_CLIENT_BIND=127.0.0.1:0 \
scripts/benchmark-dns-clients.sh
```
...
profile once with `BORONDNS_BENCH_PIPELINE_TIMING_ENABLED=false` and once with
`BORONDNS_BENCH_PIPELINE_TIMING_ENABLED=true`. The enabled run exports query
...
`BORONDNS_BENCH_ZONE_SHAPE_METRICS_ENABLED=true`. Keep it disabled for
...
`BORONDNS_BENCH_HOT_PATH_DETAIL=full` and once with
`BORONDNS_BENCH_HOT_PATH_DETAIL=reduced`. The reduced profile keeps coarse
...
`BORONDNS_BENCH_UDP_BATCH_SIZE=1` and once with a larger value such as `32` or
`64`. For `BORONDNS_BENCH_UDP_RUNTIME=dedicated` on Linux, the batch size feeds
...
`BORONDNS_BENCH_UDP_CLIENT_SOCKETS_PER_THREAD` above `1`. The load client then
...
run the same UDP profile with `BORONDNS_BENCH_UDP_REUSEPORT_WORKERS=1` and
then with a larger value such as `4`. Set `BORONDNS_BENCH_UDP_RUNTIME=dedicated`
to run standard UDP workers on dedicated OS threads instead of Tokio tasks. On
Linux, dedicated workers can also use
`BORONDNS_BENCH_UDP_WORKER_CPU_AFFINITY=0,1,2,3` to request explicit CPU
...
`BORONDNS_BENCH_PERF_STAT=true` and optionally
`BORONDNS_BENCH_PERF_RECORD=true`. `BORONDNS_BENCH_PERF_EVENTS` defaults to
`cycles,instructions,branches,branch-misses`. The benchmark attaches `perf` to
the BoronDNS server process for the client load window and retains
...
`/usr/local/libexec/borondns-perf-capture`, and adds a sudoers rule for the
...
kxdpgun/BoronDNS normalization explicit so Knot GB/s-style results are not
compared against BoronDNS small-response QPS in isolation.
...
`BORONDNS_UDP_BATCH_SWEEP_PREFLIGHT_ONLY=true` to validate the profile without
running it, and `BORONDNS_UDP_BATCH_SWEEP_TRACE_FILE=/path/to/query-trace.tsv`
...
`BORONDNS_BENCH_REQUIRE_NON_LOOPBACK_DEVICE=true`, and non-loopback
listen/client settings that satisfy the stricter comparator checks.
Set `BORONDNS_BENCH_PACKET_CAPTURE_ENABLED=true` to retain a bounded UDP DNS
capture for the selected `BORONDNS_BENCH_NETWORK_DEVICE`; the harness prefers
`dumpcap` when available and falls back to `tcpdump`. Use
`BORONDNS_BENCH_PACKET_CAPTURE_COUNT=N` to control the stop count. Capture
...
`BORONDNS_BENCH_TRACE_ENABLED=true`. The script generates and retains
...
set `BORONDNS_BENCH_TRACE_FILE=/path/to/query-trace.tsv`; rows use:
...
Set `BORONDNS_BENCH_STRESS_CANDIDATES=N` to also load `N` delegation and DNAME
...
`BORONDNS_ZONE_IMAGE_GATE_OVERWRITE=true` is set.
...
that reaches the BoronDNS host address and set:
...
`BORONDNS_BENCH_LISTEN_ADDRESS` controls the BoronDNS UDP/TCP listener, while
`BORONDNS_BENCH_CLIENT_SERVER` controls the load-client destination. Use a
concrete destination address, not `0.0.0.0`. `BORONDNS_BENCH_CLIENT_BIND`
controls the UDP source bind only; TCP source address selection is left to the
OS. `BORONDNS_BENCH_NETWORK_DEVICE=auto` records `lo` for loopback and otherwise
uses `ip route get` against the client destination when available.
`BORONDNS_BENCH_REQUIRE_NON_LOOPBACK_DEVICE=true` fails fast if the destination
...
resolved or configured `BORONDNS_BENCH_NETWORK_DEVICE` exists on the server
...
BoronDNS host and driving load from a second machine over SSH:
...
In SSH mode the server host still starts the synthetic primary and BoronDNS,
...
`BORONDNS_BENCH_REMOTE_CLIENT_WORKDIR` on the remote client and records the
...
`BORONDNS_BENCH_REMOTE_CLIENT_ALLOW_ARCH_MISMATCH=true` only when you intend to
...
`BORONDNS_BENCH_REMOTE_CLIENT_SSH_CONNECT_TIMEOUT_SECONDS`.
When `BORONDNS_ZONE_IMAGE_GATE_REQUIRE_NON_LOOPBACK=true`, the wrapper fails
before starting either run unless `BORONDNS_BENCH_CLIENT_MODE=ssh` and
`BORONDNS_BENCH_REMOTE_CLIENT_SSH` are set. In SSH mode the wrapper defaults
...
tune the timeout with `BORONDNS_ZONE_IMAGE_GATE_SSH_CONNECT_TIMEOUT_SECONDS`.
Unless `BORONDNS_BENCH_REMOTE_CLIENT_SSH_CONNECT_TIMEOUT_SECONDS` is set
...
starting the synthetic primary or BoronDNS.
...
`BORONDNS_BENCH_PREFLIGHT_ONLY=true scripts/benchmark-dns-clients.sh`.
...
TSIG-authenticated AXFR, starts BoronDNS pinned to four CPUs when `taskset` is
available, waits for catalog readiness, then drives randomized UDP or TCP direct
hit queries across the whole zone mix.

The default target is an 8 GiB BoronDNS resident set after catalog load:
...
BORONDNS_LARGE_BENCH_SERVER_CPUS=4 \
BORONDNS_LARGE_BENCH_CLIENT_THREADS=16 \
BORONDNS_LARGE_BENCH_CLIENT_WINDOW=64 \
BORONDNS_LARGE_BENCH_DURATION_SECONDS=60 \
...
- `BORONDNS_LARGE_BENCH_ZONES` controls the catalog member count.
- `BORONDNS_LARGE_BENCH_BIG_ZONES` selects how many members receive the large
  record count.
- `BORONDNS_LARGE_BENCH_BIG_NAMES` overrides the computed large-zone owner-name
  count when exact sizing matters.
- `BORONDNS_LARGE_BENCH_SMALL_NAMES` controls the small-zone owner-name count.
- `BORONDNS_LARGE_BENCH_TXT_BYTES` adjusts per-owner TXT payload size and is the
  simplest way to move the resident set up or down without changing query
  cardinality.
- `BORONDNS_LARGE_BENCH_ADDRESS_RECORDS_PER_NAME` controls how many A records
...
- `benchmark-phases.tsv` for phase timing, including
  `borondns_startup_to_ready` catalog transfer/load time, `warmup_serve`, and
  `measured_serve`.
...
- `latency_us_p99` and `latency_us_p999` are client-observed round-trip
  latencies.
- TCP loopback runs are useful for isolating BoronDNS in-memory lookup,
...
- With `BORONDNS_LARGE_BENCH_PIPELINE_TIMING_ENABLED=true`, the Prometheus
  `borondns_query_pipeline_duration_seconds_*` metrics split server-side parse,
...
- For saturation-only transport experiments, `BORONDNS_BENCH_HOT_PATH_DETAIL=off`
