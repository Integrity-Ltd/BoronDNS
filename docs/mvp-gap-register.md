...
| Unknown RR Handling | Unit parser/response coverage and retained `scripts/interop-unknown-rr.sh` runtime artifacts for private-use and future numeric RR transfer, exact numeric QTYPE lookup, zero-length RDATA, pointer-looking opaque RDATA, exact RDLENGTH/RDATA emission, and bit-distinct same-owner unknown RRset membership | Add retained bad-transfer fault artifacts for every prohibited pseudo/meta/reserved type if acceptance review requires runtime evidence beyond focused parser tests |
