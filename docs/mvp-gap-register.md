...
| Interface/CLI | `serve`, `check-config`, `--validate-config`, redacted `--dump-config`, `--version`/`-V`, `--help`/`-h`, config parsing, JSON/logfmt logging, `ODS_<SECTION>_<KEY>` env overrides for the scalar server/health subset plus non-fatal unrecognised-`ODS_*` warnings, implemented suspicious-warning subset for DNS Cookies disabled, global RRL allowlists, HMAC-SHA1 TSIG keys, and long TCP idle timeouts, `/livez`, JSON `/readyz`, `/healthz` readiness alias, `/metrics`, gzip-capable and per-source-IP rate-limited metrics responses, config-warning count gauge, build-info gauge, query latency histogram, SIGTERM/SIGINT handling, SIGHUP ignore behavior, SIGHUP/SIGPIPE `SIG_IGN` disposition evidence on Linux, and binary-level tests for config/usage/version/help exit codes plus config/startup/bind failure exit codes 2, 64, 71, 73, 74, and 78 | Complete SRS v0.7 Alpha alignment: broader sysexits mapping for less common runtime classes, canonical log fields, remaining suspicious-configuration warning catalogue items for SOA timers, XoT trust-anchor expiry, TSIG fudge configurability, and AXFR/IXFR ingestion-size cap configurability, optional `interface.notify`, per-zone status metrics, broader retained health/metrics/signal timing evidence, stdout/stderr broken-pipe runtime evidence, default handling evidence for SIGUSR1/SIGUSR2/SIGQUIT, and release evidence for build-info label accuracy, latency histogram behavior, version/help output retention, and metrics rate limiting under production-representative scrape traffic |
