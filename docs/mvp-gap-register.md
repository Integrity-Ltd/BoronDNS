...
| DNS Cookies | RFC 9018 version-1 server-cookie construction/validation, COOKIE option parsing, startup random runtime secret, configurable disabled/lenient/strict policy, strict BADCOOKIE extended-RCODE responses, lenient refresh of invalid server cookies, same-client validation, timestamp/source/tamper rejection, malformed length FORMERR handling, and UDP valid-cookie RRL exemption have unit/runtime tests | Add optional in-process secret rotation, startup/debug logs, cookie metrics, BIND/Knot interop evidence, and retained MVP release artifacts |
...
| Interface/CLI | `serve`, `check-config`, `--validate-config`, redacted `--dump-config`, config parsing, JSON/logfmt logging, `/healthz`, `/readyz`, `/metrics`, SIGTERM/SIGINT handling, and binary-level tests for config/usage exit codes 2, 64, and 78 | Complete SRS v0.7 Alpha alignment: full sysexits mapping for runtime/bind failures, canonical log fields, config warning catalogue, `ODS_<SECTION>_<KEY>` env naming, retained `--version`/`--help` output artifacts, optional `interface.notify`, and per-zone status metrics |
