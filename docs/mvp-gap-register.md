...
| EDNS and EDE | EDNS parsing, OPT response foundations, payload-limit tests, non-EDNS 512-octet truncation/no-OPT unit evidence, configured NSID response tests for `ODS-FR-EDNS-016..017`, bounded EDE profile tests for `ODS-FR-EDNS-018` (`Not Ready` and `Unsupported NSEC3 Iterations`), retained fake-primary runtime artifacts in `scripts/interop-dnssec-serve.sh` for non-EDNS truncation plus NSID empty/non-empty request handling, and retained `scripts/interop-edns-behavior.sh` artifacts for malformed/duplicate/misplaced OPT FORMERR, BADVERS, payload floor, exact floor, below/exact/above configured ceiling, response OPT fields, legacy DO-clearing behaviour now identified as an RFC 6840 gap, UDP/TCP keepalive behavior, configured padding, unknown-option ignore, non-EDNS truncation, NSID, metrics, and `edns-traceability.tsv` mapping `ODS-FR-EDNS-001..017` | Replace legacy response-DO evidence with RFC 6840 query-DO copy evidence, then broaden release artifacts for DNSSEC-augmented DO behavior, EDE real-client interop/traceability row, TSIG extended-RCODE interactions, and real-client interop before release signoff |
