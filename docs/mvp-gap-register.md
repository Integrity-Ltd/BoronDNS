...
| DNS Cookies | RFC 9018 version-1 server-cookie construction/validation, COOKIE option parsing, startup random runtime secret, lenient refresh of invalid server cookies, same-client validation, timestamp/source/tamper rejection, malformed length FORMERR handling, and UDP valid-cookie RRL exemption have unit/runtime tests | Add operator policy config for disabled/lenient/strict modes, strict BADCOOKIE extended-RCODE responses, secret rotation, startup/debug logs, cookie metrics, BIND/Knot interop evidence, and retained MVP release artifacts |
