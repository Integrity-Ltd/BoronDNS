...
| Fuzzing and soak | The parser fuzz targets, two-host campaign tooling, and completed ASan-backed two-host 24-hour fuzz evidence are present; `docs/two-host-fuzz-soak-campaign.md` records campaign `20260614T003811Z` with 75 fuzz services across five targets, 104,361,257,036 total runs, and no crash/leak/OOM/timeout markers. Soak report schemas and handoff scripts exist. | Decide whether the existing 24-hour fuzz evidence is accepted for the final candidate or rerun it after the release commit; run or explicitly defer the 30-day production-representative soak and attach the completed report/sign-off if claiming full SRS acceptance. |
