...
| Verification Governance | `scripts/release-evidence-snapshot.sh` captures command logs and git/tool state; evidence runners copy successful real-primary `primary-version.txt` artifacts into `interop-primary-versions/` with an index for `ODS-VER-013` sampling; `docs/release-notes-template.md` and `scripts/check-release-notes.sh` define the release-note gate shape for verification counts, regression deltas, interop primary versions, failed-requirement decisions, RFC assertions, and sign-off | Add populated release notes to each release with the recorded primary versions (`ODS-VER-010`, `ODS-VER-013`, `ODS-VER-014`, `ODS-VER-015`), method cadence/Test Plan mapping (`ODS-VER-011`), and full regression triage policy/baseline tracking (`ODS-VER-012`) |
