...
| Interface/CLI | `serve`, `check-config`, `--validate-config`, redacted `--dump-config`, config parsing, JSON/logfmt logging, `/livez`, JSON `/readyz`, `/healthz` readiness alias, `/metrics`, build-info gauge, query latency histogram, SIGTERM/SIGINT handling, and binary-level tests for config/usage exit codes 2, 64, and 78 | Complete SRS v0.7 Alpha alignment: full sysexits mapping for runtime/bind failures, canonical log fields, config warning catalogue, `ODS_<SECTION>_<KEY>` env naming, retained `--version`/`--help` output artifacts, optional `interface.notify`, per-zone status metrics, broader retained health/metrics timing evidence, gzip metrics, `/metrics` rate limiting, and release evidence for build-info label accuracy and latency histogram behavior |
