...
## SRS v0.9.1 New Acceptance Items

| v0.9 area | Current Evidence | Remaining Gap |
| --- | --- | --- |
| Zone provisioning and RFC 9432 catalog zones | `docs/catalog-zone-mvp-rfc9432.md`; `[[zones]]` / `[[catalog_zones]]` config; mandatory catalog `tsig_key`; `[transfer].require_tsig`; internal catalog consumption; `catalog_member_added` / `catalog_member_removed` logs; `oxidedns_catalog_member_info` metric; BIND live catalog interop over TSIG and XoT+TSIG; PowerDNS Authoritative plus PostgreSQL producer-catalog interop covering TSIG-only transfer, live member add/remove, and member-zone record update | Implement the `ODS-NFR-SEC-013` catalog member-zone cap (`max_member_zones` or equivalent) or revise that SRS requirement, then add broader release-level catalog evidence |
| DNAME synthesis overflow | Query path returns YXDOMAIN with the DNAME and without synthesized CNAME when DNAME substitution would overflow; Appendix A now tracks `ODS-FR-QRY-025` in the query-processing range | Broaden retained query-path release evidence for DNAME overflow cases if acceptance review requires more than the focused local coverage |
| DNAME multiplicity in transfers | AXFR validation rejects multiple DNAME records at the same owner with `MultipleDnameRecords`; Appendix A now tracks `ODS-FR-AXFR-026` in the AXFR range | Add retained fault-injection artifacts for the DNAME multiplicity rejection if release acceptance requires runtime evidence beyond parser tests |
...
| TSIG secret environment loading | `[[tsig_keys]]` supports inline `secret` and filesystem `secret_file`; secret files are validated for readability and rejected when world-readable; `--dump-config` redacts inline secrets and preserves file paths; scalar `ODS_*` overrides are documented for supported process/config toggles | Implement `ODS-NFR-SEC-008` TSIG secret loading from environment variables, including post-load environment clearing and zeroization evidence, or revise the SRS requirement if file-backed secrets are the intended operational boundary |
| Runtime timeout defaults from C.5 | TCP idle/read/write/connect timeouts are implemented, and health probes are covered by local response-timing evidence | Decide whether to implement `query.processing_timeout_ms` plus the related per-zone drop metric and `health.livez_timeout_ms`, or revise the SRS to rely on existing TCP/query resource bounds and external orchestrator probe timeouts |
| Reference idle CPU target | `scripts/capture-resource-evidence.sh` can retain a short zero-query idle CPU sample; large benchmark handoff scripts record resource samples for later review | Confirm or revise the formal `ODS-NFR-RES-006` 1% idle CPU target for the Reference Hardware/Profile run; this is release-acceptance evidence, not Engineering MVP evidence |
...
| Operator Docs | README, implementation plan, verification ledger, Appendix A range-level traceability matrix with generated all-requirement coverage index, example config, Operator Deployment Guide with informative SLO section, Test Plan, Security Policy, Architecture/release-governance document with module mapping, release evidence snapshot helper, and `scripts/check-operator-guide.py` guide-shape check | Replace or augment range-level Appendix A evidence rows with completed release-specific per-requirement dispositions where acceptance review needs finer granularity; add remaining Architecture Document content, actual signed-release artifacts, and completed release-specific verification notes before formal SRS MVP acceptance |

## Pending SRS C.5 Decision Overlay

SRS v0.9.1 Appendix C.5 is the canonical pending-decision list. All C.5 entries
