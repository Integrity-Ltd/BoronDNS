...
| Real-primary and current-version interop | BIND, NSD, Knot, PowerDNS/PostgreSQL, XoT, TSIG, NOTIFY, IXFR, catalog, DNSSEC, and DNS Cookie scripts exist and can retain primary versions/configs. | Run the selected release matrix on the final candidate, retain primary versions/configurations, record skipped/unsupported primary capabilities explicitly, and publish the tested matrix in release notes. |
| XoT release evidence | TLS 1.3-or-later client profile, ALPN, certificate, mTLS, TSIG-over-XoT, no-cleartext-fallback, and revocation-posture checks exist. | Retain current-version primary capability records, ClientHello/cipher/ALPN evidence where required, malformed/failure artifacts, and NSD XoT evidence if the selected NSD version exposes TLS-protected transfer configuration. |
...
| Reproducible build and signed artifacts | Reproducible-build handoff schemas, release installer/Docker packaging, checksum generation, and signing policy/runbooks exist. | Produce release artifacts for the final tag, perform the independent reproducible-build comparison, sign artifacts with the chosen mechanism, and attach manifest/checksum/signature verification records. |
