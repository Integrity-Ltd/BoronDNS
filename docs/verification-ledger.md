...
| --- | --- | --- | --- | --- | --- |
| Architectural invariants | Alpha | ODS-INV-001..ODS-INV-006 | Partial | docs/implementation-plan.md; scripts/check.sh; scripts/audit-invariants.sh | `scripts/audit-invariants.sh` now records repeatable static inspection evidence for secondary-only scope, memory-resident query path, atomic snapshot publication, no persistent operational writes, static configuration/control surface, and first-party safe Rust. `dns_update_opcode_gets_notimp_without_zone_mutation` covers DNS UPDATE rejection without zone mutation. Concurrent refresh/query atomicity stress and read-only root filesystem runtime evidence remain acceptance gaps. |
