...
- catalog member zones inherit the catalog entry's transfer primaries, TSIG key, NOTIFY source policy, transfer source binding, and transfer limits by default; catalog member transfers MAY be unsigned only when an explicit local per-catalog legacy member-transfer policy allows it and the member primary address is private; catalog per-member `primaries` properties MUST NOT override those inherited coordinates per ODS-NFR-SEC-011.
