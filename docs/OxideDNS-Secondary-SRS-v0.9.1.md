...
- the current Engineering MVP DNS Cookie secret of ODS-FR-COOKIE-004 (runtime-generated at process start, held in memory for the process lifetime, zeroed at termination per ODS-NFR-SEC-003; future configured Server Secret material remains startup configuration);
...
**ODS-FR-COOKIE-004.** The server cookie secret MUST be at least 16 octets of cryptographic secret material and MUST be suitable for the SipHash-2-4 Server Cookie construction of ODS-FR-COOKIE-003. Formal RFC 9018 release acceptance MUST provide configurable Server Secret material, shared-secret deployment support for anycast or load-balanced sets, and current-plus-previous secret verification during staged rollover. The current Engineering MVP DNS Cookie secret is process-local: it is generated from a cryptographically secure random source at process startup, is not configured, is not persisted to disk, and is not portable across restarts, anycast instances, or load-balanced replicas.
*Source.* RFC 9018 §4.4, §5; RFC 7873 §6, §7.1; ODS-INV-004.
*Note.* The process-local Engineering MVP behavior is acceptable for a single running instance and keeps secret material off disk, but it is not the complete formal RFC 9018 shared Server Cookie secret configurability posture. Restarts invalidate all previously issued server cookies; legitimate clients re-acquire a fresh server cookie on their next exchange, costing one round-trip per client.
*Verification.* Code review confirming secret generation at startup and absence of disk persistence for the current Engineering MVP; later release-acceptance tests for configured shared secrets, staged rollover, and multi-instance validation.
...
**ODS-FR-COOKIE-008.** DNS Cookies MUST be enabled by default with the "lenient" policy. The configuration MUST permit the operator to disable cookies entirely or to switch to "strict" policy per §6.2. The current Engineering MVP configuration permits an in-process cookie secret regeneration interval (default: once per process lifetime, i.e., the secret persists until process restart) for operators who want periodic local rotation within a long-running process. This interval is not a shared anycast rollover mechanism and does not satisfy the formal RFC 9018 shared Server Cookie secret configurability gap recorded in ODS-FR-COOKIE-004.
...
**ODS-NFR-OBS-009.** The metrics endpoint MUST expose a DNSSEC counter for responses where NSEC3 denial-of-existence proof records were omitted because the configured iteration cap was exceeded. The Engineering MVP implementation metric is `oxidedns_dnssec_nsec3_iterations_exceed_cap_total`. This counter intentionally has no `zone` label in the current profile: the diagnostic signal is global, and per-zone attribution is provided by retained query artifacts rather than high-cardinality runtime metrics. Current code increments this counter by observing emitted EDE INFO-CODE 27 in serialized responses, so `edns.extended_dns_errors = "off"` undercounts affected responses; independent counting of proof omissions is an explicit formal SRS acceptance gap, not a completed Engineering MVP claim.
*Source.* ODS-FR-DNSSEC-014; ODS-FR-EDNS-018.
*Verification.* Metric endpoint inspection after a query that triggers EDE INFO-CODE 27 in the Engineering MVP profile; formal SRS acceptance additionally requires evidence that over-cap proof omissions increment the counter when EDE is disabled. *Added in v0.9.*
