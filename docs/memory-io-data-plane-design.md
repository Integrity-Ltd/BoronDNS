...
completion ring sizes, batch size, TX wakeup interval, zero-copy policy, XDP
attach mode, and the project-built redirect object. Selecting `af_xdp` requires
`xdp.interface` and `xdp.redirect_object`, and ring sizes are validated as
non-zero powers of two before runtime setup reaches the lower-level AF_XDP
crate. The default backend remains `std`, and AF_XDP keeps its own packet
worker model rather than sharing the standard UDP runtime toggle.
