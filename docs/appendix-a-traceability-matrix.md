...
| ODS-FR-XOT-011 | MVP / Not Verified | No accepted evidence yet | Add log-inspection evidence for successful handshake/session establishment, failure logging, session termination, negotiated TLS version/cipher fields, byte counts, and absence of certificate/private-key/session-key material in logs. |
...
| ODS-FR-DNSSEC-013 | MVP / Not Verified | `docs/OxideDNS-Secondary-SRS-v0.1.md`; `docs/implementation-plan.md` | Produce retained static-analysis or code-review evidence that no code path generates DNSSEC records, manages DNSSEC key material, validates DNSSEC signatures, or participates in DNSSEC key rollover. |
...
| ODS-NFR-MAINT-001..ODS-NFR-MAINT-005 | MVP / Partial | `docs/implementation-plan.md`; `scripts/audit-safe-rust.sh`; `scripts/audit-maintainability.sh`; this matrix | Current line-count audit reports the first-party Rust source is over the ODS-NFR-MAINT-001 target; add architecture/release-note justification or refactor plan, requirement-to-module mapping, in-code requirement reference review, and reproducible-build artifact. |
