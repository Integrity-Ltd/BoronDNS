...
| Server module decomposition (server/lib.rs monolith) | §5.4, BDS-NFR-MAINT-002 | Decompose `server::health` and `server::transfer` from monolithic `server/lib.rs` | **Pending: non-normative maintainability candidate; module organisation per BDS-NFR-MAINT-002 to be tracked in Architecture Document** |
...
| External operator acceptance as MVP criterion | §7.4 | Confirm as MVP criterion | **Resolved (v0.9.1): required for the formal BDS-VER-008 SRS MVP release gate and release-notes sign-off, but explicitly outside the bounded Engineering MVP profile** |
...
| 10% memory growth threshold over 30 days (default) | §5.2, BDS-NFR-REL-003 | Confirm | **Resolved (v0.9.1): 10% remains the formal soak threshold and is the default in `scripts/capture-soak-handoff.sh`; actual 30-day soak execution remains BDS-VER-008 release acceptance, not Engineering MVP evidence** |
...
| 30-day / 90-day CVE response targets (defaults) | §5.3, BDS-NFR-SEC-007 | Confirm | **Resolved (v0.9.1): Security Policy records 30-day Critical/High and 90-day Medium/Low remediation targets, with release-specific exceptions recorded as evidence** |
