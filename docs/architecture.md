...
| `crates/borondns-server/src/zone_persistence.rs` | `BDS-FR-IXFR`, `BDS-FR-ZSM`, and last-good-zone restart behavior | Bounded checksummed full checkpoints plus a bounded incremental RRset journal, atomic persistence, strict restoration validation, and corrupt-cache rejection. The journal avoids rewriting a large checkpoint for each small IXFR. Checksums detect accidental corruption; filesystem permissions provide the trust boundary. |
