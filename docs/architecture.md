...
| Continuous CI posture | `.github/workflows/ci.yml` runs `./scripts/check.sh` on main-branch pushes, pull requests, manual dispatch, and a weekly schedule; `scripts/check-ci-workflow.py` keeps the workflow trigger and tool shape under local review. | `ODS-VER-011`, `ODS-NFR-SEC-006` |
