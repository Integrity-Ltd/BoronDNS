...
| Continuous verification posture | `scripts/check.sh` is the shared local, pull-request, main-branch, and release verification entry point. Hosted continuous CI runs for every pull request and main-branch candidate through `.github/workflows/continuous-verification.yml`; the tag-push/workflow-dispatch release workflow reruns the gate against exact artifact source. | `BDS-VER-011`, `BDS-NFR-SEC-006` |
