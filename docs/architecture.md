...
| `crates/borondns-server/src/zone_persistence.rs` | `BDS-FR-IXFR`, `BDS-FR-ZSM`, and last-good-zone restart behavior | Bounded authenticated zone-cache serialization, atomic persistence, strict restoration validation, and corrupt-cache rejection. |
...
| Continuous verification posture | `scripts/check.sh` is the shared local, SSH-host, and release verification entry point. While the repository is private, automatic hosted GitHub Actions verification is limited to `v*` tag pushes through `.github/workflows/release-installer.yml`; its explicit `workflow_dispatch` path is operator-invoked only. The release workflow runs the gate against the exact artifact source before packaging. | `BDS-VER-011`, `BDS-NFR-SEC-006` |
