...
| NSEC3 cap creates a DNSSEC authentication downgrade | Accepted and fixed. Affected negative responses now fail closed with SERVFAIL, AA=0, and no partial DNSSEC denial response; optional EDE INFO-CODE 27 remains diagnostic only. | `docs/BoronDNS-Secondary-SRS-v0.9.1.md`; `docs/dnssec-conformance-matrix.tsv`; `crates/borondns-core/src/dns_tests/any_negative_dnssec.rs`. |
