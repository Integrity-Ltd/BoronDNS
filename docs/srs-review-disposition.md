...
| Catalog zones | RFC 9432 sections 3, 4.1, 5, and 7; RFC 6761 and the IANA Special-Use Domain Names registry | Catalog zones remain in scope because OxideDNS implements catalog transfer/parsing/reconciliation/observability. Section 4.1 owns the member-zone PTR structure and says PTR RDATA is used so all valid domain names can be represented; RFC 9432 examples use `example.com.`, `example.net.`, and `example.org.`. The SRS now follows that source behavior: special-use or wildcard-looking names are not implicitly invalid, while RFC 9432 §5.2 name clashes are ignored and logged. |
