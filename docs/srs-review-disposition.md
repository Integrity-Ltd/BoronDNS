...
| DNS Cookies | Retained in Engineering MVP as an implemented UDP source-address confirmation mechanism. | Release evidence remains broader than Engineering MVP evidence; DNS Cookies are not described as TSIG-equivalent authentication. Current code uses a process-local startup secret, so RFC 9018 configured shared Server Secret support for anycast/load-balanced deployments remains an explicit acceptance gap rather than a reason to trim the implemented feature. |
...
| NSEC3 iteration cap | RFC 9276 sections 2.3, 3.1, 3.2, 6, and Appendix A | SRS treats proof omission above the cap as an availability/CPU-protection downgrade with optional diagnostic EDE, not normal authenticated denial. RFC 9276 recommends zero iterations for publishers; OxideDNS uses 100 as a project compatibility default informed by Appendix A measurements, not as an RFC compliance ceiling. Current metric evidence is narrower than the formal requirement because the implementation counts over-cap responses by observing emitted EDE INFO-CODE 27; EDE-off metric independence is tracked as an acceptance gap. |
| Catalog zones | RFC 9432 sections 3, 4.1, 5, and 7 | Catalog zones remain in scope because OxideDNS implements catalog transfer/parsing/reconciliation/observability. Section 4.1 owns the member-zone PTR structure that current code parses; the stricter `ODS-NFR-SEC-015` semantic member-name exclusions remain a tracked code-alignment gap. The SRS states observable behavior and keeps implementation shape in architecture docs. |
| DNS Cookies | RFC 7873 sections 2, 3, 5.2, 5.4, 6, and 7.1; RFC 9018 sections 4 and 5 | SRS treats DNS Cookies as limited UDP off-path spoofing resistance and source-address confirmation, not TSIG-equivalent client authentication. The `lenient` default is a project compatibility policy; `strict` BADCOOKIE enforcement is available for stronger anti-spoofing posture. RFC 9018 configured shared Server Secret support and staged rollover are tracked as formal acceptance gaps because current code generates process-local startup secrets. |
...
| DNS Cookie shared Server Secret deployment | The DNS Cookie server secret is generated randomly at process startup, held only in memory, and optionally rotated inside the process. It is not configured or shared across instances. | Engineering MVP keeps DNS Cookies because RFC 9018 version-1 cookie construction, validation, lenient/strict policy, BADCOOKIE responses, RRL exemption, logging, metrics, and `dig` interop evidence are implemented. Formal RFC 9018 anycast/load-balanced acceptance needs configured shared Server Secret support, current-plus-previous secret validation during staged rollover, and retained multi-instance evidence. |
| NSEC3-cap metric/EDE coupling | The query path marks over-cap NSEC3 proof omission in response metadata, but the server increments `oxidedns_dnssec_nsec3_iterations_exceed_cap_total` by inspecting emitted EDE INFO-CODE 27 in the serialized response. With `edns.extended_dns_errors = "off"`, affected responses are not counted. | Engineering MVP keeps bounded EDE and NSEC3 cap behavior because proof omission and EDE-on evidence are implemented. Formal ODS-NFR-OBS-009 acceptance needs metric counting decoupled from EDE emission and retained evidence for both `off` and `minimal` EDE modes. |
| Catalog member-name semantic exclusions | The catalog parser validates RFC 9432 schema version and structural member PTR data, rejects duplicate member zones, detects a member equal to the catalog apex as whole-candidate catalog rejection, and the server enforces the configured member-zone cap. It does not yet implement the full `ODS-NFR-SEC-015` per-offending-member semantic rejection profile for self-catalog diagnostics, subordinate-to-catalog members, root-zone members, reserved-zone members, or wildcard-label members. | Engineering MVP keeps catalog zones because configured catalog transfer, parsing, runtime add/remove, TSIG requirement, catalog hiding, member caps, logs, and `oxidedns_catalog_member_info` are implemented. Formal SRS acceptance needs the semantic member-name exclusions and retained tests recorded in the gap register before the current RFC 9432 catalog slice can be treated as complete. |
