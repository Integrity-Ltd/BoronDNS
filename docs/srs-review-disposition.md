...
| Panic isolation wording prescribed `catch_unwind` internals | Accepted and corrected. The SRS now requires the observable panic-isolation property and current supervised Tokio task `JoinError` handling, rather than a concrete unwinding mechanism not used by the runtime. | `docs/OxideDNS-Secondary-SRS-v0.9.1.md`; `crates/oxidedns-server/src/lib.rs`; `scripts/check-srs-hygiene.py`. |
