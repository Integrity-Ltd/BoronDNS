...
This Software Requirements Specification (SRS) defines the functional and non-functional requirements of BoronDNS-Secondary, a secondary-only authoritative DNS server written in Rust. It records the project's RFC compliance target as concrete, traceable, testable requirements suitable for implementation, review, and independent verification.
...
- **BDS** denotes the BoronDNS-Secondary requirement namespace.
- **CATEGORY** is one of:
...
References to RFCs follow the form *RFC NNNN §S.S* where a specific clause is cited. References to requirements within this document follow the form of the identifier itself (`BDS-CAT-AREA-NNN`). References to sections of this document follow the form *§N.N*. External planning references, if any appear in archived documents, are provenance only and do not define current requirements unless the requirement text restates the operative scope in this SRS.
...
| SRS | This document. |
| BDS | BoronDNS-Secondary requirement namespace. |
...
- **Operator Deployment Guide** — `docs/operator-deployment-guide.md`, the
  current operator-facing guide required by BDS-NFR-MAINT-009.
...
At a behavioural level, BoronDNS-Secondary performs the following functions. Section 4 decomposes these functions into numbered requirements; where a requirement intentionally groups a coherent operational case, its verification text identifies the observable sub-cases to test.
...
*Implications.* There is no DNS UPDATE (RFC 2136) handler: UPDATE messages are received on the DNS query interface (per BDS-FR-NOTIFY/QRY listening) but are rejected with RCODE NOTIMP per BDS-FR-CORE-005 and BDS-NEG-001; no UPDATE-message processing logic exists in the server. There is no zone-file editing interface, no administrative interface for modifying records, and no acceptance of out-of-band zone data injection. The complementary prohibitions are enumerated as negative requirements in §4.18.
...
*Statement.* Every query against a zone MUST be answered from a single, internally consistent version of that zone's data — a single SOA generation with all RRsets reflecting that generation's content. Refresh transitions between zone versions MUST be atomic from the perspective of any concurrent query handler. A multi-delta IXFR transfer MAY result in multiple distinct atomic transitions (one per applied difference sequence per BDS-FR-IXFR-008 and BDS-FR-IXFR-010), each transitioning the zone to a self-consistent SOA generation; but no query MAY observe a torn read within any single transition (i.e., no query MAY see records from two SOA generations co-existing in a response).
...
*Implications.* `server.zone_cache_directory` is mandatory for the supported profile. `server.allow_non_rfc5936_cold_start=true` explicitly selects an unsupported compatibility/test profile. Cache files are bounded regular files, mode 0600, checksummed, fully zone-validated on restore, written through a same-directory temporary file, fsynced, and atomically renamed. Query serving remains memory-only and performs no filesystem I/O.

The root filesystem and `/tmp` MAY remain read-only; the configured cache directory and standard streams require writes. When local administration is explicitly enabled, the configured operator socket directory also requires writes for socket creation, permissions, and removal. This transient socket lifecycle does not persist operator commands or additional zone data. Crash dumps generated by the kernel remain outside this invariant.
...
- for `[[catalog_zones]]` (per §4.20.2 and BDS-FR-PROV-002), the set of member zones derived from the catalog zone's contents, including additions and removals occurring during process lifetime as the catalog zone is updated by the primary.
...
*Statement.* **First-party** Rust code authored by this project MUST use Rust's safe subset for all code that processes data received from the network — including but not limited to DNS query parsing, EDNS option parsing, RR-type-specific decoders, zone transfer payload parsing, NOTIFY message handling, and TSIG verification input handling. Any use of `unsafe` blocks in first-party code MUST be accompanied by a comment in the source code stating the reason the block is necessary and the invariants on which its soundness depends (per BDS-NFR-MAINT-003).

Third-party dependencies (Rust crates depended on by the server per BDS-NFR-SEC-006) MAY contain `unsafe` blocks where they are required for performance, FFI to system libraries, or interfacing with cryptographic primitives in lower-level C; such dependencies are selected and reviewed under BDS-NFR-SEC-006. The intent of this invariant is the discipline applied to code authored by this project, not a wholesale prohibition of well-reviewed third-party libraries that contain `unsafe` internally.
...
- If an RRset with type CNAME is present at the QNAME, return that RRset (CNAME owner names by definition have no other types of co-located data per BDS-FR-RR-005, so this case is exclusive).
...
This subsection specifies the server's behaviour with respect to DNSSEC, restricted to the serve-only role. The server transfers DNSSEC records from the primary and serves them faithfully; it does not sign, does not generate denial-of-existence records, does not synthesize new denial-proof material, does not validate signatures, and does not manage key material. The primary is the sole source of all signed data. BoronDNS may select already-transferred NSEC or NSEC3 RRsets that prove a negative response, but it never creates new DNSSEC records to do so.

The governing standards within this scope are RFC 4033 (introduction), RFC 4034 (DNSSEC resource records), RFC 4035 (protocol modifications), RFC 5155 (NSEC3), RFC 6840 (clarifications), and RFC 6944 (algorithm requirements). The corresponding primary-side standards — DNSKEY generation, signing, NSEC/NSEC3 chain construction, RFC 5011 key rollover — are explicitly excluded per BDS-INV-001 and Appendix C.
...
The architectural invariants from §3 establish the foundational properties of the zone store: memory-resident query serving (BDS-INV-002), atomic refresh (BDS-INV-003), and validated last-good restart state (BDS-INV-004).
...
- **LOADING.** No valid persisted last-good snapshot exists and no transfer has completed successfully. The zone store holds no authoritative data for the zone. Queries against a LOADING zone MUST receive RCODE = 2 (SERVFAIL).
...
DNS Cookies complements the RFC 5452 measures of §4.5 (QID randomisation, source-port randomisation, strict response matching). The §4.5 measures resist off-path attackers blindly guessing transaction details; a valid Server Cookie additionally indicates that the client has previously completed an exchange with this server from the claimed source address and Client Cookie. BoronDNS uses that address-confirmation property for UDP spoofing resistance and RRL exemption policy, not as general client identity or TSIG-equivalent authorization.
...
**BDS-NFR-SEC-005.** The server MUST NOT listen on any TCP or UDP port beyond those required for configured DNS query service and the optional health, metrics, and read-only observability endpoints. Outbound XoT connections do not require listening sockets. The server MUST NOT open a network administrative or debugging port. The optional `server.operator_socket` is the sole local administration exception: a Unix socket bound after privilege drop, with mode 0600 and peer-UID checks permitting only root or the runtime user. It MUST be disabled when unconfigured, refuse pre-existing paths, and require trusted parent directories. Its commands MUST NOT edit records or reload configuration. The current management HTTP implementation uses TCP; it does not provide a metrics Unix socket.
...
- the environment variable names per BDS-IF-CONF-006;
...
- the explicit ICMP/firewall requirement of BDS-IF-NET-006;
- the security posture statements of BDS-FR-XOT-012 (no real-time revocation checking);
- the clock-synchronisation requirement of BDS-NFR-REL-007;
- the operational implications of BDS-FR-ZSM-013 (long-LOADING zones);
- the privilege-drop expectations of BDS-NFR-SEC-004;
...
*Operator format exception.* The existing opt-in `plain` runtime format is a
human-readable alternative, not evidence of the structured-format contract above.
It retains levels, messages, and diagnostic fields, without
formatter-generated ANSI styles or Rust module prefixes. JSON remains the default;
JSON and logfmt retain their structured fields. Bootstrap records remain JSON.
Plain runtime timestamps are enabled by default; `logging.plain_timestamps = false`
omits them when an external collector supplies its own timestamp.
...
- counter metrics MUST carry the `_total` suffix (e.g., `borondns_secondary_queries_total`);
- explicit unit suffixes MUST be used where applicable: `_seconds` for duration, `_bytes` for size, `_ratio` for unitless ratios;
- per-zone, per-response-category, per-RCODE, and per-source-prefix dimensions MUST be expressed via Prometheus labels (e.g., `borondns_secondary_queries_total{zone="example.com"}` and `borondns_secondary_query_responses_total{zone="example.com",rcode="NOERROR"}`), not via name multiplication;
...
- AXFR/IXFR ingestion size cap (BDS-FR-AXFR-024) below 100 MiB (likely operationally insufficient);
...
catalog_primaries = ["10.0.0.1:53", "10.0.0.2:53"]  # per BDS-NFR-SEC-012
catalog_tsig_key = "catalog-tsig-key."               # MANDATORY per BDS-NFR-SEC-010
member_primaries = ["10.0.0.10:53"]
member_tsig_key = "member-transfer-key."
member_transfer_extensions = false
serve_catalog_zone = false
max_member_zones = 10000                             # per BDS-NFR-SEC-013

[catalog_zones.member_transfer_policy]
unsigned_axfr = "deny"                       # default; "allow-legacy-private" is explicit private-address legacy policy

[secret_store]
path = "/etc/borondns-secondary/secrets"
```
...
- catalog member zones inherit the catalog entry's member transfer primaries, TSIG key, NOTIFY source policy, transfer source binding, and transfer limits by default;
- `catalog_*` and `member_*` split fields are accepted so catalog-zone acquisition and catalog-derived member-zone acquisition can use different primaries, TSIG key names, or transfer profiles;
- when `member_transfer_extensions = true`, only the extension records listed in BDS-FR-PROV-014 may override member transfer coordinates; ordinary RFC 9432 `primaries` properties MUST NOT override those inherited coordinates per BDS-NFR-SEC-011;
...
# (BDS-FR-CHAS-001). Empty or absent means REFUSED.
version = ""

# Response to `hostname.bind.` / `id.server.` CH/TXT queries
# (BDS-FR-CHAS-002). Empty or absent means fall back to [server].nsid
...
**BDS-IF-CONF-020.** The supported secondary profile MUST configure
`server.zone_cache_directory` as an absolute durable directory writable by the
runtime identity. Omitting it MUST fail validation unless
`server.allow_non_rfc5936_cold_start=true` explicitly selects the unsupported
cold-start compatibility/test profile. The equivalent environment overrides
are `BORONDNS_SERVER_ZONE_CACHE_DIRECTORY` and
`BORONDNS_SERVER_ALLOW_NON_RFC5936_COLD_START`. Cache-file size is bounded from
the validated transfer-ingest ceiling while allowing worst-case DNS name
decompression. *Source.* RFC 5936 §6; BDS-INV-004. *Verification.* Configuration
and environment-override tests plus restart/corruption/publication tests.

### Local operator interface (implementation extension)

The optional `server.operator_socket` enables Unix-only local administration,
disabled by default. `borondns zone --socket PATH show|dump|refresh|retransfer`
operates on live server state without reloading configuration. Socket permissions
and peer credentials restrict access to the runtime user and root; this does not
add a public HTTP mutation endpoint or DNS transfer-serving role.

An explicit retransfer uses authenticated full AXFR and may replace an equal
serial. Routine transfers retain their advancing-serial rule. Both paths reject
invalid SOAs, serial rollback/ambiguity, stale publication candidates, and failed
transfer validation before durable promotion. Failed attempts retain last-good
data. Refresh acknowledgement means queue acceptance, not successful transfer.

The implementation admits four clients and one streaming dump, with a five-second
request-read deadline and five-minute command deadline. Dump output belongs to
one active installed generation; it does not clone the whole zone. Limits,
permissions, output, and stale-socket handling are documented in
[Local zone commands](operator-commands.md).
...
The opt-in operator `plain` format follows the exception in BDS-NFR-OBS-001;
it does not change the JSON/logfmt contract. `logging.plain_timestamps` controls
only plain runtime timestamps (default `true`). Bootstrap records emitted before
runtime logging initialization remain JSON with timestamps regardless of the
selected runtime format or this setting.
...
| `category` | string | Coarse category for filtering (`query`, `transfer`, `notify`, `tsig`, `xot`, `rrl`, `cookie`, `chaos`, `control_plane`, `configuration_warning`, `secret_store`, `signal`, `startup`, `shutdown`) |
...
**BDS-IF-PROC-001.** The server MUST use the following exit code convention, consistent with the BSD `sysexits.h` (FreeBSD <sysexits.h>) and POSIX convention where applicable:

| Exit code | Symbolic name | Meaning |
|---|---|---|
| 0 | `EX_OK` | Successful termination (graceful shutdown completed per BDS-NFR-REL-001; `--dump-config`, `--validate-config`, `--version`, `--help` modes completed successfully). |
...
| 2 | `EX_CONFIG_INVALID` | Configuration validation failure (per BDS-IF-CONF-005 or BDS-IF-CONF-010): the configuration file was read but contained errors. |
...
| 73 | `EX_CANTCREAT` | Cannot create or bind a required output: listen socket failed to bind per BDS-IF-NET-004; secret file unreadable per BDS-IF-CONF-004. |
...
- a pointer to the Operator Deployment Guide per BDS-NFR-MAINT-009 for fuller documentation;
...
The requirement category for this section is **BDS-VER-NNN** for verification requirements; the AREA component is omitted, following the pattern of BDS-INV-NNN and BDS-NEG-NNN. The category is registered in §1.4.3 and Appendix D.5.1.
...
**Fuzz test.** Coverage-guided fuzzing using `cargo-fuzz`, AFL++, or equivalent, applied to wire-format parsers and any code path consuming untrusted input. Used for BDS-NFR-SEC-002 and as supporting evidence for parser-related functional requirements. Short-cadence fuzz compile/smoke checks (≤ 1 hour per parser) run through the active continuous gate where enabled; long-cadence runs (≥ 24 hours per parser) are gated to release per BDS-NFR-SEC-002.
...
- The SRS specifies *what* is to be verified, *by what method* (per the catalogue of §7.1), *at what cadence* (per the classification of BDS-VER-011), and *for what acceptance criterion* (per the milestone definitions of BDS-VER-007 and BDS-VER-008).
...
and to retained evidence per BDS-VER-001, BDS-VER-005, BDS-VER-006, and
BDS-VER-014.
...
- `docs/rfc-compliance-assertions.md` owns the current structured RFC compliance
  assertion register for BDS-VER-014.
...
Requirement identifiers remain immutable per BDS-§1.4.4. Deprecated or replaced
...
| 4.17 RRL | RRL | (no IETF RFC; BoronDNS project RRL policy) |
...
specified by §4.14 (`BDS-FR-RR-001`). It does not reproduce the §4.14 table or
...
- `docs/appendix-a-traceability-matrix.md`, where the `BDS-FR-RR-*` evidence
...
in §4.9, and TKEY is prohibited by `BDS-NEG-014`. Transfer/meta/query types
that are not valid zone content, including OPT, TKEY, TSIG, IXFR, AXFR, MAILB,
MAILA, and ANY, are rejected from zone transfers under `BDS-FR-URR-009`.

Reserved type values 0 and 65535 are rejected per `BDS-FR-URR-009` for zone
transfer content and per `BDS-FR-QRY-009` for query QTYPE handling. Private-use
...
§4.4 (`BDS-FR-URR-001` through `BDS-FR-URR-009`): they are accepted from
...
composition follows `BDS-FR-QRY-023` for known types and `BDS-FR-URR-006` for
...
confirmation rather than implicit endorsement.
...
### C.2.9 Persistent operational state other than last-good zones

*Description.* Writing refresh history, query statistics, configuration, or operational state other than the RFC 5936 last-good zone cache to disk.

*Rationale.* BDS-INV-004 narrowly permits and requires validated last-good zone persistence; all non-zone runtime state remains reconstructible.
...
*Description.* A network administrative or debugging listener.
...
set of operations (§2.3). The local same-UID/root Unix operator socket is a narrow
exception under BDS-NFR-SEC-005, not a network listener. General configuration
changes still require restart under BDS-INV-005.
...
*Note.* Flagged at §4.10 closing notes; tracked in the project decision
register referenced by C.5.
...
*Note.* Flagged at §4.17 closing notes; tracked in the project decision
register referenced by C.5.
...
*Status.* This item was excluded in v0.1 through v0.7 on the grounds that BDS-INV-005 (Static Configuration) precluded catalog-driven dynamic zone provisioning. The v0.8 introduction of §4.20 Zone Provisioning resolves the exclusion by isolating statically configured `[[catalog_zones]]` coordinates from the *derived member-zone set* (runtime-derived state, expressly admitted by the updated BDS-INV-005). Catalog zones are now supported under `[[catalog_zones]]` per BDS-FR-PROV-005 et seq., with the security narrowings of BDS-NFR-SEC-010 through BDS-NFR-SEC-015 (mandatory TSIG on the catalog, prohibition of `primaries` property honouring, bounded member count, validation of member-zone names, mandatory per-transfer timeout).
...
*Description.* Web-based administrative interfaces; configuration GUIs; configuration migration tools (e.g., a converter from BIND `named.conf` to this server's TOML schema); zone diff tools.

*Rationale for exclusion.* These broader tools remain outside the server scope. Local zone inspection, streaming dumps, and refresh/retransfer are implemented in the server binary through the bounded Unix operator interface documented in §6.2; they are not excluded by this item.
...
*Status.* Feature-gated experimental BoronDNS server profile; ordinary kernel
UDP sockets remain the supported default. When the `af-xdp` server feature and
backend are selected, the packet adapter MUST discard invalid source addresses
as required by RFC 1122, MUST generate IPv4 UDP checksums by default, and MUST
either generate non-repeating IPv4 Identification values for fragmentable
packets or emit atomic packets (`DF=1`) for which RFC 6864 permits an arbitrary
Identification value. IPv6 UDP checksums remain mandatory. Regression tests
...
**CD bit.** Checking Disabled bit (RFC 4035). Used primarily between security-aware resolvers and recursive name servers. For BoronDNS authoritative responses, RFC 4035 §3.1.6 says the bit is mostly irrelevant and that security-aware authoritative name servers SHOULD clear it; this server makes clearing CD mandatory by project policy (BDS-FR-DNSSEC-011).

**CHAOS class.** DNS resource class 3 (CH), historically allocated for Chaosnet and now commonly used by DNS tooling for diagnostic TXT probes such as `version.bind.` and `id.server.`. BoronDNS support is specified in §4.21.
...
**EDE.** Extended DNS Errors (RFC 8914). An EDNS option carrying fine-grained error condition information from the responder to the requestor. Implemented only for the bounded profile in BDS-FR-EDNS-018.
...
**Logfmt.** Structured log line format (key=value pairs). One of the two structured log formats per BDS-NFR-OBS-001, with JSON as the other. The optional `plain` operator format is not a structured format.
...
**NSID.** Name Server Identifier (RFC 5001). An EDNS option allowing a client to request and a server to return an opaque identifier of the responding server instance, used for diagnostic purposes in anycast and load-balanced deployments. Implemented in §4.11 (BDS-FR-EDNS-016 / BDS-FR-EDNS-017).
...
**OpenMetrics.** Standardised metrics exposition format related to Prometheus text exposition. BoronDNS does not currently emit OpenMetrics framing or the `application/openmetrics-text` media type; BDS-NFR-OBS-003 specifies Prometheus text exposition format 0.0.4 instead.
...
**SipHash.** A keyed pseudo-random function (Aumasson, Bernstein, 2012; cryptographic-quality though not a hash function in the SHA-family sense) used for Server Cookie MAC computation per RFC 9018. Cited at BDS-FR-COOKIE-003.
...
Area codes are short uppercase mnemonics (3–6 characters) registered in the table below. Each area code is unique across the SRS and is associated with one subsection. New area codes are allocated only by SRS revision; per the identifier-stability rule of §1.4.4, area codes are never reused for different concerns. Categories with omitted AREA components, such as `BDS-NEG-NNN`, are registered in D.5.1 rather than in the area-code tables below.
