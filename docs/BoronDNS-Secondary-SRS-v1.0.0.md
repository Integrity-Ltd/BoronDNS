...
The root filesystem and `/tmp` MAY remain read-only; only the configured cache directory and standard streams require writes. Crash dumps generated by the kernel remain outside this invariant.
