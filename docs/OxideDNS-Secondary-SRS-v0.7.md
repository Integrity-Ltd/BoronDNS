...
| Related documents | PID v0.1 (May 2026); Architecture Document (forthcoming); Test Plan (forthcoming); Operator Deployment Guide (forthcoming, per ODS-NFR-MAINT-009) |
...
**ODS-NFR-SEC-006.** Third-party Rust crates depended on by the server MUST be from well-maintained sources, subjected to security review at adoption time, and tracked against ongoing security advisories. The dependency set MUST be minimised consistent with functional requirements. Specific crate choices, with security justification, are recorded in the Architecture Document. Continuous monitoring MUST be implemented in CI: `cargo audit` (or equivalent advisory database) MUST run on every push to the main branch and on a scheduled basis at least weekly, with build failures on findings of severity High or Critical that are not yet mitigated.
*Source.* PID §7.1; standard supply-chain security practice.
*Verification.* Periodic `cargo audit` execution observed in CI logs; review of advisories at each release.
...
*Note.* The two-endpoint split replaces the v0.3 single-endpoint design with four states. The split is the dominant orchestration convention and avoids the ambiguity of "starting" vs "draining" both reporting non-ready while requiring different operator responses.
...
```
oxidedns_secondary_build_info{version="0.1.0",commit="abc123de",rust_version="1.81.0",build_timestamp="2026-05-24T12:00:00Z"} 1
```
...
- **DNS query interface** (`interface.dns`): the address or addresses on which the server receives and answers DNS queries (UDP and TCP, default port 53). This is the high-traffic, externally reachable interface. It MUST be possible to configure it independently of the other interfaces. By default, the DNS query interface also receives inbound NOTIFY messages from primaries (NOTIFY is delivered to UDP/53 or TCP/53 per RFC 1996), unless a separate NOTIFY interface is configured per ODS-IF-NET-008.
...
**ODS-IF-NET-007.** The configuration schema (§6.2) MUST provide an `[interfaces]` section with sub-keys `dns`, `mgmt`, `transfer`, and (optionally per ODS-IF-NET-008) `notify` — all optional, defaulting to wildcard / OS-selected as specified in ODS-IF-NET-005 and ODS-IF-NET-008. Each key accepts a list of (address, port) pairs. The server MUST validate that addresses assigned to `dns` and `mgmt` do not overlap unless the operator explicitly sets them equal (signalling intentional co-location), emitting a warning at startup when overlap is detected. Overlapping `transfer` with `dns` or `mgmt` is permitted without warning, as the transfer interface is outbound-only and does not bind a listening socket. Overlap of `notify` with `dns` on the same (address, port) is rejected as a configuration error per ODS-IF-CONF-005 (both would attempt to bind the same socket).
...
**ODS-IF-NET-008.** The configuration MAY specify an optional **NOTIFY interface** (`interface.notify`) on which the server opens additional UDP/53 and TCP/53 listening sockets dedicated to receiving NOTIFY messages from primaries. When configured, the server MUST accept NOTIFY messages on the NOTIFY interface's listening sockets in addition to those on the DNS query interface; ordinary DNS queries received on the NOTIFY interface's sockets are processed normally (the interface is not query-isolated, only NOTIFY-additive). When `interface.notify` is omitted from configuration, NOTIFY messages are received on the DNS query interface per the default behaviour of §4.8.

The rationale for separating NOTIFY traffic onto its own interface is security and operational: primaries are trusted, known, often on a management or backbone network; clients on the DNS query interface are untrusted internet-facing. Operators wanting to firewall NOTIFY traffic narrowly to primary IP addresses on a private network can configure `interface.notify` on that private network and apply tight firewall rules at the network layer.
*Source.* Operational requirement for security-isolated NOTIFY reception; resolution of the v0.4 audit finding about inbound NOTIFY interface assignment.
*Verification.* Configuration tests with `interface.notify` set to an address distinct from `interface.dns`; verify NOTIFY messages from primaries are accepted on the NOTIFY interface; verify DNS queries on the NOTIFY interface are processed normally; verify NOTIFY messages from the DNS query interface continue to be accepted as before when no `interface.notify` is configured.
...
```json
{"status":"alive","version":"0.1.0","uptime_seconds":12345}
```
...
```json
{"status":"ready","version":"0.1.0","zones_active":1234,"zones_loading":12,"zones_expired":0}
```

HTTP status 503 with the following body when **not-ready** (no zone yet ACTIVE):

```json
{"status":"not-ready","reason":"loading","version":"0.1.0","zones_active":0,"zones_loading":42,"zones_expired":0}
```

HTTP status 503 with the following body when **draining** (SIGTERM received, graceful shutdown per ODS-NFR-REL-001):

```json
{"status":"draining","version":"0.1.0","grace_period_remaining_seconds":15}
```

**`/healthz`** — provided for backward compatibility with the v0.1/v0.2/v0.3 single-endpoint convention. Behaviour and body content are identical to `/readyz`. Marked as deprecated; operators SHOULD migrate to the two-endpoint pattern. The deprecation timeline is governed by ODS-NFR-MAINT-006.
...
```
oxidedns-secondary 0.1.0
build commit: 1a2b3c4d
build timestamp: 2026-05-24T12:00:00Z
rustc: 1.81.0
```
...
**Static analysis.** Automated source-code analysis by tooling, runnable in CI without human intervention and producing build-blocking pass/fail results. Tools in scope include: `cargo clippy` (style and common-mistake linting), `cargo geiger` (enumeration of `unsafe` blocks per ODS-NFR-MAINT-003 and ODS-INV-006), `cargo audit` (dependency security advisory matching per ODS-NFR-SEC-006), `cargo-llvm-cov` (line coverage measurement per ODS-NFR-MAINT-007), and dependency-tree audits. Distinct from manual Inspection; runs continuously per ODS-VER-011.
...
*Verification.* Test Plan review at each release confirming method-cadence classification is documented; CI configuration review confirming the classification is enacted.
...
| Interface | v0.5 | §6 | `interface.xot` → `interface.transfer` rename; new `interface.notify`; CLI helper modes (`--dump-config`, `--validate-config`, `--version`, `--help`); canonical logging field names; new PROC area code (§6.6) with exit-code convention. |
...
| Separate inbound NOTIFY interface | §6.1, ODS-IF-NET-008 | Add optional `interface.notify` | **Resolved (v0.5): added; ODS-IF-NET-008** |
