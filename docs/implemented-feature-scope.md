...
| --- | --- | --- | --- | --- | --- |
| IXFR and AXFR fallback | The transfer client can build an IXFR query from the held SOA, parse valid IXFR responses, apply accepted deltas, and fall back to AXFR when IXFR is unavailable or unsuitable. | Primary-server behavior, dynamic UPDATE, UDP IXFR, or serving IXFR to downstream secondaries. | `BDS-FR-IXFR-001..019`; `BDS-FR-AXFR-001..026` | `crates/borondns-core/src/axfr.rs`; `crates/borondns-server/src/lib.rs`; `crates/borondns-server/src/transfer.rs` | `crates/borondns-server/src/tests/transfer_protocol.rs`; `scripts/interop-bind-ixfr-refresh.sh`; `scripts/interop-knot-ixfr-refresh-docker.sh`; `scripts/interop-ixfr-notimp-fallback.sh` |
