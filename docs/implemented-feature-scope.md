...
| Minimal health, readiness, metrics, and structured logs | Implemented `/livez`, `/readyz`, `/healthz`, `/metrics`, Prometheus exposition, bounded JSON/logfmt structured logging, and build/version fields. | `crates/oxidedns-server/src/lib.rs`; `crates/oxidedns-cli/src/main.rs`; `docs/health-metrics-interface.md` | `scripts/capture-health-metrics-evidence.sh`; `scripts/check-interface-compatibility.py`; `scripts/audit-log-fields.py`; `scripts/audit-log-lazy-formatting.py` |
