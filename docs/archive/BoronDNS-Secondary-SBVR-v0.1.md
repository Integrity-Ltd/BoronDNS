# BoronDNS-Secondary — SBVR Structured English Specification

## v0.1 (Draft 1) — Companion to SRS v0.1

**Date:** 23 May 2026
**Source document:** *BoronDNS-Secondary Software Requirements Specification*, v0.1
...
This document expresses the BoronDNS-Secondary requirements in SBVR Structured English, an OMG-standardised controlled natural language for vocabulary and rules. Because plain Markdown cannot render the four-colour SBVR font convention, this document adopts the following ASCII-renderable approximations, applied uniformly throughout:
...
| **Individual concept** (name) | bold italics | ***bold italics*** | ***BoronDNS-Secondary***, ***LOADING***, ***RFC 5936*** |
...
*authoritative server* — a *server* holding *zone data* for one or more *zones*. The ***BoronDNS-Secondary*** is an *authoritative server*.
...
*architectural invariant* — a property the ***BoronDNS-Secondary*** maintains at all times during operation; specified in §2.
...
*AXFR session* — the TCP-borne exchange constituting one full zone transfer between the ***BoronDNS-Secondary*** and a *primary*.
...
*configuration* — the operator-supplied parameters governing the ***BoronDNS-Secondary***, established at *process startup*.
...
*DNS client* — an entity sending *DNS queries* to the ***BoronDNS-Secondary***.
...
*IXFR session* — an exchange of *IXFR query* and *IXFR response* between the ***BoronDNS-Secondary*** and a *primary*.

*listening socket* — a UDP or TCP socket on which the ***BoronDNS-Secondary*** accepts inbound traffic.
...
*master file* — the presentation-format zone-data file defined in *RFC 1035 §5*; the ***BoronDNS-Secondary*** does not read *master files*.
...
*operator* — the human administrator responsible for the ***BoronDNS-Secondary***'s *configuration* and lifecycle.

*OPT pseudo-RR* — the EDNS0 record type 41 conveying EDNS metadata.

*orchestrator* — an automated process supervisor (container orchestrator, init system) that starts, stops, and probes the ***BoronDNS-Secondary***.
...
*primary* — the *authoritative server* holding the master copy of a *zone*, from which the ***BoronDNS-Secondary*** transfers.

*process* — an executing instance of the ***BoronDNS-Secondary*** binary.
...
*served zone* — a *zone* designated for service by the ***BoronDNS-Secondary*** through *configuration*.

*server* — the ***BoronDNS-Secondary*** instance; equivalent throughout this document to the named individual ***BoronDNS-Secondary***.

*signal* — a POSIX process signal; the ***BoronDNS-Secondary*** handles ***SIGTERM*** and ***SIGINT***.
...
***BoronDNS-Secondary*** — the named software product, equivalent to *server*/*authoritative server* in rule context.
...
**[R-IF-CONF-001]** **It is obligatory that** all operational *configuration* is supplied via a single TOML-format *configuration file*, path specified at startup via command-line argument (default `/etc/borondns-secondary/config.toml`).
...
This SBVR document is *derived from* and *normatively subordinate to* the *BoronDNS-Secondary Software Requirements Specification* v0.1. Where this document and the SRS disagree on substantive content, the SRS prevails; this document is then updated to match.

The SRS itself remains subordinate to the *BoronDNS-Secondary Project Initiation Document* (PID) v0.1 for matters of scope and stakeholder assignment.
