...
This Software Requirements Specification (SRS) defines the functional and non-functional requirements of BoronDNS-Secondary, a secondary-only authoritative DNS server written in Rust. It expands the RFC compliance target established in the Project Initiation Document (PID) into concrete, atomic, testable requirements suitable for implementation, review, and independent verification.
...
```
BDS-<CATEGORY>-<AREA>-<NNN>
```

where:

- **RDS** denotes BoronDNS-Secondary.
- **CATEGORY** is one of:
...
- `BDS-FR-AXFR-014` — fourteenth functional requirement in the AXFR area.
- `BDS-NFR-PERF-003` — third non-functional requirement concerning performance.
- `BDS-IF-NET-001` — first network interface requirement.
- `BDS-INV-002` — second architectural invariant.
- `BDS-NEG-007` — seventh prohibition requirement.
- `BDS-VER-001` — first verification requirement.
...
References to RFCs follow the form *RFC NNNN §S.S* where a specific clause is cited. References to requirements within this document follow the form of the identifier itself (`BDS-CAT-AREA-NNN`). References to PID sections follow the form *PID §N.N*. References to sections of this document follow the form *§N.N*.
...
| SRS | This document. |
| RDS | BoronDNS-Secondary. |

## 1.6 References

### 1.6.1 Project Documents

- **PID** — *BoronDNS-Secondary Project Initiation Document*, v0.1, May 2026.
- **Architecture Document** — *BoronDNS-Secondary Architecture Design* (to be produced in PID Phase 2; informed by this SRS).
- **Test Plan** — *BoronDNS-Secondary Test Plan* (to be produced alongside this SRS; verification procedures derive from §7).
...
At a behavioural level, BoronDNS-Secondary performs the following functions. Each is decomposed into atomic requirements in §4.
...
**BDS-INV-002 — Memory-Resident Zone Data**
...
**BDS-INV-003 — Atomic Zone Refresh**
...
**BDS-INV-004 — No Persistent Operational State**
...
**BDS-INV-006 — Memory Safety Discipline**
...
**BDS-FR-CORE-014.** In responses, the server MUST set the AA bit to 1 when the answer is authoritative for the queried name (a direct match, NODATA, NXDOMAIN, or wildcard synthesis within a served zone) and MUST set the AA bit to 0 for referral responses to delegated child zones.
...
**BDS-FR-CORE-018.** The server MUST respond with RCODE = 5 (REFUSED) to queries with QCLASS values other than IN or ANY when no zone of the requested class is served.
...
**BDS-FR-QRY-002.** The processing of a query MUST NOT alter the served zone data nor any operational state observable to other queries, with the sole exceptions of statistics counters (BDS-FR-QRY-023) and rate-limit accounting state (§4.17).
...
**BDS-FR-QRY-004.** In "full" any-response mode, for QTYPE = 255 (ANY) queries against a name with at least one RRset present, the server MUST return all RRsets present at the QNAME in the answer section, applying the standard lookup semantics of §4.1.
*Source.* RFC 1034 §3.7; RFC 1035 §3.2.5.
*Verification.* Lookup tests in "full" mode against names with multiple RRsets.

**BDS-FR-QRY-005.** In "minimal" any-response mode, for QTYPE = 255 (ANY) queries against a name with at least one RRset present, the server MUST return a single RRset selected from those present at the QNAME, per RFC 8482 §4.1. The selection algorithm MUST be deterministic for a given zone state.
*Source.* RFC 8482 §4.1.
...
**BDS-FR-QRY-012.** The server MUST terminate CNAME chain resolution at a configurable maximum chain length, with a default of 8 CNAME records. Exceeding this limit MUST cause the response to be delivered as constructed up to that point, and MUST be logged at warning level.
...
**BDS-FR-QRY-013.** The server MUST detect CNAME loops (a chain in which any target name has already been included in the answer section of the current response) and MUST terminate processing of the chain at the point of detection.
...
- exceeding the session timeout under BDS-FR-AXFR-021;
...
**BDS-FR-IXFR-001.** The server MAY initiate IXFR queries over UDP for efficiency. Where UDP is used, the server MUST retry the IXFR query over TCP if any of the following hold on the UDP response: the TC (truncated) bit is set; the response on parsing does not constitute a complete IXFR transfer in one of the three modes specified in BDS-FR-IXFR-004.
*Source.* RFC 1995 §2.
...
- exceeding the session timeout under BDS-FR-IXFR-016;
...
- glue records: BDS-FR-AXFR-013;
- occluded data handling: BDS-FR-AXFR-014;
- compression scope within messages: BDS-FR-AXFR-015;
...
**BDS-FR-TSIG-005.** TSIG keys MUST be configured at process startup per §6.2 and MUST be immutable for the lifetime of the process, in accordance with BDS-INV-005. Each key entry comprises a key name (a DNS name), an algorithm identifier, and a shared secret.
...
- MAC computed over the message (and the prior-MAC envelope context where applicable per BDS-FR-TSIG-011);
...
**BDS-FR-XOT-001.** The server MUST implement TLS 1.2 (RFC 5246) for XoT connections, and SHOULD implement TLS 1.3 (RFC 8446). Where both endpoints support TLS 1.3, it MUST be selected in preference to TLS 1.2 during TLS version negotiation.
*Source.* RFC 9103 §7.1.
...
EDNS options not enumerated in this subsection are recognised as unknown and handled per BDS-FR-EDNS-014. EDNS Client Subnet (RFC 7871), DNS Cookies (RFC 7873), and EDNS Expire (RFC 7314) are not in scope per PID Appendix A.
...
**BDS-FR-EDNS-009.** The DO bit in the response OPT RR's TTL field MUST be set in accordance with the DNSSEC response semantics specified in §4.13. The setting of DO in the response does not echo the query's DO setting verbatim; the §4.13 requirements govern.
...
The governing standards within this scope are RFC 4033 (introduction), RFC 4034 (DNSSEC resource records), RFC 4035 (protocol modifications), RFC 5155 (NSEC3), RFC 6840 (clarifications), and RFC 6944 (algorithm requirements). The corresponding primary-side standards — DNSKEY generation, signing, NSEC/NSEC3 chain construction, RFC 5011 key rollover — are explicitly excluded per PID §3.2 and BDS-INV-001.
...
**BDS-FR-DNSSEC-004.** In an NXDOMAIN response (per BDS-FR-CORE-023) where DO = 1 and the queried zone is signed, the server MUST include in the authority section the NSEC or NSEC3 records (and their corresponding RRSIGs) that authenticate the non-existence of the QNAME, in accordance with RFC 4035 §3.1.3 for NSEC or RFC 5155 §7.2.2 for NSEC3.
...
**BDS-FR-DNSSEC-009.** The DO bit in the response OPT RR's TTL field MUST be set to 1 when the response contains DNSSEC records included as augmentation under BDS-FR-DNSSEC-003 through BDS-FR-DNSSEC-007. In all other responses (including DO = 0 responses and responses to unsigned zones), the DO bit MUST be set to 0.
...
The architectural invariants from §3 establish the foundational properties of the zone store: memory-resident (BDS-INV-002), atomic refresh (BDS-INV-003), no persistent operational state (BDS-INV-004). This subsection specifies the behavioural consequences of those invariants in concrete, testable form.
...
**BDS-FR-ZONE-006.** Each designated zone is in one of three query-observable states at any time:
...
- **EXPIRED.** The time elapsed since the most recent successful transfer exceeds the zone's SOA EXPIRE field. Queries against an EXPIRED zone MUST receive RCODE = 2 (SERVFAIL) per BDS-FR-QRY-021.
...
**BDS-FR-ZSM-001.** At process startup, all designated zones enter the LOADING state per BDS-FR-ZONE-006. The state machine MUST initiate an AXFR transfer (not IXFR, as no prior zone data exists) for each LOADING zone, subject to the concurrent transfer session limit of BDS-FR-AXFR-022.
...
- Leave the zone in its prior state (ACTIVE if previously ACTIVE, LOADING if previously LOADING) with its prior data intact, subject to the EXPIRE evaluation of BDS-FR-ZSM-009;
- Schedule the next refresh attempt at (current time + RETRY interval), where RETRY is read from the zone's currently held SOA RDATA for an ACTIVE zone, or from the initial-load backoff (BDS-FR-ZSM-002) for a LOADING zone, subject to BDS-FR-ZSM-010 (jitter) and BDS-FR-ZSM-011 (minimum).
...
RRL is one of three complementary anti-amplification mechanisms in this server: the ANY-query minimisation policy of §4.2 (BDS-FR-QRY-005, BDS-FR-QRY-006) reduces the per-response amplification factor for ANY queries specifically; TCP fallback via TC=1 (§4.12) requires reflected clients to establish a TCP handshake to receive substantial data; and RRL itself bounds the rate at which the server contributes to any reflection campaign.
...
This subsection consolidates the prohibition requirements — those phrased as "MUST NOT" — that enforce the architectural invariants of §3 (chiefly BDS-INV-001) and the scope boundaries of §2. The prohibitions catalogued here are largely the negative restatement of constraints already established positively elsewhere in the SRS; their consolidation into a single audit-reviewable section is the purpose of this subsection.
...
*Enforces.* BDS-INV-001; PID §3.2 (out-of-scope).
...
*Enforces.* BDS-INV-001; PID §3.2 (the server does not act as a transfer source for downstream secondaries).
...
*Enforces.* BDS-INV-001 (the server does not act as a validator); PID §3.2.
...
**BDS-NEG-010.** The server MUST NOT write operational state — zone data, transfer history, query statistics, configuration data, or any data intended to survive process restart — to persistent storage. Log output to standard streams is not "operational state" within the scope of this prohibition.
...
*Enforces.* PID §3.2; static-configuration invariant for TSIG keys per BDS-FR-TSIG-005.
...
**BDS-NFR-PERF-001.** The server MUST sustain a query-handling throughput of at least 50,000 queries per second per CPU core under nominal workload — defined as a median DNS query mix against fully in-memory zones, UDP transport, no TSIG verification on queries.
...
**BDS-NFR-PERF-002.** Under nominal workload at no more than 50% of the throughput of BDS-NFR-PERF-001, the server MUST achieve a 99th-percentile query response latency below 1 millisecond for direct-hit lookups (queries answered without CNAME chain expansion).
*Source.* Operational requirement.
*Verification.* Latency-distribution inspection under controlled load.

**BDS-NFR-PERF-003.** Under workload at up to 90% of the throughput of BDS-NFR-PERF-001, the server MUST achieve a 99th-percentile query response latency below 10 milliseconds.
*Source.* Operational requirement.
*Verification.* Latency-distribution inspection at near-capacity load.

**BDS-NFR-PERF-004.** AXFR transfer ingestion MUST sustain at least 100,000 records per second on a contemporary Linux host with adequate network bandwidth to deliver the transfer.
...
**BDS-NFR-REL-002.** A process crash MUST NOT corrupt any persistent state on the host (per BDS-INV-004, no persistent state exists to corrupt). A subsequent process start MUST initialize cleanly from configuration, performing full zone acquisition per §4.16.
*Source.* BDS-INV-004.
*Verification.* Forced-kill tests followed by restart verification.

**BDS-NFR-REL-003.** Steady-state memory consumption MUST be bounded across extended operation (≥ 30 days continuous runtime). The server MUST NOT exhibit unbounded memory growth under sustained query load with zones of stable size and stable client population.
...
**BDS-NFR-REL-004.** Network errors on inbound or outbound connections — malformed packets, mid-transfer connection drops, exhausted file descriptors, kernel buffer exhaustion — MUST NOT cause process termination. Errors MUST be handled per the requirements of §4.6, §4.7, §4.8, §4.10, and §4.12; the process continues serving subsequent traffic.
*Source.* Operational requirement.
*Verification.* Fault-injection tests across the enumerated failure modes.

**BDS-NFR-REL-005.** The server MUST be deployable under rolling-restart patterns: replacement processes can be started while existing processes drain. SIGTERM-initiated drain completes within the grace period of BDS-NFR-REL-001 with no observable service interruption from clients exhibiting reasonable retry behaviour.
...
**BDS-NFR-SEC-003.** Cryptographic key material — TSIG shared secrets per BDS-FR-TSIG-006, XoT client TLS private keys per BDS-FR-XOT-007 — MUST NOT appear in any log entry at any verbosity level, MUST NOT appear in error messages, and MUST be zeroed in process memory at process termination.
*Source.* BDS-FR-TSIG-006; BDS-FR-XOT-007; standard cryptographic key handling.
...
**BDS-NFR-SEC-006.** Third-party Rust crates depended on by the server MUST be from well-maintained sources, subjected to security review at adoption time, and tracked against ongoing security advisories. The dependency set MUST be minimised consistent with functional requirements. Specific crate choices, with security justification, are recorded in the Architecture Document.
...
**BDS-NFR-MAINT-002.** The codebase MUST be organised into a small number of clearly-named, single-purpose modules, with each major functional area of §4 mappable to identifiable modules. The mapping is recorded in the Architecture Document.
...
**BDS-NFR-MAINT-004.** Implementation of functional requirements of §4 SHOULD include code-level comments referencing the requirement identifier and relevant RFC clause. The Appendix A traceability matrix is the canonical mapping; in-code references aid review.
...
**BDS-NFR-MAINT-005.** The build process MUST produce deterministic, reproducible binaries given a fixed source tree and pinned dependency set. The reproducibility approach (e.g., `cargo build --locked`, container build with pinned base image and tooling) is recorded in the Architecture Document.
...
**BDS-NFR-OBS-003.** The server MUST expose its in-memory counters — the query-handling counters of BDS-FR-QRY-024, the RRL counters of BDS-FR-RRL-012, the NOTIFY counters of §4.8, the TSIG counters per §4.9, the transfer-session counters per §4.6 and §4.7 — via a metrics endpoint per §6.4. The exposition format MUST be compatible with the Prometheus / OpenMetrics text format.
...
**BDS-NFR-OBS-005.** The metrics endpoint MUST expose per-zone status: zone state (LOADING / ACTIVE / EXPIRED per BDS-FR-ZONE-006), currently held SOA serial, timestamp of most recent successful refresh, timestamp of next scheduled refresh, count of refresh failures since the most recent success, and count of queries served for the zone since process start.
*Source.* Operational requirement.
...
**BDS-IF-CONF-001.** All operational configuration MUST be supplied via a single TOML-formatted configuration file, the path of which is specified to the process at startup via a command-line argument (default path: `/etc/borondns-secondary/config.toml`).
...
- network bind configuration per BDS-IF-NET-001 and BDS-IF-NET-002;
...
*Source.* BDS-FR-TSIG-006; BDS-FR-XOT-007; operational security for key material.
...
*Source.* Container-native logging convention; BDS-INV-004 (no persistent state, no filesystem writes beyond standard streams).
...
**BDS-IF-LOG-003.** Log level MUST be configurable via the configuration file per §6.2 and via environment variable (`BORONDNS_LOG_LEVEL` or equivalent) per BDS-IF-CONF-006. The default log level MUST be info per BDS-NFR-OBS-002.
...
**BDS-IF-HEALTH-002.** When activated, the endpoint MUST serve the following HTTP paths in response to GET requests:

- `/healthz` — returns the server's overall health state per BDS-NFR-OBS-004 as a plain-text body. HTTP status 200 when in the *ready* state; HTTP status 503 when in *starting*, *draining*, or *unhealthy* states.
- `/readyz` — returns the server's readiness to serve queries, distinct from `/healthz` in that it returns 200 only when at least one zone has reached ACTIVE state per BDS-FR-ZONE-006 (not merely when the process has started). HTTP status 200 when ready; 503 otherwise.
- `/metrics` — returns server metrics in the Prometheus / OpenMetrics text exposition format per BDS-NFR-OBS-003. HTTP status 200 with the metrics text body.
...
**BDS-IF-SIG-001.** The server MUST handle SIGTERM by initiating graceful shutdown in accordance with BDS-NFR-REL-001.
*Source.* BDS-NFR-REL-001; container orchestrator convention.
...
**BDS-IF-SIG-004.** The server MUST NOT install handlers for SIGUSR1, SIGUSR2, SIGQUIT, or any other signal not enumerated in this subsection. Such signals MUST follow operating-system default behaviour (typically process termination with core dump for SIGQUIT, termination for SIGUSR1 and SIGUSR2).
*Source.* Minimal signal-handling surface; principle of least operational interface.
*Verification.* Code review confirming the signal-handler registrations exactly match the enumeration of BDS-IF-SIG-001 through BDS-IF-SIG-003.
...
A new requirement category is introduced in this section: **BDS-VER-NNN** for verification requirements. The AREA component is omitted from these identifiers, following the pattern of BDS-INV-NNN and BDS-NEG-NNN. The category is added to the §1.4.3 enumeration in a future SRS revision; flagged in closing notes.
...
**Fuzz test.** Coverage-guided fuzzing using `cargo-fuzz` or equivalent, applied to wire-format parsers and any code path consuming untrusted input. Used for BDS-NFR-SEC-002 and as supporting evidence for parser-related functional requirements.
...
**BDS-VER-001.** Verification of each requirement in §3 through §6 MUST be performed using the method or combination of methods specified in that requirement's *Verification* field. Each *Verification* field MUST map to one or more methods enumerated in this subsection.
*Source.* SRS internal consistency.
*Verification.* Self-referential; covered by the SRS review process.

**BDS-VER-002.** Verification evidence — test outputs, benchmark results, code-review records, fuzz-test summaries, interop test logs — MUST be captured by the project's continuous integration system and retained for each release.
...
**BDS-VER-006.** Where an RFC referenced by PID Appendix A contains normative clauses that fall outside this server's scope — for example, primary-side requirements within an RFC that also covers secondary-side behaviour, or resolver-side requirements within an RFC primarily about authoritative service — the traceability matrix MUST mark those clauses as out-of-scope with a brief rationale referencing BDS-INV-001 (secondary-only) or PID §3.2. The RFC is then assessed for compliance limited to the in-scope clauses, and the compliance claim is documented accordingly (for example, "Compliant with RFC X, secondary-side clauses only; primary-side clauses out of scope per BDS-INV-001").
...
- All BDS-INV requirements (§3);
- Functional requirements: §4.1 (CORE) in full; §4.2 (QRY) excluding RFC 8482 minimal-ANY (BDS-FR-QRY-003 through -007 deferred); §4.3 (NRESP) in full; §4.4 (URR) in full; §4.5 (SPOOF) in full; §4.6 (AXFR) in full; §4.8 (NOTIFY) in full; §4.11 (EDNS) in full; §4.12 (TCP) in full; §4.14 (RR) restricted to RFC 1035 types plus AAAA; §4.15 (ZONE) in full; §4.16 (ZSM) in full;
...
**BDS-VER-008 — MVP Milestone.** The MVP milestone is achieved when the following are demonstrably satisfied:
...
- All BDS-NFR-PERF performance targets met under benchmarking;
- A 30-day soak test per BDS-NFR-REL-003 completed without anomaly;
...
**BDS-VER-009.** The traceability matrix in Appendix A MUST record, for each requirement in §3 through §6, the verification status: **Not Verified**, **Verified** (with date and reference to the evidence), or **Deferred** (with target milestone). The matrix is the canonical record of verification progress.
...
- to track verification status per requirement, supporting the milestone acceptance criteria of BDS-VER-007 and BDS-VER-008.
...
Requirement identifiers in this Appendix are immutable per BDS-§1.4.4. A requirement marked **Deprecated** in §3 through §6 remains in the Appendix tables with a pointer to its replacement; rows are never removed.
...
- **Partial (secondary-side).** The RFC's secondary-server-side clauses are in scope and mapped; primary-side, resolver-side, or validator-side clauses are out of scope per BDS-INV-001 or PID §3.2, and are catalogued in A.5.
...
Each requirement carries a verification status per BDS-VER-009:
...
- **Deferred** — verification is deferred to a specific milestone (typically MVP per BDS-VER-008).
...
- **Fine-grained.** RFC clause → SRS requirement identifier (for example, RFC 5936 §2.2.1 → BDS-FR-AXFR-005, BDS-FR-AXFR-007). Required for partial-scope RFCs and for clause-level audit.
...
*Out-of-scope clauses.* §5 (resolver-side) — BDS-INV-001; §6.1 (master files) — BDS-NEG-006; primary-role aspects of §4.3.5 — BDS-INV-001.
...
*Out-of-scope clauses.* §5 (master file format) — BDS-NEG-006; §7 (resolver implementation) — BDS-INV-001; §4.3 (zone transfer original specification) — superseded by RFC 5936 for AXFR and RFC 1995 for IXFR; primary-role aspects throughout — BDS-INV-001.
...
*Out-of-scope clauses.* §6.2 (zone publication, primary-side) — BDS-INV-001.
...
*Out-of-scope clauses.* AXFR server-side requirements throughout — BDS-NEG-005.
...
*Out-of-scope clauses.* IXFR server-side requirements — BDS-NEG-005.
...
*Out-of-scope clauses.* NOTIFY origination (§3.x originator semantics) — BDS-NEG-004.
...
*Key clauses.* §6 (port, ALPN) → XOT-003, XOT-004; §6.5 (connection persistence) → XOT-009; §7.1 (TLS versions) → XOT-001; §7.4 (ALPN) → XOT-004; §9.1 (Strict Profile, authentication) → XOT-005, XOT-006; §9.2 (Opportunistic Profile) → BDS-NEG-016 (prohibited); §9.3 (combined with TSIG) → XOT-008; §9.4 (mTLS) → XOT-007.
*Out-of-scope clauses.* §6.4 (NOTIFY over TLS, receiver side) — BDS-NEG-017; XoT server-side requirements throughout — BDS-NEG-005 implicitly via secondary-only role.
...
*Out-of-scope clauses.* §7 (resolver caching) — BDS-INV-001.
...
*Out-of-scope clauses.* §5 (master file representation) — BDS-NEG-006.
...
*Out-of-scope clauses.* DANE validation semantics — BDS-INV-001 (server is not a validator).
...
*Out-of-scope clauses.* Signing aspects (primary-side) — BDS-NEG-002.
...
*Out-of-scope clauses.* §4 (resolver-side validation) — BDS-INV-001.
...
*Out-of-scope clauses.* §7.1 (chain generation, primary-side) — BDS-NEG-002.
...
*Out-of-scope clauses.* Validator-side clarifications — BDS-INV-001.
...
|---|---|---|---|
| §2.1.1 | TCP transport mandate | BDS-FR-AXFR-001 | Draft |
| §2.1.2 | Query construction | BDS-FR-AXFR-002 | Draft |
| §2.2 | Multi-message response reassembly | BDS-FR-AXFR-004 | Draft |
| §2.2 | Terminal SOA detection | BDS-FR-AXFR-008, BDS-FR-AXFR-009 | Draft |
| §2.2.1 | QID and OPCODE validation | BDS-FR-AXFR-005 | Draft |
| §2.2.1 | Header flag bit non-significance | BDS-FR-AXFR-006 | Draft |
| §2.2.1 | First record SOA validation | BDS-FR-AXFR-007 | Draft |
| §2.2.1 | Class consistency | BDS-FR-AXFR-011 | Draft |
| §2.2.4 | Glue records | BDS-FR-AXFR-013 | Draft |
| §2.2.4 | Occluded data handling | BDS-FR-AXFR-014 | Draft |
| §2.2.5 | TSIG signing of multi-message | BDS-FR-AXFR-017, BDS-FR-AXFR-018 | Draft |
| §3.1 | Error handling | BDS-FR-AXFR-019, BDS-FR-AXFR-020 | Draft |
| §3.1 | No middle SOA | BDS-FR-AXFR-010 | Draft |
| §3.4 | Compression scope within message | BDS-FR-AXFR-015 | Draft |
| §4.1 | TCP connection management | BDS-FR-AXFR-003 | Draft |
| §5 | Error RCODE handling | BDS-FR-AXFR-020 | Draft |

### A.4.2 RFC 8945 — fine-grained mapping (TSIG)

| RFC Clause | Topic | Implementing Requirement(s) | Status |
|---|---|---|---|
| §3 | Key configuration | BDS-FR-TSIG-005 | Draft |
| §4 | TSIG RR wire format | BDS-FR-TSIG-007, BDS-FR-TSIG-008 | Draft |
| §4.2 | Original ID handling | BDS-FR-TSIG-008(d) | Draft |
| §4.3 | Error codes | BDS-FR-TSIG-013 | Draft |
| §5.2 | TSIG on requests/responses | BDS-FR-TSIG-008, BDS-FR-TSIG-012 | Draft |
| §5.2.1 | UDP truncation of signed responses | BDS-FR-TSIG-016 | Draft |
| §5.2.2 | TSIG error response semantics | BDS-FR-TSIG-013 | Draft |
| §5.2.2.1 | MAC truncation rules | BDS-FR-TSIG-014 | Draft |
| §5.3 | Signing algorithm | BDS-FR-TSIG-012 | Draft |
| §5.3.1 | Multi-message TSIG envelope | BDS-FR-TSIG-010, BDS-FR-TSIG-011 | Draft |
| §5.4 | Verification algorithm | BDS-FR-TSIG-008 | Draft |
| §5.4.1 | BADTIME / fudge handling | BDS-FR-TSIG-008(c) | Draft |
| §6 | Algorithm requirements | BDS-FR-TSIG-001, BDS-FR-TSIG-002, BDS-FR-TSIG-003 | Draft |
| §6 | HMAC-MD5 prohibition | BDS-FR-TSIG-004, BDS-NEG-013 | Draft |
| §10.5 | Truncation discouragement | BDS-FR-TSIG-015 | Draft |

## A.5 Out-of-Scope Clauses Register

This section catalogues, by RFC, those clauses that fall outside this server's scope, with reference to the architectural invariant or PID scope clause that excludes them. The register supports accurate partial-compliance assertions per BDS-VER-006.

**RFC 1034.** Resolver-side aspects (§5 in part) — BDS-INV-001. Master file format (§6.1) — BDS-NEG-006. Primary-role aspects of zone management (§4.3.5, various) — BDS-INV-001.

**RFC 1035.** Master file format (§5) — BDS-NEG-006. Resolver implementation (§7) — BDS-INV-001. Original zone transfer specification (§4.3) — superseded by RFC 5936 / RFC 1995.

**RFC 2181.** Primary-side zone publication concerns (§6.2 in part) — BDS-INV-001.

**RFC 2308.** Resolver caching semantics (§7) — BDS-INV-001.

**RFC 4035.** Resolver-side validation (§4) — BDS-INV-001. Signing semantics (cross-cutting with RFC 4034) — BDS-NEG-002.

**RFC 5155.** Chain generation (§7.1) — BDS-NEG-002.

**RFC 5936.** AXFR server-side requirements — BDS-NEG-005 (no outbound AXFR serving).

**RFC 1995.** IXFR server-side requirements — BDS-NEG-005.

**RFC 1996.** NOTIFY origination (originator semantics) — BDS-NEG-004.

**RFC 6698 (TLSA).** DANE validation semantics — BDS-INV-001 (server is not a validator).

**RFC 9103.** NOTIFY-over-TLS reception (§6.4) — BDS-NEG-017. Opportunistic Privacy Profile (§9.2) — BDS-NEG-016.
...
Verification status is tracked per requirement per BDS-VER-009. The expected mechanism for production maintenance of status is a structured table (spreadsheet, database, or version-controlled CSV) alongside the SRS source, rather than inline in the SRS document. The following columns are required:
...
|---|---|---|---|---|---|---|
| BDS-INV-001 | Inspection | Not Verified | — | — | Alpha | Foundational; verify at first build |
| BDS-FR-CORE-001 | Conformance | Not Verified | — | — | Alpha | — |
| BDS-FR-AXFR-001 | Inspection | Not Verified | — | — | Alpha | — |
| BDS-FR-IXFR-001 | Conformance | Not Verified | — | — | MVP | Per VER-007 deferred |
| BDS-FR-DNSSEC-001 | Conformance | Not Verified | — | — | MVP | Per VER-007 deferred |
| BDS-FR-XOT-001 | Interop | Not Verified | — | — | MVP | Per VER-007 deferred |
| ... | ... | ... | ... | ... | ... | ... |
...
This appendix is the implementer's reference for the resource record types listed in the catalogue of §4.14 (BDS-FR-RR-001). For each known type it provides the RDATA structure summary, the compression policy, owner-name conventions where they exist, and notes on parsing subtleties or cross-references to dependent SRS subsections. The catalogue table reproduced in B.2 below is identical in content to the table in §4.14; per-type expansion follows.

The §4.14 table is the *normative* source — it is part of the normative requirement BDS-FR-RR-001. This appendix is implementation-supporting documentation. Where the two are silently in conflict, §4.14 prevails; the Appendix should be updated to match.
...
*Validation.* RDLENGTH MUST be exactly 4 octets per BDS-FR-RR-007.
...
*Cross-references.* BDS-FR-RR-003 requires at least one NS at zone apex. NS RRsets in the authority or answer sections trigger glue inclusion per BDS-FR-QRY-017.
...
*Cross-references.* CNAME exclusivity per BDS-FR-RR-005 (no coexistence with other RRsets at the same owner name except DNSSEC RRs). CNAME chain handling per BDS-FR-QRY-010 through BDS-FR-QRY-013.
...
*Owner-name convention.* MUST be the zone apex per BDS-FR-RR-002. Exactly one SOA per zone.
*Notes.*
- SERIAL is compared using RFC 1982 arithmetic per BDS-FR-RR-004.
...
*Compression.* Not applicable.
*Notes.* HINFO is largely deprecated for operational use due to information-disclosure concerns. The HINFO synthesis pattern of RFC 8482 §4.2 for ANY-query minimisation is explicitly prohibited by BDS-NEG-015; this server's minimal-ANY policy returns a real RRset per BDS-FR-QRY-005.
...
*Cross-references.* Triggers additional-section inclusion of A/AAAA for EXCHANGE per BDS-FR-QRY-018.
...
*Compression.* Not applicable.
*Validation.* RDLENGTH MUST be exactly 16 octets per BDS-FR-RR-007.
...
*Cross-references.* Triggers additional-section inclusion of A/AAAA for TARGET per BDS-FR-QRY-018.
...
- REPLACEMENT is meaningful for further DNS resolution only when FLAGS indicate continuation (per RFC 3404); the server includes A/AAAA glue for REPLACEMENT per BDS-FR-QRY-018 where REPLACEMENT names a record in a served zone.
...
*Notes.*
- Triggers CNAME synthesis per BDS-FR-QRY-014, BDS-FR-QRY-015.
- DNAME cannot coexist with CNAME at the same owner name per BDS-FR-RR-006.
...
*Cross-references.* Used in DNSSEC referral response composition per BDS-FR-DNSSEC-007.
...
*Cross-references.*
- The TYPE COVERED field identifies which RRset this RRSIG signs; required by §4.13 response composition (per BDS-FR-DNSSEC-001).
- The server does not generate RRSIG records (BDS-NEG-002).
...
*Cross-references.* Used in authenticated negative responses per BDS-FR-DNSSEC-004, BDS-FR-DNSSEC-005, BDS-FR-DNSSEC-006.
...
*Compression.* Not applicable.
*Cross-references.* Per BDS-NEG-002, this server does not generate DNSKEY records or maintain DNSSEC key material.
...
*Cross-references.* Used in authenticated negative responses per BDS-FR-DNSSEC-004, BDS-FR-DNSSEC-005, BDS-FR-DNSSEC-006. Per BDS-NEG-002, this server does not generate NSEC3 records.
...
*Notes.* This server serves TLSA records as data; DANE validation semantics are out of scope per BDS-INV-001 (the server is not a validator).
...
*Cross-references.* Triggers additional-section inclusion of A/AAAA for TARGETNAME per BDS-FR-QRY-019.
...
*RFC.* RFC 2930.
*Handling.* Prohibited per BDS-NEG-014.
*Disposition.* Queries with QTYPE = 249 receive RCODE = 1 (FORMERR) per BDS-FR-QRY-009. No TKEY processing code path exists in this server.

## B.4 Out-of-Catalogue Types

The following RR types are not in the type-aware catalogue of §4.14 and are handled under the unknown-type semantics of §4.4 (BDS-FR-URR-001 through BDS-FR-URR-009). They are accepted from primaries, stored bit-for-bit, served on direct query, and propagated faithfully without type-aware interpretation.
...
Reserved type values 0 and 65535 are rejected per BDS-FR-URR-009 (zone transfers containing such records are aborted) and per BDS-FR-QRY-009 (queries using such values as QTYPE receive FORMERR). The IANA Private Use range (65280–65534 as of the current registry) is accepted and handled under §4.4.
...
The catalogue table of B.2 records the policy per type. When emitting responses, the server follows the policy per BDS-FR-QRY-023 (general compression policy) and BDS-FR-URR-006 (no compression for unknown types).
...
- **Foundational exclusions.** Items whose inclusion would violate an architectural invariant of §3 (typically BDS-INV-001, the secondary-only invariant). These cannot be brought into scope without redefining the project's identity.
...
*Enforcement.* BDS-INV-001; BDS-NEG-002 (no DNSSEC signing); BDS-NEG-003 (no non-transfer modification); BDS-NEG-004 (no NOTIFY origination); BDS-NEG-006 (no master file reading).
...
*Enforcement.* BDS-INV-001; BDS-NEG-001.
...
*Enforcement.* BDS-INV-001; BDS-NEG-002.
...
*Rationale.* Recursive resolution is the resolver-role function; this server is exclusively authoritative. Queries for names outside served zones receive REFUSED per BDS-FR-CORE-019.

*Enforcement.* BDS-INV-001; BDS-NEG-007.
...
*Enforcement.* BDS-INV-001; BDS-NEG-008.
...
*Enforcement.* BDS-INV-001; BDS-NEG-009; BDS-FR-DNSSEC-010 (AD bit always 0).
...
*Enforcement.* BDS-INV-001; BDS-NEG-006.
...
*Enforcement.* BDS-INV-001; BDS-NEG-005.
...
*Rationale.* BDS-INV-004 establishes that the server holds no persistent operational state. Every restart performs full zone acquisition from primaries; orchestrator configuration is the source of truth for all non-zone state.

*Enforcement.* BDS-INV-004; BDS-NEG-010.
...
*Rationale.* BDS-INV-005 establishes static configuration. Configuration changes are applied only by process restart. This eliminates an entire category of reload-consistency bugs and aligns with container-native deployment patterns.
...
*Rationale for exclusion.* Implementation simplicity. The current version applies RRL globally per BDS-FR-RRL-009.

*Enforcement.* BDS-FR-RRL-009.
...
*Enforcement.* Not implemented; per BDS-INV-001 the secondary serves what the primary delivers.
...
*Rationale for exclusion.* Per BDS-INV-005, configuration is static; catalog zones imply dynamic zone-set changes outside the operator's static configuration.

*Enforcement.* BDS-INV-005 prevents implementation under current invariants.

*Note.* A future version relaxing BDS-INV-005 to permit catalog-driven zone configuration would constitute a significant architectural change requiring SRS revision.
...
*Enforcement.* BDS-FR-TSIG-004; BDS-NEG-013.
...
*Rationale for exclusion.* Per BDS-INV-005, TSIG keys are configured statically. Dynamic key establishment is structurally incompatible with the static-configuration invariant.

*Enforcement.* BDS-INV-005; BDS-NEG-014.
...
| 9432 | DNS Catalog Zones | Current-scope exclusion (incompatible with BDS-INV-005) | C.3.9 |
...
**AD bit.** Authentic Data bit (RFC 4035 §3). Used by validating resolvers to indicate that response data has been verified by DNSSEC. This server sets AD = 0 unconditionally (BDS-FR-DNSSEC-010), as it does not perform validation.

**ALPN.** Application-Layer Protocol Negotiation (RFC 7301). Used in TLS to negotiate the application protocol carried over the TLS connection; this server uses the ALPN identifier `dot` for XoT connections (BDS-FR-XOT-004).
...
**Architectural invariant.** A property the system must hold at all times during operation, specified in §3. Invariants constrain the space of possible behaviours and are distinct from behaviours themselves. *See also.* BDS-INV-NNN in D.5.
...
**CD bit.** Checking Disabled bit (RFC 4035). Used in queries from validating resolvers. This server sets CD = 0 unconditionally in responses (BDS-FR-DNSSEC-011).
...
**DNAME.** Delegation Name record (type 39, RFC 6672). Redirects a subtree of the namespace to another subtree; the server synthesises CNAMEs in response (BDS-FR-QRY-014).
...
**EXPIRE.** SOA RDATA field (RFC 1035 §3.3.13). The interval after which a secondary that has not successfully refreshed the zone must cease serving it. *See also.* BDS-FR-ZONE-006 (EXPIRED state), BDS-FR-QRY-021.
...
**Graceful shutdown.** Process termination that completes in-flight work before exit, per BDS-NFR-REL-001.
...
**Jitter.** Random variation added to scheduled intervals to avoid synchronised events across many actors; specified for the zone state machine in BDS-FR-ZSM-010.

**Label.** A single component of a DNS name (RFC 1035 §3.1). Maximum 63 octets per label; maximum 255 octets per name.

**LOADING.** Zone state (BDS-FR-ZONE-006). No successful zone transfer has yet completed for the zone since process startup; queries against a LOADING zone receive SERVFAIL.

**Logfmt.** Structured log line format (key=value pairs). One of the two supported log formats per BDS-NFR-OBS-001, with JSON as the other.
...
**MD5.** Message Digest 5 hash function (RFC 1321). Prohibited for TSIG use per BDS-NEG-013.
...
**mTLS.** Mutual TLS. TLS with client-certificate authentication of the connection initiator. Supported optionally for XoT per BDS-FR-XOT-007.
...
**NODATA.** A response where the queried name exists in the zone but no RRset of the queried QTYPE exists at that name. RCODE = NOERROR with empty answer section and SOA in authority. *See also.* BDS-FR-CORE-022.
...
**NXDOMAIN.** Non-existent domain. RCODE = 3, returned when the queried name does not exist in the zone (BDS-FR-CORE-023). *See also.* RFC 8020 for the "no descendants either" cut semantic.

**OpenMetrics.** Standardised metrics exposition format compatible with Prometheus; used for the metrics endpoint per BDS-NFR-OBS-003.
...
**RA bit.** Recursion Available bit (RFC 1035 §4.1.1). Set in responses by recursive servers. This server sets RA = 0 unconditionally (BDS-FR-CORE-012) as it does not perform recursion.
...
**RD bit.** Recursion Desired bit (RFC 1035 §4.1.1). Echoed from query to response per BDS-FR-CORE-011 but otherwise ignored by this server.
...
**Recursion.** Resolution of a query by issuing further queries on the client's behalf. Not performed by this server (BDS-NEG-007).

**Referral.** A response that delegates resolution to another zone. RCODE = NOERROR, empty answer section, NS records of the child zone in authority section, optional glue in additional section. *See also.* BDS-FR-CORE-025.
...
**SERVFAIL.** Server Failure response (RCODE = 2). Returned for internal errors or expired zones (BDS-FR-QRY-021, BDS-FR-ZONE-006).
...
**Slip.** RRL configuration parameter (BDS-FR-RRL-005). Controls the ratio of rate-limited responses emitted as truncated (TC=1) versus dropped.
...
**TKEY.** Transaction Key record (type 249, RFC 2930). Not implemented (BDS-NEG-014).
...
**TOML.** Tom's Obvious Minimal Language. The configuration file format selected per BDS-IF-CONF-001.
...
**Wildcard.** An owner name beginning with the `*` label (RFC 4592). The server synthesises responses for non-existent names matching the wildcard per BDS-FR-CORE-024.
...
Per §1.4.3, requirement identifiers follow the form `BDS-<CATEGORY>-<AREA>-<NNN>` (with AREA omitted for some categories). The current categories are:
...
#### Functional area codes (BDS-FR-`AREA`-NNN)
...
#### Non-functional area codes (BDS-NFR-`AREA`-NNN)
...
#### Interface area codes (BDS-IF-`AREA`-NNN)
