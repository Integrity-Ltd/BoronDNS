# OxideGun Software Requirements Specification
...
This SRS defines the target behaviour for the `oxide-gun` crate:
...
`kxdpgun` is an independent reference tool. OxideGun may be compared with it for throughput and operational behaviour, but OxideGun MUST NOT link to it, execute it, parse its output, reuse its files as a runtime contract, or require it to be installed.
...
The target OxideGun is a single-purpose lab tool:
...
- The AF_XDP backend is used when OxideGun must place controlled source addresses on the wire.
...
**OXG-INV-006.** OxideGun MUST NOT leave persistent host network changes after normal or graceful termination. XDP programs, AF_XDP sockets, UMEM, and temporary maps must be cleaned up.

**OXG-INV-007.** OxideGun MUST NOT implement DNS resolver logic, DNS caching, DNSSEC validation, or persistent per-client state beyond one configured test run.

**OXG-INV-008.** OxideGun MUST NOT depend on `kxdpgun` at build time or runtime.

## 4. Functional Requirements

### 4.1 Packet Generation (PGN)

**OXG-FR-PGN-001.** OxideGun MUST generate RFC 1035 DNS query messages with a valid header and exactly one question unless a future requirement explicitly allows another shape.
...
**OXG-FR-PGN-003.** OxideGun MUST support at least `A`, `AAAA`, `NS`, `SOA`, `MX`, `TXT`, `PTR`, `SRV`, `CNAME`, `ANY`, and RFC 3597-style `TYPEnnnn`.

*Verification.* CLI/config tests and parser-based packet tests.

**OXG-FR-PGN-004.** OxideGun MUST support EDNS0 OPT records. EDNS0 SHOULD be enabled by default with UDP payload size 1232.

*Verification.* Packet tests confirm OPT type 41 and payload-size encoding.

**OXG-FR-PGN-005.** OxideGun MUST support configuring the DNSSEC OK bit in the EDNS0 flags field.
...
**OXG-FR-QRY-001.** OxideGun MUST keep support for a single configured query for smoke tests and simple runs.

**OXG-FR-QRY-002.** OxideGun MUST support query pools loaded from a simple text file of `qname QTYPE` rows.

**OXG-FR-QRY-003.** OxideGun MUST support generated query pools from a name template and count.

**OXG-FR-QRY-004.** OxideGun MUST support sequential and seeded-random query selection.
...
**OXG-FR-SRC-001.** OxideGun MUST support fixed source IP and source port configuration.

**OXG-FR-SRC-002.** OxideGun MUST support source IP strategies: fixed, round-robin list, random CIDR, random selected prefixes, and sequential range.

**OXG-FR-SRC-003.** OxideGun MUST support IPv4 and IPv6 for fixed, random CIDR, and sequential strategies.

**OXG-FR-SRC-004.** OxideGun MUST support source port ranges with sequential and seeded-random strategies.

**OXG-FR-SRC-005.** Source selection MUST be O(1), allocation-free after setup, and deterministic from seed/config.

**OXG-FR-SRC-006.** In portable UDP mode, OxideGun MUST clearly document OS limitations: it can bind only addresses available to the host. Full arbitrary source address control is an XDP-mode capability.
...
**OXG-FR-SEND-001.** When built with `xdp` and invoked with the XDP backend, OxideGun MUST send through AF_XDP, not through UDP socket send calls.
...
**OXG-FR-RATE-001.** OxideGun MUST support max packet count and max duration; the first reached limit wins.

**OXG-FR-RATE-002.** OxideGun MUST support unlimited backend rate with `target_qps = 0`.

**OXG-FR-RATE-003.** OxideGun MUST support constant target QPS with achieved-rate reporting.
...
**OXG-FR-STAT-001.** OxideGun MUST count TX packets/bytes/errors, RX packets/bytes, DNS response classes, truncated responses, unmatched responses, unanswered queries, and drop-mode kernel drops where available.
...
**OXG-FR-LOG-001.** OxideGun MUST emit valid one-record-per-line JSON for interval and summary records.
...
**OXG-FR-LIFE-001.** OxideGun MUST handle SIGINT and SIGTERM with graceful shutdown where practical.
...
**OXG-IF-CLI-001.** OxideGun MUST provide one binary, `oxide-gun`.
...
**OXG-NEG-001.** OxideGun MUST NOT implement resolver recursion.

**OXG-NEG-002.** OxideGun MUST NOT implement DNSSEC validation.

**OXG-NEG-003.** OxideGun MUST NOT support TCP, DoT, DoH, or DoQ before MVP.

**OXG-NEG-004.** OxideGun MUST NOT log per-query events in production builds.

**OXG-NEG-005.** OxideGun MUST NOT modify persistent `sysctl`, route, firewall, or interface configuration.

**OXG-NEG-006.** OxideGun MUST NOT claim arbitrary source-address control for portable UDP mode.
...
cargo clippy -p oxide-gun -- -D warnings
```
...
RUSTFLAGS="-Zsanitizer=address" \
  cargo +nightly test -Zbuild-std --target x86_64-unknown-linux-gnu -p oxide-gun
```
...
**OXG-VER-005.** High-rate claims require retained evidence: command line, effective config, git commit, kernel version, NIC and driver, queue/preflight state, copy/zero-copy status, CPU pinning when used, OxideGun JSONL and summary, external packet counters or captures, and explicit pass/fail thresholds for any claimed TX-rate floor, interface-counter corroboration ratio, and NIC TX error/drop ceiling.
