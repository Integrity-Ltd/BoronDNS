...
- `scripts/engineering-mvp-evidence.sh` captures the narrow Engineering MVP gate: repository checks, parser fuzz compile, invariant audit, read-only runtime audit, performance smoke, TCP truncation retry evidence, and BIND AXFR, TSIG AXFR, and NOTIFY refresh interop logs under `target/evidence/engineering-mvp/<timestamp>/`.
