...
- **Engineering MVP**: the first deployable and reviewable secondary DNS server
  ...
  smoke/runtime evidence, checked traceability, and the implemented post-Alpha
  feature slices retained by `docs/implemented-feature-scope.md`.
...
Engineering MVP must not require completed long-running evidence. Handoff and
runbook artifacts for long fuzz campaigns, Reference Hardware/Profile
benchmarks, 30-day soak, production-depth logging profiles, external operator
acceptance, independent reproducible-build comparison, and signed release
artifacts may remain in the repository, but they are not Engineering MVP
...
work may require additional retained evidence without narrowing the Engineering
MVP feature boundary unless the SRS or code changes.
