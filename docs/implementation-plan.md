...
- response OPT TTL handling is a known implementation-alignment gap after the SRS RFC 6840 cleanup: the response DO bit currently follows the older augmentation-derived behaviour, but the corrected requirement is to copy the query DO bit whenever the query contains OPT.
- BADVERS responses preserve only the extended RCODE bits apart from the same pending response-DO copy fix.
...
- `scripts/interop-bind-packet-torture-docker.sh` generates valid BIND zones with broad RR coverage, long labels, wildcard and negative cases, private/future RR types, and RFC 7553 URI RDATA; runs BIND, a freshly packaged OxideDNS image, and a dumpcap client in Docker; captures the UDP query exchange; and compares BIND/OxideDNS packet content after normalizing BIND-only positive-answer apex NS/glue extras while retaining strict checks for answer RRsets and required NS/MX/SRV/SVCB/HTTPS target additionals. It can retain BIND, OxideDNS, client, metrics, summary, diff, pcapng, and generated fixture artifacts with `OXIDEDNS_BIND_PACKET_TORTURE_ARTIFACT_DIR`.
...
- `scripts/interop-edns-behavior.sh` starts a fake AXFR primary with a large RRset and verifies retained running-service EDNS behavior for valid OPT parsing, malformed option FORMERR, duplicate OPT FORMERR, OPT outside the additional section FORMERR, BADVERS response encoding, advertised-payload floor, exact floor, below/exact/above configured maximum UDP ceiling truncation, response OPT presence and field values, legacy DO-clearing behaviour now identified as an SRS/RFC 6840 implementation gap, UDP keepalive ignore, TCP keepalive timeout advertisement, configured padding alignment, unknown-option ignore, non-EDNS 512-octet truncation without OPT, configured NSID with empty and non-empty request data, metrics, and `edns-traceability.tsv` with `OXIDEDNS_EDNS_BEHAVIOR_ARTIFACT_DIR`.
- `scripts/interop-chaos-queries.sh` starts OxideDNS against a fake primary and uses a built-in Python DNS client to verify CHAOS CH/TXT behavior over UDP and TCP: REFUSED defaults, configured `version.server.`, configured `hostname.bind.`, printable NSID fallback for `id.server.`, unsupported CHAOS names, non-TXT CHAOS types including AXFR, IN-class orthogonality, and `oxidedns_chaos_queries_total` metrics. It can retain client, config, metrics, log, summary, and `chaos-query-traceability.tsv` artifacts with `OXIDEDNS_CHAOS_QUERIES_ARTIFACT_DIR`.
...
- `scripts/benchmark-large-catalog-zones.sh` creates an opt-in RFC 9432 catalog-zone performance harness for local optimization work: generated mixed large/small member zones, BIND TSIG-authenticated AXFR source, OxideDNS CPU affinity, randomized UDP/TCP query load, 8 GiB default RSS target with 16 GiB override support, phase timing for generation/build/BIND readiness/OxideDNS startup-to-ready/warmup/measured serving/final capture, Prometheus snapshots before warmup and after measured serving, `/proc` resource samples, optional `perf stat`, and optional flamegraph generation when `perf record` plus Inferno tools are available. `scripts/check.sh` checks only its shell syntax; the benchmark itself remains opt-in and outside Engineering MVP evidence execution.
- `scripts/audit-invariants.sh` records repeatable static inspection evidence for the SRS v0.9.1 architectural invariants: secondary-only scope, memory-resident query path, atomic zone snapshot publication, no persistent operational writes, static configuration/control surface, first-party safe-Rust discipline, authoritative-only response composition, single-process operation, and static composition with no runtime code loading.
...
- `scripts/package-docker-image.sh` builds the Alpine-based Docker image from the same x86_64 musl static binary used by the installer, saves it as a release-attached `*-docker-image.tar.xz`, and emits a SHA256 sidecar plus image manifest/inspect artifacts; `scripts/test-docker-image.sh` loads that compressed archive and smoke-tests version output, `/livez`, `/metrics`, non-root UID, read-only root filesystem, dropped capabilities, and `no-new-privileges`.
...
- `scripts/audit-spoof-evidence.py` records repeatable static evidence for SRS v0.9.1 `ODS-FR-SPOOF-001..007`, including CSPRNG QID selection, kernel ephemeral UDP binding, concurrent SOA poll source-port uniqueness, connected UDP response source filtering, QID and question-section validation tests for SOA/AXFR/IXFR, and SOA poll warning-log evidence.
- `scripts/audit-log-fields.py` records repeatable static evidence for SRS v0.9.1 `ODS-IF-LOG-005` by rejecting legacy peer/failure-cause aliases and category values outside the canonical structured-log category set.
- `scripts/audit-log-lazy-formatting.py` records repeatable static evidence for SRS v0.9.1 `ODS-IF-LOG-008` by checking every first-party `debug!`/`trace!` emission site for eager `format!`, `format_args!`, `String::from`, `.to_string()`, or `.to_owned()` allocation patterns inside the lazy tracing macro arguments.
...
- `scripts/check-functional-requirement-references.py` enforces SRS v0.9.1 `ODS-NFR-MAINT-004` by parsing section 4 functional requirement IDs and requiring every ID to appear in a Rust source comment at a principal implementation owner.
...
- `crates/oxidedns-core/src/dns.rs` NSID tests cover configured NSID EDNS option responses, default-empty suppression, and non-empty query OPTION-DATA treated as a request per SRS v0.9.1 `ODS-FR-EDNS-016..017`; `scripts/interop-dnssec-serve.sh` now exercises configured NSID responses over the running UDP server for empty and non-empty NSID request data. `non_edns_udp_response_over_512_octets_is_truncated_without_opt` covers the `ODS-FR-EDNS-015` no-OPT 512-octet ceiling path.
- `scripts/engineering-mvp-evidence.sh` captures the narrow Engineering MVP evidence profile under `target/evidence/engineering-mvp/<timestamp>/`: security-policy checks, CLI/log/signal/health/malformed-query runtime evidence, portability/resource/coverage/unsafe-dependency/interface-compatibility evidence, unused-code audit output, and functional-requirement reference checks. Every captured command is guarded by `OXIDEDNS_ENGINEERING_MVP_COMMAND_TIMEOUT_SECONDS` (default 300 seconds). Broader interop, performance-smoke, read-only runtime, invariant/log/spoof audits, fuzz compile checks, and release/operations evidence commands are recorded as deferred in the snapshot and are not executed by this profile.
...
- `docs/test-plan.md` records the ODS-VER-011 Continuous/Periodic/Gate cadence mapping and the ODS-VER-012 regression policy; `scripts/check-test-plan.sh` keeps that structure in the continuous check. `scripts/check-shell-scripts.sh` applies `shfmt -w` and `shellcheck` across the shell-script corpus. `scripts/check.sh` is the current local continuous verification entry point. Hosted CI is intentionally deferred while the repository remains private to avoid spending CI minutes on heavyweight evidence tooling before a public-release gate exists. `scripts/check-perf-regression.py` compares retained smoke metrics with a rolling history when provided. Full weekly Reference Hardware/Profile benchmark baselines remain an SRS acceptance gap; long-running benchmark handoff artifacts are later release/operations material, not Engineering MVP evidence.
- `scripts/capture-info-verbosity-handoff.sh` records later release/operations setup for profiling `info` verbosity under production-representative traffic: runbook, report template, log-volume/structured-field/metrics TSV schemas, requirement traceability map, release-note snippet, environment values, and operator sign-off scaffold. It is not part of the Engineering MVP evidence profile and does not claim production-depth profiling has been executed.
...
- `scripts/capture-benchmark-handoff.sh` records later release/operations setup for the Reference Hardware/Profile benchmarks: runbook, report template, metric/resource TSV schemas, baseline-history template, requirement traceability map, release-note snippet, environment values, and operator sign-off scaffold. It is not part of the Engineering MVP evidence profile and does not claim that production benchmarks have been executed.
- `scripts/capture-soak-handoff.sh` records later release/operations setup for the ODS-NFR-REL-003 30-day soak: report template, RSS/file-descriptor/metrics/event TSV schemas, requirement traceability map, weekly summary template, environment values, and operator sign-off scaffold. It is not part of the Engineering MVP evidence profile and does not claim the long-running soak has been executed.
- `scripts/capture-reproducible-build-handoff.sh` records later release/operations setup for the ODS-NFR-MAINT-005 independent reproducible-build comparison: fixed `OXIDEDNS_BUILD_*` and `SOURCE_DATE_EPOCH` inputs, lockfile/toolchain hashes, `cargo metadata --locked`, runbook, artifact-manifest and comparison TSV schemas, requirement traceability map, release-note snippet, and release-engineer sign-off scaffold. It is not part of the Engineering MVP evidence profile and does not claim two independent bit-identical builds have completed.
- `scripts/capture-release-handoff.sh` records later release-governance setup: release attachment map, role ownership TSV, scheduled CI/manual-run plan, signing runbook, release-notes fill plan, external-operator acceptance scaffold, and release-readiness checklist. It is not part of the Engineering MVP evidence profile and does not claim release acceptance has been completed.
- `scripts/audit-safe-rust.sh` verifies the workspace `unsafe_code = "forbid"` lint, rejects unregistered `#![allow(unsafe_code)]` attributes, and scans first-party Rust source for unsafe construct candidates, allowing only current adapter paths from `docs/unsafe-boundaries.tsv`; `scripts/check-unsafe-boundaries.py` keeps the current POSIX FFI source-shape checks and deferred XDP/eBPF, AF_XDP, io_uring, packed-store, and response-cache safety gates tied to that registry.
- `scripts/audit-maintainability.sh` records the first-party production Rust source line count excluding `#[cfg(test)]` code, enforces the current 14-module map against `docs/architecture.md`, and reports the current `ODS-NFR-MAINT-001` line-count target status for release review.
...
- `scripts/audit-dnssec-passive.sh` records repeatable static evidence for SRS v0.9.1 `ODS-FR-DNSSEC-013`, and is included in `scripts/check.sh` plus release evidence snapshots.
- Runtime startup validates the SRS v0.9.1 file-descriptor rlimit formula for `ODS-NFR-RES-004`: `2 * (max_tcp_connections + max_concurrent_transfers + 100)`, and exits with an OS-startup error if the current soft `RLIMIT_NOFILE` is too low.
- `crates/oxidedns-server/build.rs` embeds build commit, Rust compiler version, and build timestamp labels for `oxidedns_secondary_build_info`; `[metrics].latency_histogram_buckets` configures the SRS v0.9.1 query latency histogram bucket boundaries with validation for positive finite strictly increasing seconds; `crates/oxidedns-server/src/lib.rs` metrics tests cover build-info exposition and latency histogram bucket/count/sum output using configured buckets.
