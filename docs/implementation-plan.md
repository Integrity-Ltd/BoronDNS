...
- prohibited pseudo/meta/reserved RR types from SRS v0.7 `ODS-FR-URR-009` are rejected when encountered as zone-transfer content, leaving the zone not-ready and recording failed AXFR metrics rather than publishing a partial zone;
...
- `docs/rrl-release-thresholds.md` records the current SRS v0.7 RRL release-review baseline and explicitly keeps the Appendix C.5 `Slip = 2` confirmation pending.
...
- `scripts/interop-bind-axfr.sh` starts a local BIND 9 primary with the `alpha.test.` fixture, validates BIND SOA and AXFR service with `dig`, starts OxideDNS against that primary, waits for `/readyz`, verifies UDP, TCP, CNAME-chain, and metrics behavior from the transferred zone, and can retain primary SOA/AXFR output, served-answer output, readiness, metrics, logs, configs, primary-version, and `axfr-traceability.tsv` artifacts with `OXIDEDNS_BIND_AXFR_ARTIFACT_DIR`.
- `scripts/interop-bind-tsig-axfr.sh` starts a BIND 9 primary whose AXFR is restricted to HMAC-SHA256 TSIG, proves unsigned AXFR is rejected and signed AXFR succeeds with `dig`, starts OxideDNS with the matching TSIG key, verifies readiness and served data after the signed transfer, checks OxideDNS logs do not contain the shared secret, and can retain redacted configs, BIND/OxideDNS logs, `dig` output, metrics, readiness, primary-version, and summary artifacts with `OXIDEDNS_BIND_TSIG_AXFR_ARTIFACT_DIR`.
...
- `scripts/interop-bind-ixfr-refresh.sh` starts a BIND 9 primary with IXFR journals enabled, observes OxideDNS issuing an IXFR refresh through a transfer proxy, confirms BIND returns a true incremental IXFR response, verifies OxideDNS publishes the newer serial and data with IXFR success metrics, and can retain redacted config, initial/updated zone, primary-version, proxy-log, readiness, query-output, metrics, and summary artifacts with `OXIDEDNS_BIND_IXFR_ARTIFACT_DIR`.
- `scripts/interop-nsd-axfr-docker.sh` starts NSD inside an Alpine Docker container with the `alpha.test.` fixture, validates NSD SOA and AXFR service with `dig`, starts OxideDNS against that primary, waits for `/readyz`, verifies UDP, TCP, CNAME-chain, and metrics behavior from the transferred zone, and can retain primary SOA/AXFR output, served-answer output, readiness, metrics, logs, configs, primary-version, and `axfr-traceability.tsv` artifacts with `OXIDEDNS_NSD_AXFR_ARTIFACT_DIR`.
- `scripts/interop-nsd-tsig-axfr-docker.sh` starts NSD inside an Alpine Docker container with AXFR restricted to HMAC-SHA256 TSIG, proves unsigned AXFR is rejected and signed AXFR succeeds with `dig`, starts OxideDNS with the matching TSIG key, verifies readiness and served data after the signed transfer, checks OxideDNS logs do not contain the shared secret, and can retain redacted configs, NSD/OxideDNS logs, `dig` output, metrics, readiness, primary-version, and summary artifacts with `OXIDEDNS_NSD_TSIG_AXFR_ARTIFACT_DIR`.
...
- `scripts/interop-knot-axfr-docker.sh` starts Knot DNS inside an Alpine Docker container with the `alpha.test.` fixture, validates Knot SOA and AXFR service with `dig`, starts OxideDNS against that primary, waits for `/readyz`, verifies UDP, TCP, CNAME-chain, and metrics behavior from the transferred zone, and can retain primary SOA/AXFR output, served-answer output, readiness, metrics, logs, configs, primary-version, and `axfr-traceability.tsv` artifacts with `OXIDEDNS_KNOT_AXFR_ARTIFACT_DIR`.
- `scripts/interop-knot-tsig-axfr-docker.sh` starts Knot DNS with AXFR restricted to HMAC-SHA256 TSIG, proves unsigned AXFR is rejected and signed AXFR succeeds with `dig`, starts OxideDNS with the matching TSIG key, verifies readiness and served data after the signed transfer, checks OxideDNS logs do not contain the shared secret, and can retain redacted configs, Knot/OxideDNS logs, `dig` output, metrics, readiness, primary-version, and summary artifacts with `OXIDEDNS_KNOT_TSIG_AXFR_ARTIFACT_DIR`.
- `scripts/interop-bind-notify-refresh.sh`, `scripts/interop-nsd-notify-refresh-docker.sh`, and `scripts/interop-knot-notify-refresh-docker.sh` exercise BIND, NSD, and Knot DNS real-primary NOTIFY refresh paths, assert OxideDNS accepted-NOTIFY logs, capture metrics and summary artifacts, and emit per-primary traceability TSVs for real-primary NOTIFY reception, response, refresh signalling, accepted logging, and ZSM-triggered refresh.
- `scripts/interop-knot-ixfr-refresh-docker.sh` starts Knot DNS with deterministic IXFR journal settings, verifies Knot exposes a true incremental IXFR after zone reload, routes OxideDNS transfer traffic through a classifier proxy, triggers OxideDNS refresh with NOTIFY, verifies OxideDNS publishes the updated serial/data from a mode 1 IXFR response, and can retain config, initial/updated zone, primary-version, proxy-log, Knot/OxideDNS logs, readiness, query-output, metrics, and summary artifacts with `OXIDEDNS_KNOT_IXFR_ARTIFACT_DIR`.
- `scripts/interop-knot-xot-docker.sh` starts Knot DNS with an XoT listener, verifies ALPN `dot`, starts OxideDNS with `transport = "xot"` and a generated trust anchor, waits for JSON `/readyz`, verifies served data and transfer metrics from the transferred zone, and can retain ALPN, certificate, readiness, metrics, log, query-output, primary-version, and summary artifacts with `OXIDEDNS_KNOT_XOT_ARTIFACT_DIR`.
- `scripts/interop-knot-xot-tsig-docker.sh` starts Knot DNS with an XoT listener and TSIG-restricted transfer ACL, verifies unsigned XoT AXFR rejection and signed XoT AXFR success, starts OxideDNS with `transport = "xot"` plus the matching TSIG key, verifies served data, transfer metrics, and secret redaction, and can retain redacted config, ALPN, certificate, readiness, metrics, log, query-output, primary-version, and summary artifacts with `OXIDEDNS_KNOT_XOT_TSIG_ARTIFACT_DIR`.
...
- `scripts/interop-rrl-udp.sh` starts a fake AXFR primary and verifies runtime UDP RRL drop/slip behavior for all response categories plus Prometheus RRL counters, with optional retained raw artifacts.
- `scripts/rrl-evidence-campaign.sh` wraps the RRL UDP interop script for repeated retained runs, with dry-run/list-config modes, an RRL threshold-decision artifact, and aggregate TSV/env output for reviewable campaign setup and release evidence.
- `scripts/release-evidence-snapshot.sh` special-cases the RRL campaign when `OXIDEDNS_EVIDENCE_RUN_RRL_CAMPAIGN=1` or when the gap-register interop command list is enabled, so campaign raw artifacts are retained inside the snapshot instead of an unrelated `target/rrl-evidence/` directory.
- `scripts/interop-dnssec-serve.sh` starts a fake AXFR primary carrying DNSKEY, DS, RRSIG, NSEC, wildcard, delegation, and large TXT records, verifies OxideDNS serves DO-sensitive positive, NXDOMAIN, wildcard, direct DS, signed-child referral DS, and unsigned-child referral NSEC no-DS DNSSEC augmentation, verifies non-DO suppression for positive, NXDOMAIN, NODATA, wildcard, signed-child referral, and unsigned-child referral responses, serves direct DNSKEY plus explicit non-DO RRSIG/NSEC queries, sends AD/CD-set client queries and verifies all covered response categories clear AD/CD, handles DNSSEC UDP truncation/response-DO semantics, proves non-EDNS UDP truncation stays within 512 octets without adding a response OPT, and can retain fake-primary, client, config, metrics, summary, and OxideDNS log artifacts with `OXIDEDNS_DNSSEC_SERVE_ARTIFACT_DIR`.
- `scripts/interop-dnssec-nsec3-serve.sh` starts a fake AXFR primary carrying DNSKEY, RRSIG, NSEC3, and NSEC3PARAM records, verifies direct NSEC3/NSEC3PARAM serving, verifies DO-sensitive NXDOMAIN NSEC3 proof material with covering RRSIGs, and can retain fake-primary, client, config, metrics, summary, and OxideDNS log artifacts with `OXIDEDNS_DNSSEC_NSEC3_ARTIFACT_DIR`.
- `scripts/interop-negative-responses.sh` starts a fake AXFR primary with a representative unsigned zone, verifies retained runtime evidence for NXDOMAIN, NODATA, empty non-terminal, CNAME negative terminal, CNAME NODATA terminal, DNAME out-of-zone terminal, out-of-zone REFUSED, direct SOA positive TTL, SOA negative TTL, and zone/global RCODE metrics, and can retain client, config, metrics, logs, summary, and `negative-response-traceability.tsv` artifacts with `OXIDEDNS_NEGATIVE_RESPONSE_ARTIFACT_DIR`.
- `scripts/interop-notify-negative.sh` starts a fake AXFR primary and OxideDNS with NOTIFY handled on the DNS interface, verifies malformed NOTIFY FORMERR, unknown-zone REFUSED, accepted refresh signalling/deduplication over UDP and TCP with QID/question/AA response checks, unauthorized-source discard, required-TSIG BADKEY, valid signed NOTIFY with signed NOERROR response, tampered signed NOTIFY with BADSIG, repeated unauthorized and TSIG-failure log-rate suppression summary, NOTIFY metrics, accepted/deduplicated/warning log events, and can retain client, config, metrics, log, summary, and `notify-traceability.tsv` artifacts with `OXIDEDNS_NOTIFY_NEGATIVE_ARTIFACT_DIR`.
- `scripts/interop-tcp-truncation-retry.sh` starts a fake AXFR primary with a large A RRset and a question-section-preserving AXFR response, verifies a non-EDNS UDP query receives TC=1 at the 512-octet ceiling, verifies the same query over TCP receives the complete untruncated answer, verifies an intentionally large-then-small pipelined TCP pair completes out of order with matching IDs while retaining response order, timing, and size evidence, verifies an over-limit TCP connection is closed with log evidence, verifies SIGTERM puts `/readyz` into draining while an accepted TCP query completes, and can retain client, pipeline, metrics, config, timing summaries, readiness, log, and `tcp-transport-traceability.tsv` artifacts with `OXIDEDNS_TCP_TRUNCATION_ARTIFACT_DIR`.
- `scripts/interop-edns-behavior.sh` starts a fake AXFR primary with a large RRset and verifies retained running-service EDNS behavior for valid OPT parsing, malformed option FORMERR, duplicate OPT FORMERR, OPT outside the additional section FORMERR, BADVERS response encoding, advertised-payload floor, exact floor, below/exact/above configured maximum UDP ceiling truncation, response OPT presence and field values, DO clearing without DNSSEC augmentation, UDP keepalive ignore, TCP keepalive timeout advertisement, configured padding alignment, unknown-option ignore, non-EDNS 512-octet truncation without OPT, configured NSID with empty and non-empty request data, metrics, and `edns-traceability.tsv` with `OXIDEDNS_EDNS_BEHAVIOR_ARTIFACT_DIR`.
- `scripts/interop-dns-cookie-dig.sh` starts a fake AXFR primary, verifies BIND `dig` no-cookie, client-cookie-only, valid-server-cookie, invalid-server-cookie lenient, strict client-cookie-only BADCOOKIE, strict invalid-server-cookie BADCOOKIE, and strict valid-server-cookie retry exchanges against the running UDP server, checks cookie metrics and startup/BADCOOKIE logging, and can retain dig output, metrics, config, log, summary, and traceability artifacts with `OXIDEDNS_DNS_COOKIE_ARTIFACT_DIR`.
- `scripts/interop-unknown-rr.sh` starts a fake AXFR primary carrying private-use and unassigned future numeric RR types, verifies exact numeric QTYPE lookups against OxideDNS, checks zero-length, pointer-looking, case-different, and future-type unknown RDATA are emitted with exact RDLENGTH/RDATA bytes, and can retain fake-primary, client, config, readiness, metrics, log, summary, and `unknown-rr-traceability.tsv` artifacts with `OXIDEDNS_UNKNOWN_RR_ARTIFACT_DIR`.
- `scripts/interop-unknown-rr-bad-transfer.sh` starts a fake AXFR primary for each prohibited pseudo/meta/reserved transfer-content type in `ODS-FR-URR-009`, verifies OxideDNS rejects the transfer, remains not-ready with zero active zones, records a failed AXFR metric, and can retain per-case config, primary logs, OxideDNS logs, readiness, metrics, summary, and `unknown-rr-bad-transfer-traceability.tsv` artifacts with `OXIDEDNS_UNKNOWN_RR_BAD_ARTIFACT_DIR`.

Non-functional evidence foundations:

- `scripts/perf-smoke.sh` starts a synthetic 1,000-record fake AXFR primary with a question-section-preserving transfer response, measures startup-to-ready time after launching OxideDNS, confirms transfer metrics and SOA serial publication, and runs a small UDP direct-hit latency sample against the transferred zone. With `OXIDEDNS_PERF_SMOKE_METRICS_OUT`, it writes retained machine-readable smoke metrics for release snapshots; with `OXIDEDNS_PERF_SMOKE_ARTIFACT_DIR`, it retains raw `/metrics`, `/readyz`, OxideDNS, primary, and client artifacts including build-info and query-latency histogram evidence. This is a repeatable smoke harness for performance evidence collection, not final ODS-NFR-PERF conformance.
...
- `scripts/audit-readonly-runtime.sh` starts a fake AXFR primary, runs OxideDNS with a non-writable `TMPDIR`, waits for `/readyz`, verifies a UDP answer from the loaded zone, confirms zero child processes while recording thread count through `/proc`, and checks file-write intent when `strace` is available. With `OXIDEDNS_READONLY_RUNTIME_CONTAINER=1` or `auto`, it also starts a fake-primary container plus a `--read-only` OxideDNS container on a private Docker network, verifies readiness, query serving, denied `/tmp` writes, zero child processes, and records container inspect, mountinfo, process, log, config, metrics, and client artifacts. With `OXIDEDNS_READONLY_RUNTIME_ARTIFACT_DIR`, it retains config, logs, metrics, process-status, client summary, optional syscall trace artifacts, and any container pass/skip evidence.
- `scripts/capture-malformed-query-evidence.sh` starts a fake AXFR primary and running OxideDNS, sends a retained malformed DNS query corpus over both UDP and TCP, verifies a valid query before and after the corpus, records `/livez`, `/readyz`, metrics, per-case outcomes, and an empty panic-log scan, and can retain those artifacts with `OXIDEDNS_MALFORMED_QUERY_EVIDENCE_DIR`.
- `scripts/capture-portability-evidence.sh` builds and runs the current host binary, captures Linux OS, architecture, Rust target, linked-library, container-runtime inventory, IPv4/IPv6 TCP/UDP loopback probes, and static first-party runtime/config scan evidence for init-system, package-manager, and distribution-layout independence with `OXIDEDNS_PORTABILITY_EVIDENCE_DIR`.
- `scripts/capture-resource-evidence.sh` builds the release binary, starts a short local zero-query runtime profile, and retains release binary size, runtime file-descriptor count versus the configured SRS formula bound, `/proc` status/limits/RSS, and idle CPU sample artifacts with `OXIDEDNS_RESOURCE_EVIDENCE_DIR`.
- `scripts/capture-coverage-evidence.sh` runs `cargo llvm-cov --workspace --summary-only`, retains JSON/env/TSV coverage artifacts with `OXIDEDNS_COVERAGE_EVIDENCE_DIR`, and enforces the `ODS-NFR-MAINT-007` 70% overall and 85% parser/XoT-file line-coverage thresholds.
- `scripts/capture-unsafe-dependency-evidence.sh` runs `cargo geiger` against the release binary root, retains package-level first-party and transitive unsafe counts, first-party expected-count checks, scanner warnings, unscanned-file lists, and requirement traceability with `OXIDEDNS_UNSAFE_DEPENDENCY_EVIDENCE_DIR`. It is part of `scripts/check.sh` so the first-party expected-count gate runs continuously; `geiger-summary.env` records whether the scanner output is complete or partial for release review.
- `scripts/check-unsafe-prone-dependencies.py` blocks listed low-level crates while their boundary rows are deferred and, for any current unsafe-prone dependency, verifies first-party Rust references are confined to the dependency row's declared adapter `allowed_paths`.
...
- `scripts/audit-unused-code.sh` records repeatable code-hygiene evidence by running strict compiler `unused`/`dead_code`/`unreachable_pub`/`unused_crate_dependencies` lints, `cargo machete` unused-dependency analysis, and `cargo bloat` release-binary crate/symbol attribution for `oxidedns`.
- `scripts/check-functional-requirement-references.py` enforces SRS v0.7 `ODS-NFR-MAINT-004` by parsing section 4 functional requirement IDs and requiring every ID to appear in a Rust source comment at a principal implementation owner.
...
- `scripts/engineering-mvp-evidence.sh` captures the narrow Engineering MVP gate: repository checks, parser fuzz compile, invariant audit, read-only runtime audit, malformed-query runtime evidence, portability evidence, resource evidence, coverage evidence, anti-spoofing evidence audit, canonical log-field audit, lazy log-formatting audit, unused-code audit, performance smoke, negative-response artifacts, negative-NOTIFY artifacts, TCP truncation retry evidence, EDNS behavior artifacts, DNS Cookie dig artifacts, IXFR NOTIMP fallback artifacts, retained unknown-RR artifacts, retained prohibited unknown-RR transfer artifacts, retained BIND IXFR refresh artifacts, DNSSEC fake-primary artifacts, RRL UDP artifacts, retained BIND TSIG AXFR artifacts, and BIND AXFR and NOTIFY refresh interop logs under `target/evidence/engineering-mvp/<timestamp>/`.
- `scripts/release-evidence-snapshot.sh` captures release-review command logs, tool versions, git state, fuzz compile checks, cargo-deny output, read-only runtime artifacts, malformed-query runtime artifacts, portability artifacts, resource artifacts, coverage artifacts, unsafe dependency enumeration artifacts, functional requirement source-reference output, info verbosity handoff artifacts, interface compatibility artifacts, benchmark handoff artifacts, soak handoff artifacts, reproducible-build handoff artifacts, release-governance handoff artifacts, unused-code audit artifacts, negative-response artifacts, negative-NOTIFY artifacts, TCP truncation retry artifacts, EDNS behavior artifacts, DNS Cookie dig artifacts, IXFR NOTIMP fallback artifacts, DNSSEC fake-primary artifacts, RRL UDP artifacts, optional retained RRL campaign artifacts, and optional fuzz campaign and interop script output under `target/evidence/<timestamp>/`.
- `docs/test-plan.md` records the ODS-VER-011 Continuous/Periodic/Gate cadence mapping and the ODS-VER-012 regression policy; `scripts/check-test-plan.sh` keeps that structure in the continuous check. `.github/workflows/ci.yml` runs `./scripts/check.sh` on `main`, pull requests, manual dispatch, and a weekly schedule; `scripts/check-ci-workflow.py` keeps that workflow shape reviewable in local checks. `scripts/check-perf-regression.py` compares retained smoke metrics with a rolling history when provided. Full weekly Reference Hardware/Profile benchmark baselines remain an SRS acceptance gap, but `scripts/capture-benchmark-handoff.sh` now records the local MVP runbook, metric/resource schemas, baseline-history format, requirement traceability, release-note snippet, and sign-off scaffold for later release/operations execution.
- `scripts/capture-info-verbosity-handoff.sh` records the local MVP setup artifact for later release/operations profiling of `info` verbosity under production-representative traffic: runbook, report template, log-volume/structured-field/metrics TSV schemas, requirement traceability map, release-note snippet, environment values, and operator sign-off scaffold. It is captured by the release evidence snapshot but does not claim production-depth profiling has been executed.
- `scripts/capture-interface-compatibility-evidence.sh` records the local MVP setup artifact for ODS-NFR-MAINT-006 interface compatibility: the current baseline, semantic-versioning policy, optional previous-baseline diff, requirement traceability map, release-note snippet, and release-engineer sign-off scaffold. Without `OXIDEDNS_PREVIOUS_INTERFACE_BASELINE`, it establishes the current baseline but does not claim release-to-release compatibility review has completed.
- `scripts/capture-benchmark-handoff.sh` records the local MVP setup artifact for later release/operations execution of the Reference Hardware/Profile benchmarks: runbook, report template, metric/resource TSV schemas, baseline-history template, requirement traceability map, release-note snippet, environment values, and operator sign-off scaffold. It is captured by the release evidence snapshot but does not claim that production benchmarks have been executed.
- `scripts/capture-soak-handoff.sh` records the local MVP setup artifact for later release/operations execution of the ODS-NFR-REL-003 30-day soak: report template, RSS/file-descriptor/metrics/event TSV schemas, requirement traceability map, weekly summary template, environment values, and operator sign-off scaffold. It is captured by the release evidence snapshot but does not claim the long-running soak has been executed.
- `scripts/capture-reproducible-build-handoff.sh` records the local MVP setup artifact for later release/operations execution of the ODS-NFR-MAINT-005 independent reproducible-build comparison: fixed `OXIDEDNS_BUILD_*` and `SOURCE_DATE_EPOCH` inputs, lockfile/toolchain hashes, `cargo metadata --locked`, runbook, artifact-manifest and comparison TSV schemas, requirement traceability map, release-note snippet, and release-engineer sign-off scaffold. It is captured by the release evidence snapshot but does not claim two independent bit-identical builds have completed.
- `scripts/capture-release-handoff.sh` records the local MVP setup artifact for release-governance execution: release attachment map, role ownership TSV, scheduled CI/manual-run plan, signing runbook, release-notes fill plan, external-operator acceptance scaffold, and release-readiness checklist. It is captured by the release evidence snapshot but does not claim release acceptance has been completed.
- `scripts/audit-safe-rust.sh` verifies the workspace `unsafe_code = "forbid"` lint, rejects unregistered `#![allow(unsafe_code)]` attributes, and scans first-party Rust source for unsafe construct candidates, allowing only the audited POSIX signal-disposition and file-descriptor rlimit FFI modules.
- `scripts/audit-maintainability.sh` records the first-party production Rust source line count excluding `#[cfg(test)]` code, enforces the current 11-module map against `docs/architecture.md`, and reports the current `ODS-NFR-MAINT-001` line-count target status for release review.
- `docs/dnssec-conformance-matrix.tsv` records the Engineering MVP DNSSEC serving conformance posture for `ODS-FR-DNSSEC-001..013`, and `scripts/check-dnssec-conformance-matrix.py` keeps that matrix complete and tied to existing local evidence paths without claiming final SRS acceptance.
