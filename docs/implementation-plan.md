...
- `scripts/engineering-mvp-evidence.sh` captures the narrow Engineering MVP gate: repository checks, security-policy checks, CLI evidence, parser fuzz compile, invariant audit, read-only runtime audit, malformed-query runtime evidence, portability evidence, resource evidence, coverage evidence, unsafe-dependency evidence, interface-compatibility evidence, anti-spoofing evidence audit, canonical log-field audit, lazy log-formatting audit, unused-code audit, functional-requirement reference checks, performance smoke, negative-response artifacts, negative-NOTIFY artifacts, TCP truncation retry evidence, EDNS behavior artifacts, DNS Cookie dig artifacts, IXFR NOTIMP fallback artifacts, retained unknown-RR artifacts, retained prohibited unknown-RR transfer artifacts, retained BIND IXFR refresh artifacts, DNSSEC fake-primary artifacts, RRL UDP artifacts, retained BIND TSIG AXFR artifacts, and BIND AXFR and NOTIFY refresh interop logs under `target/evidence/engineering-mvp/<timestamp>/`.
