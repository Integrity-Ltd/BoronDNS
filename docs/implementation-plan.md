...
- `scripts/interop-dnssec-serve.sh` starts a fake AXFR primary carrying DNSKEY, DS, RRSIG, NSEC, wildcard, delegation, and large TXT records, verifies OxideDNS serves DO-sensitive positive, NXDOMAIN, wildcard, direct DS, signed-child referral DS, and unsigned-child referral NSEC no-DS DNSSEC augmentation, verifies non-DO suppression for positive, NXDOMAIN, NODATA, wildcard, signed-child referral, and unsigned-child referral responses, serves direct DNSKEY plus explicit non-DO RRSIG/NSEC queries, clears AD/CD, handles DNSSEC UDP truncation/response-DO semantics, proves non-EDNS UDP truncation stays within 512 octets without adding a response OPT, and can retain fake-primary, client, config, metrics, summary, and OxideDNS log artifacts with `OXIDEDNS_DNSSEC_SERVE_ARTIFACT_DIR`.
