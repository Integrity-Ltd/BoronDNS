...
- AXFR response parsing validates QID, OPCODE, RCODE, initial and terminating SOA, rejects answer records after the terminating SOA, and validates class, bailiwick, reserved RR types, fixed-length A/AAAA RDATA, uncompressed single-name DNAME target RDATA, presence of an apex NS RRset, CNAME coexistence restrictions with DNSSEC exceptions, and DNAME/CNAME non-coexistence;
...
- IXFR fault coverage rejects mismatched QIDs, unexpected opcodes, error RCODEs, missing initial SOAs, incomplete newer single-SOA responses, mismatched starting old-SOA serials, final SOA chain mismatches, absent-record deletions, already-present record additions, class mismatches, out-of-zone owners, reserved record types, invalid fixed-length A/AAAA RDATA, invalid DNAME target RDATA, and final updated zones that violate apex NS, CNAME coexistence, or DNAME/CNAME coexistence requirements.
