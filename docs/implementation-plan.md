...
- JSON logs include an RFC 3339 UTC timestamp, level, target, message, and structured event key-value fields; logfmt logs include the same canonical core fields as `key=value` pairs and preserve event fields; plain logs use the standard `tracing-subscriber` text formatter and remain outside the final JSON/logfmt acceptance claim.
