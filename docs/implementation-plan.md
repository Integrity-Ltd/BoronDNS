...
- `[process].run_as_user` configures the unprivileged target for root-launched deployments; runtime startup resolves the account, binds configured DNS/management sockets, drops supplementary groups plus real/effective/saved gid and uid through the audited POSIX adapter, and only then starts DNS listeners, transfer workers, health listeners, and background tasks;
...
- `scripts/audit-maintainability.sh` records the first-party production Rust source line count excluding `#[cfg(test)]` code, enforces the current 12-module map against `docs/architecture.md`, and reports the current `ODS-NFR-MAINT-001` line-count target status for release review.
