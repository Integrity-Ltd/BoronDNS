...
- AXFR response parsing validates QID, OPCODE, RCODE, initial and terminating SOA, rejects answer records after the terminating SOA, normalizes permitted compressed RDATA names for SOA/NS/CNAME/PTR/MX while preserving unknown-type RDATA opaquely, and validates class, bailiwick, reserved RR types, fixed-length A/AAAA RDATA, uncompressed post-RFC3597 embedded names for SRV/NAPTR/DNAME/RRSIG/NSEC/SVCB/HTTPS, SVCB parameter framing and ordering, exactly one apex SOA in the final zone, presence of an apex NS RRset, CNAME coexistence restrictions with DNSSEC exceptions, and DNAME/CNAME non-coexistence;
