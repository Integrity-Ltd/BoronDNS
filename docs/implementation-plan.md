...
- `scripts/interop-dnssec-serve.sh` starts a fake AXFR primary carrying DNSKEY, DS, RRSIG, NSEC, and large TXT records, verifies OxideDNS serves DO-sensitive positive, NXDOMAIN, direct DS, and signed-child referral DNSSEC augmentation, serves direct DNSKEY plus explicit non-DO RRSIG/NSEC queries, clears AD/CD, handles DNSSEC UDP truncation/response-DO semantics, proves non-EDNS UDP truncation stays within 512 octets without adding a response OPT, and can retain fake-primary, client, config, metrics, summary, and OxideDNS log artifacts with `OXIDEDNS_DNSSEC_SERVE_ARTIFACT_DIR`.
