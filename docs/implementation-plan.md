...
- `GET /metrics` exposes minimal Prometheus text gauges for configured and ACTIVE zones, per-zone LOADING/ACTIVE/EXPIRED state, held SOA serials, and AXFR/IXFR transfer-session started, completed, and failed counters;
...
- richer per-zone scheduler metrics: last successful refresh time, next scheduled refresh time, refresh failures since last success, and query counters;
