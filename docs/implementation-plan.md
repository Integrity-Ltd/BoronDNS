...
- `scripts/capture-unsafe-dependency-evidence.sh` runs `cargo geiger` against the release binary root, retains package-level first-party and transitive unsafe counts, first-party expected-count checks, scanner warnings, unscanned-file lists, and requirement traceability with `OXIDEDNS_UNSAFE_DEPENDENCY_EVIDENCE_DIR`. This is release-review evidence; `geiger-summary.env` records whether the scanner output is complete or partial.
