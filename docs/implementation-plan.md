...
- `scripts/perf-smoke.sh` starts a synthetic 1,000-record fake AXFR primary, measures startup-to-ready time after launching OxideDNS, confirms transfer metrics and SOA serial publication, and runs a small UDP direct-hit latency sample against the transferred zone. With `OXIDEDNS_PERF_SMOKE_METRICS_OUT`, it writes retained machine-readable smoke metrics for release snapshots. This is a repeatable smoke harness for performance evidence collection, not final ODS-NFR-PERF conformance.
